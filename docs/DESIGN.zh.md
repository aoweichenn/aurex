...
5. C ABI 测试：验证 `extern c`、`export c`、标准库 native 支持调用和 host linker 行为。
...
- `selfhost/src/aurex/selfhost/bin/aurexc_seed.ax` 是第一个 M0 seed；
...
- `selfhost/src/aurex/selfhost/tool/lexer_file.ax` import 共享 dump helper，通过 `std` native 支持读取源码文件，并输出可 golden 对比的 token kind 流；
...
- `aurexc_stage1` 当前输出 `aurex_ir v0` 快照；对尚未被 parser seed 覆盖的
