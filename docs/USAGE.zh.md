...
```text
source -> lexer -> tokens -> parser -> AST -> sema -> checked module -> Aurex IR -> backend
```
...
当前 LLVM 后端通过 CMake 的 `find_package(LLVM CONFIG)` 接入。配置阶段会打印
LLVM 版本和 `LLVM_DIR`，换环境时推荐显式传入：
...
- `phi`，用于表达 `&&` / `||` 这类短路控制流值。
- `const` 和 enum case 的全局常量表。LLVM 后端会把它们物化为只读
  `global constant`，再从运行期引用处生成 load。
