...
As of 2026-06-01, M6-WP2 through M6-WP7 have completed the M6 implementation
baseline. WP2 adds compiler-owned `Copy`, internal
...
nodes with verifier and backend scaffolding. WP5 and WP6 add destructor body
identity, stable drop-glue keys, a target-independent drop-glue planner, IDE
resource hover projection, generic parameter hover fallback, and the `aurex-lsp`
stdio entry point. WP7 closes the implementation docs and records M7 as the
CFG-sensitive origin / loan / lifetime checker entry.
...
normal negative samples. User destructor syntax, custom destructor lowering,
aggregate rollback codegen, and the complete borrow checker are not implemented
yet. The next implementation package is M7 CFG-Sensitive Origin, Loan, And
Lifetime Checking.
...
monomorphization.
M4-WP6 completes the first associated type model: trait bodies accept
`type Item;`, trait impl bodies assign `type Item = Type;`, `Self.Item` and
generic projections lower to canonical associated-projection types,
`Trait[Item = Type]` where predicates lower to trait predicates plus associated
type equality facts, impl method requirement matching substitutes impl
associated type outputs, generic method calls can normalize through equality
...
associated-type normalization, inherent-first priority, and checked dump origin
...
`trait_default_method_override_dispatch.ax`,
`trait_default_method_generic_dispatch.ax`,
`trait_default_method_associated_type_dispatch.ax`, and
`trait_default_method_inherent_precedence.ax` cover lowering; and
...
the default-body diagnostics and completeness rule.
...
diagnostics;
`tests/samples/positive/traits/trait_associated_type_basic.ax` and
`tests/samples/positive/traits/trait_associated_type_where_equality.ax` cover
WP6 projection, impl output substitution, equality predicates, and runtime
lowering; the `trait_associated_type_*.ax` negative samples cover WP6
diagnostics for ambiguity, duplicate equality, built-in equality, duplicate
...
generic associated types, and RAII/resource semantics remain separate future
designs. The WP6 `where` grammar supports identifier trait predicates with
associated-type equalities, but qualified where predicates and generic trait
predicate arguments remain future solver work.
