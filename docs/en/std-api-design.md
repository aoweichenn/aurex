...
- Constraint: `Vec<T>` cannot be implicitly copied. Use `take()` when a buffer must move from one owner slot to another. `destroy<T>` frees only the Vec buffer; it does not destroy element payloads. When `T` has a `destroy(self: *mut T) -> void` destructor, use `destroy_deep/clear_deep/truncate_deep` to destroy elements. For now, `extend/from_span/get/set/first/last/pop/remove/swap_remove/clear/truncate` still require a copyable element type; noncopy read/remove support should wait for explicit move-out APIs, borrowed views, and a regular `drop` capability.
