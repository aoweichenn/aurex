...
> **当前状态（2026-05-18）：** 本文保留为 M2 审计历史快照和修复方案说明，下面的原始问题证据仍用于追溯，但“未修 / 过渡态 / 剩余 17 项”等旧状态不再代表当前 `m2`。当前权威状态以 `docs/zh/next-steps.md`、`docs/zh/language-feature-inventory.md` 和本节状态快照为准。
...
**结论：** M2 语法表面已接近可用；本文早期指出的 P0/P1 类型系统边界、泛型约束、unsafe projection、上下文类型推导问题已经按当前 M2.1 边界收口。后续工作不再是 M2.1 补洞，而是面向 M2.5 的 query、lossless syntax、IDE/LSP 和跨机器 perf 校准。
...
**方案：** `&x` / `&mut x` 永远是 safe reference。raw pointer 必须显式进入 unsafe 边界：先用 `ptraddr(...)` 取得地址，再用 `ptrat[T](...)` 恢复目标 raw pointer 类型；`raw_addr` 是历史方案用语，当前语法不再采用。
...
> **M2.1 当前已把 Aurex M2 从“样例能跑的前端”推进到“有资格进入 M3/M2.5 架构化工作的工业级前端骨架”。**

---

## 第十三部分：M2.5 后续路线图

> **当前优先级声明：** M2.1 的 23 项修复已按当前边界收口；以下内容现在是后续主线，优先级高于继续扩写历史 M2.1 补丁清单。
...
> **M2.1 已经以"修 bug"为名完成一次前端地基升级。当前主线转向 M2.5 query / lossless syntax / IDE-native 架构。**
...
|:-----|:------------------|:----------------------|
| Match 穷尽性 | 历史过渡方案是加硬上限 4096 并跳过；当前已删除组合枚举 | ✅ 已改为 Witness Search / Usefulness Algorithm |
...
> M2.1 已按当前 capability 边界收口；Hash 跨编译稳定性仍明确归入 M4/M5 ABI 闭包任务。
>
...
>
> **M2.1 已经把这些基础类型和边界固定下来；M2.5 可以在此基础上推进增量编译、IDE 和 query 化，而不是推倒重来。**

> **当前优先级声明：** 以下内容已经从“修复完成后的后续方向”转为当前后续主线；M2.1 历史清单不再阻塞这些工作。
...
> **最终答案：前面那些方案不是全部“终局最佳”，但方向是正确的。M2.1 已经以“修 bug”为名完成了一次前端地基升级；当前主线转向 M2.5 query / lossless syntax / IDE-native 架构。**
...
| 📋 采纳程度 | 审计报告已入库，创建了 229 行 next-steps.md 详细计划 |
| 🧪 新增测试 | 7 个（5 个 unsafe + match 上限 + 数组越界） |
| 🐛 后续问题 | LSP 结构化诊断 protocol、跨机器 perf 阈值校准、M2.5 query / lossless syntax / IDE-native 架构 |
