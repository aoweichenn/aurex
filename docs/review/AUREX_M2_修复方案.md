...
**开发者修复状态：** ✅ 已关闭 — capability 已改为结构化 `CapabilityKind`，where clause 和 generic instantiation 使用统一能力规则；direct operator availability 与 capability satisfaction 明确分层。Eq/Ord 排除 float 和 reference，float 仍支持直接比较 operator；Hash 当前是 marker-only admission predicate，支持 `bool` / `char` / integer / pointer，但不提供 hash operator、稳定 hash ABI 或用户自定义实现。
