...
> **当前状态（2026-05-17）：** 本文是 M2 审计历史快照，原始证据和建议保留用于追溯。当前实现状态不再以本文早期的“建议/缺失/待修复”段落为准，而以 `docs/zh/next-steps.md`、`docs/zh/language-feature-inventory.md` 和下面快照为准。
...
| 本次补充修复 | 已纳入 | `syntax::Token` 压缩为 source pointer + range 计算 text，parser/module import 路径 lex+parse 后立即释放 token arena |
