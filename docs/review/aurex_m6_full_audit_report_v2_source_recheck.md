...
- `src/backend/llvm/emission/llvm_backend_module.cpp:79-96` 后端使用 `llvm::sys::getDefaultTargetTriple()` 并设置 LLVM DataLayout；Sema layout 与 LLVM DataLayout 没有通过统一 TargetInfo/DataLayout 贯通。
...
- `include/aurex/frontend/syntax/core/token.hpp:145-159` `Token` 持有 `const char*` 指向源文本，生命周期没有类型表达。
...
- `include/aurex/frontend/parse/token_cursor.hpp:21-35` 的 `peek()/previous()` 在空 token span 下直接 `back()/front()`。
- `include/aurex/frontend/parse/token_cursor.hpp:52-68` 用 `current_ + 1`、`current_ + offset`，无溢出检查。
- `include/aurex/frontend/lex/lexer_cursor.hpp:60-67`、`:108-122` 依赖 `offset_ + lookahead` 和 assert-only slice。
...
- `include/aurex/infrastructure/base/bump_allocator.hpp:96-105` arena 为空时 adapter 隐式退到全局 new/delete，容易掩盖 arena owner bug。
- `include/aurex/infrastructure/base/result.hpp:49-70` `value()` / `error()` 标 `noexcept`，但内部 `std::get` 在误用时会抛 `bad_variant_access`，最后变成 terminate。
...
- `include/aurex/frontend/sema/resource_semantics.hpp:87-89` 通过 `mutable` map 和 `indexes_built_` 缓存。
...
- `include/aurex/infrastructure/query/query_provider_set.hpp:23-50` 所有 provider 都是 `std::function<std::optional<...>(...)>`。
...
- `include/aurex/application/tooling/session.hpp:134-138` 明确注释 `ToolingSourcePosition` 是 line + byte character。
...
- `tools/format_check.py --base HEAD include/aurex/frontend/sema/sema_messages.hpp include/aurex/application/tooling/lsp.hpp src/sema/internal/sema_statement_analyzer.cpp src/tooling/lsp.cpp src/tooling/lsp_stdio.cpp tests/gtest/integration/sample_suite_tests.cpp tests/gtest/tooling/session_lsp_tooling_tests.cpp`
- `git diff --check`
