...
文档基线：**M11d Principal-Set Composition IR / Backend Runtime**，建立在已经收口的 M2 language-core-no-std、
...
principal index 取出目标 vtable。当前仍不实现标准库、owning dyn runtime、`Box<dyn Trait>`、dynamic Drop
dispatch runtime、allocator policy、bare `dyn A + B` syntax 或 direct principal-qualified composition method
dispatch；下一步是 M11e hardening/release closure，且仍不引入标准库或 owning dyn。
