...
文档基线：**M13b Borrowed Composition-To-Supertrait Frontend / Query / Sema Check-Only**，建立在已经收口的 M2 language-core-no-std、
...
`CompositionProjectionFact{kind=composition_to_supertrait}`。M13b 仍不做 IR/backend runtime lowering，也继续拒绝
