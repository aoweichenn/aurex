...
文档基线：**M7b Borrow Contract、Reborrow 与 Lifetime Surface 设计基线**，建立在已经收口的 M2 language-core-no-std、
M2.5 frontend-foundation、M3 query-backed/module/generic、M4 trait/protocol 和 M5 default trait methods
基线、M6 资源/cleanup/drop-glue release baseline 以及 M7a CFG-sensitive borrow facts 实现收口基线之上。
