...
第二优先级中 slice type/expression、tuple/destructuring、function pointer / function type、raw/multiline raw string、bytes string、Unicode scalar `char`、numeric suffix 和 M2 pattern ergonomics 已补齐；下一步继续补 slice pattern、binding or-pattern alternatives 和 `let ... else`。完整 closure 捕获、trait/interface/protocol、borrow checker、macro、async、package manager 和库层重建继续暂缓。
...
状态：已补。后续 `unsafe {}`、`defer`、pattern condition 和 `let ... else` 可以基于同一套 block body 规则扩展；资源语义如果重启，也不需要再为 expression block 单独补 statement 子集。
...
- slice pattern、binding or-pattern alternatives 和 `let ... else` 仍属于后续 pattern 扩展。
- struct pattern、nested enum payload pattern、`if value is pattern` / `while value is pattern`、if 表达式 pattern condition 和 match tuple pattern 已补齐。
...
6. tuple/destructuring/pattern ergonomics 已落地，当前支持 `(A, B)` / `(A,)`、`(a, b)` / `(a,)`、`value.0`、局部 `let (a, _) = value;`、局部 struct destructuring、match tuple/struct pattern、nested enum payload pattern、`if value is pattern`、`while value is pattern` 和 if 表达式 pattern condition，空 tuple 暂缓。
7. 冻结 namespace `.` / `::` 规则。
...
1. shadowing 策略。
2. slice pattern、binding or-pattern alternatives 和 `let ... else` 等 pattern 扩展。
3. 嵌套 block comment。
