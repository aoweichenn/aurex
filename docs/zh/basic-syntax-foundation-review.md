...
第二优先级中 slice type/expression、tuple/destructuring、function pointer / function type、raw/multiline raw string、bytes string、Unicode scalar `char` 和 numeric suffix 已补齐；下一步继续补 `if let` / `let ... else` / struct pattern / match tuple pattern。完整 closure 捕获、trait/interface/protocol、borrow checker、macro、async、package manager 和库层重建继续暂缓。
...
- tuple local destructuring 已补齐；match tuple pattern 仍属于后续 pattern 扩展。
...
6. tuple/destructuring 已落地，当前支持 `(A, B)` / `(A,)`、`(a, b)` / `(a,)`、`value.0` 和局部 `let (a, _) = value;`，空 tuple 与 match tuple pattern 暂缓。
7. 冻结 namespace `.` / `::` 规则。
...
1. shadowing 策略。
2. struct pattern、match tuple pattern、`if let` / `let ... else` 等 pattern 扩展。
3. 嵌套 block comment。
