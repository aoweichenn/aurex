...
第二优先级中 slice type/expression 和 function pointer / function type 已补齐；下一步继续补 raw/multiline/bytes string、Unicode scalar `char`、tuple/destructuring、`if let` / `let ... else` / struct pattern。完整 closure 捕获、trait/interface/protocol、borrow checker、macro、async、package manager 和库层重建继续暂缓。
