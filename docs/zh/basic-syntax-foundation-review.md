...
1. 最小 `unsafe` block / `unsafe fn`
   ...
   raw pointer 解引用、`ptrcast`、`bitcast`、`ptrat`、`strraw` 当前都能在普通表达式里出现。它们会破坏 pointer validity、layout、aliasing、UTF-8 等语言不变量。M2 应先加语法和诊断边界：
...
第二优先级中 slice type/expression、function pointer / function type、raw/multiline raw string、bytes string、Unicode scalar `char` 和 numeric suffix 已补齐；下一步继续补 tuple/destructuring、`if let` / `let ... else` / struct pattern。完整 closure 捕获、trait/interface/protocol、borrow checker、macro、async、package manager 和库层重建继续暂缓。
...
- struct pattern、nested pattern、`if let` / `let ... else` 仍属于后续 pattern 扩展。
...
- 这些能力如果长期保持 safe surface，后续优化器、borrow checker 和安全库层都会缺少可信边界。
...
- `strraw` 能直接构造 `str`，但当前没有 `unsafe` 语法约束。
