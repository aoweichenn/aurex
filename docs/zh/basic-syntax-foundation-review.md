...
第二优先级再补 slice type/expression、raw/multiline/bytes string、Unicode scalar `char`、function pointer / function type、tuple/destructuring、`if let` / `let ... else` / struct pattern。完整 closure 捕获、trait/interface/protocol、borrow checker、macro、async、package manager 和库层重建继续暂缓。
