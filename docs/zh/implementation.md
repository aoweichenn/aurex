...
lexer 以 source text 和 source id 为输入，输出 token 数组。token 包含 kind、文本片段和 source range。selfhost lexer 使用 `TokenSpan` 保存 kind、begin、end，用于与 Stage0 词法输出对比。
...
Stage0 parser 是手写递归下降 parser，消费 token span，输出 AST。AST 使用 ID 引用节点池，避免节点复制和指针生命周期问题。selfhost parser seed 按 cursor、type、expr、seed 拆分，表达式部分使用显式栈。
...
`.ax` 标准库模块表达语言层 API。host-c backend support 提供 selfhost IO 需要的主机符号。稳定符号统一使用 `aurex_std_v0_*`，旧 `aurex_std_*` wrapper 保留兼容。

`std/native_support.c` 是兼容 shim，它 include 当前默认 backend support。新 driver 路径直接按 backend 选择 `std/support/host_c.c`，后续可以增加新的 support backend，而不改变 `.ax` 标准库 API。
...
主测试脚本会构建工程、检查文档布局、验证 CLI help、运行正向/反向样例、检查 IR/LLVM 输出、测试 std backend、验证安装后标准库查找，并编译 selfhost 切片。
