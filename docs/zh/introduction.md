...
- 通过常规仓库测试、90% coverage gate、query/cache/profile gates、stress gates 和文档检查保持 M10d supertrait
  release baseline 与 M11a advanced dyn design gate 稳定。
...
- M11b 继续推进 principal-set composition query prototype gate；owning dyn、`Box<dyn Trait>`、dynamic Drop
  dispatch、用户可写多 trait object composition runtime、specialization、associated constants、default associated
  types 和 generic associated type 后续单独设计。
