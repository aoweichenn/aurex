...
- 下一步进入 M19 Dyn Ownership Runtime IR / Verifier Preparation；M18 已把 dyn ownership runtime boundary
  固定为 project-level query/cache/tooling/reuse/workspace facts。owning dyn、`Box<dyn Trait>`、dynamic Drop
  dispatch、标准库 allocator、specialization、associated constants、default associated types 和 generic associated
  type 仍后续单独设计；M19 仍不实现任何标准库 API。
