...
- 下一步进入 M20c Drop / Allocator Identity Prerequisite Gate；M20b 已把 compiler-owned owned dyn handle 的
  two-field IR shape、query facts、dump/fingerprint 和 verifier negative matrix 固定下来。owning dyn 用户值、
  `Box<dyn Trait>`、dynamic Drop dispatch、标准库 allocator、specialization、associated constants、default
  associated types 和 generic associated type 仍后续单独设计；M20b 仍未实现任何标准库 API。
