...
M21 已开启宏系统主线，但当前不是完整 macro/proc-macro 实现。现有真实用户可见宏能力仍只有 M20e 的编译器内建
`#[derive(Copy, Eq, Hash)]`。M21a 新增 `m21a_macro_design_gate_baseline()`，固定后续宏系统必须使用
token tree / attribute surface、hygiene、source map、debug trace、declared generated names 和 query-backed
expansion；M21b 新增 `AttributeDecl` / `AttributeTokenDecl` / `ItemNode::attributes`，parser 可以保存通用
item attribute token tree；M21c 新增 `MacroExpansionPlan` / `MacroExpansionFact` /
`m21c_macro_expansion_plan_baseline()`，固定 no-op early item expansion query key、`SourceRole::generated` /
`ModulePartKind::generated` generated part 计划、source-map stub、builtin derive passthrough 和非 `derive`
attribute sema blocker；M21d 新增 `EarlyItemExpansionResult` 和 `expand_early_item_macros_noop()`，并在
driver pipeline 中加入 `macro.expand_items`，收集 attribute query-key fingerprint、generated part placeholder 和
source-map placeholder；M21e 新增 `GeneratedModulePartParseMergeStub` 和 `GeneratedModulePartLifecycleState`，
为每个 generated placeholder 固定 deterministic `generated_buffer_identity`、`parse_config_fingerprint`、
`merge_ordering_key`、expansion origin、buffer name 和 `merge_blocked` lifecycle stub；M21f 新增
`ExpansionHygieneStub` 和 `ExpansionTraceStub`，为每个 macro input 固定 deterministic `call_site_mark`、
...
`expansion_source_map_debug_trace_v1`；M21g 新增 `GeneratedItemDeclarationStub` 和
`DeclaredGeneratedNameStub`，为每个 macro input 固定 deterministic generated item declaration、declared generated
name、`declaration_identity`、`generated_item_key`、`declared_name_identity`、`hygiene_mark`、
`attached_item_codegen_declared_names_v1`、M21f `declared_name_set` 和对应 M21d/M21e generated module part；M21h
新增 `TokenMaterializationAdmissionStub` 和 `GeneratedTokenBufferStub`，为每个 macro input 固定 deterministic
compiler-owned token materialization admission、empty generated token buffer、`token_plan_identity`、
`token_buffer_identity`、`source_map_identity`、`trace_identity`、`hygiene_mark`、
`compiler_owned_attached_item_token_materialization_admission_v1` 和 `compiler_owned_empty_token_stream`；M21i
新增 `GeneratedTokenRecord`、`materialization_identity` 和 `token_producer_policy`，为 `derive` input 固定
`compiler_owned_builtin_derive_token_stream_prototype`、`compiler_owned_builtin_derive_token_producer_prototype_v1`
和 `derive_source_token_placeholder` compiler-owned parser-blocked token record facts，非 `derive` input 继续保持
empty generated token buffer；M21j 新增 `GeneratedTokenParserAdmissionGateStub` 和 `parser_admission_gates`，
为每个 macro input 固定 `compiler_owned_generated_token_parser_admission_gate_v1`、`parse_gate_identity`、
M21e `generated_buffer_identity` / `parse_config_fingerprint`、M21i token buffer / materialization identity、
source-map identity 和 hygiene mark，并继续保持 `parser_admitted=false`、`parse_ready=false`、
`parser_consumable=false`、`generated_part_parsed=false`、`generated_part_merged=false` 和 `sema_visible=false`；
M21k 新增 `ParserAdmissionDiagnosticProjectionStub` 和 `parser_admission_diagnostics`，把 parser admission blocker
投影为 `parser_admission_blocked_diagnostic_projection_v1`，并固定 `diagnostic_identity`、
`diagnostic_anchor_identity`、`empty_token_buffer_parser_admission_blocked` /
`derive_token_buffer_parser_admission_blocked`、source anchor、token-tree anchor、token buffer admission blocker、
generated module part parse blocker、debug projection name，以及 future `--emit-expanded` / debug trace /
source-map projection unavailable states；M21l 新增 `ParserAdmissionDiagnosticReportEntry`、
`ParserAdmissionDiagnosticReport`、`parser_admission_report_entries` 和 `parser_admission_reports`，把每个
diagnostic projection 派生为 report entry，并按 generated module part 汇总成
`parser_admission_blocked_report_query_projection_v1` report，固定 `report_entry_identity`、`report_identity`、
`report_anchor_identity`、`report_grouping_identity`、`m21l-parser-admission-report:<module>:<part>` query name、
blocked / derive / empty / token-record-available totals 和 `source_anchor_ordered=true`；M21m 新增
`GeneratedTokenParserReadinessPreflightEntry` 和 `parser_readiness_preflight_entries`，把 M21i token buffer、M21j
parser admission gate、M21k diagnostic projection、M21l report entry 和 M21f source-map / hygiene / trace facts
绑定成 `generated_token_parser_consumption_readiness_preflight_v1` preflight，记录 token stream shape、
token index continuity、delimiter balance、source-anchor coverage、parse config compatibility 和 parser-blocked
flags；M21n 新增 `GeneratedTokenParserConsumptionContractGate` 和 `parser_consumption_contract_gates`，按
generated module part 汇总 M21m preflight entries，固定
`generated_token_parser_consumption_contract_gate_v1`、`contract_identity`、`contract_grouping_identity`、
`contract_anchor_identity` 和 `m21n-parser-consumption-contract:<module>:<part>` query name；M21o 新增
`MacroExpansionBoundaryClosureReport` 和 `macro_boundary_closure_reports`，把 M21m/M21n 汇总为
`m21_macro_expansion_boundary_release_closure_v1` closure，固定 `m21o-macro-boundary-closure` query name，并把
result name 推进为 `M21o Macro Expansion Boundary Release Closure`。M22a 新增 `BuiltinDeriveExpansionAdmissionGate` 和 `builtin_derive_expansion_admissions`，固定 `builtin_derive_expansion_admission_gate_v1`、`admission_identity`、`m22a-builtin-derive-admission:<module>:<part>:<item>:<attr>:<name>` query name、derive/non-derive admission kind 和 capability candidate 计数；M22b 新增 `BuiltinDeriveSemanticExpansionPlan` 和 `builtin_derive_semantic_plans`，把现有内建 `#[derive(Copy, Eq, Hash)]` capability path 投影为 `builtin_derive_semantic_expansion_plan_v1`、`capability_fact_lowering_plan`、`capability_set_identity` 和 `semantic_plan_identity`；M22c 新增 `BuiltinDeriveParserConsumptionReleaseGate` 和 `builtin_derive_parser_release_gates`，固定 `builtin_derive_parser_consumption_release_gate_v1`、`m22c-builtin-derive-parser-release:<module>:<part>`、rollback diagnostics/debug trace/source-map/hygiene prerequisites；M22d 新增 `BuiltinDeriveReleaseHardeningMatrix` 和 `builtin_derive_release_hardening_matrices`，固定 `builtin_derive_release_hardening_matrix_v1`、`m22d-builtin-derive-release-hardening:<module>:<part>`、part-local/global/cross-part 计数、negative matrix completeness 和 no-parser-consumption release hardening blocker；M22e 新增 `BuiltinDeriveDebugDumpStabilityContract` 和 `builtin_derive_debug_dump_contracts`，固定 `builtin_derive_debug_dump_stability_contract_v1`、`m22e-builtin-derive-debug-dump:<module>:<part>`、四段 dump section contract、query reuse 和 parser-blocked debug dump stability；M22f 新增 `BuiltinDeriveRollbackDiagnosticDesignGate` 和 `builtin_derive_rollback_diagnostic_gates`，固定 `builtin_derive_rollback_diagnostic_design_gate_v1`、`m22f-builtin-derive-rollback-diagnostic:<module>:<part>`、rollback prerequisite、rollback execution blocker、diagnostic/report counters 和 generated/user/std/runtime 关闭状态，并把 result name 推进为 `M22f Builtin Derive Rollback Diagnostic Design Gate`。真实宏展开、generated source text、
parser-consumable generated token buffer、真实 hygiene resolution、declared generated names lookup、真实 expansion
source map、debug trace CLI、`--emit-expanded`、typed expression macro、external procedural macro、文本替换宏、
宏生成用户代码和标准库仍未实现。
...
`m23a-builtin-derive-parser-consumption-admission:<module>:<part>`、M21n/M22c/M22f identity 链接、part-local
token buffer / token record / derive candidate / empty candidate / blocked diagnostic counts，以及
`builtin derive parser consumption admission protocol remains no-parser-consumption in M23a` blocker。
M23b 新增 `BuiltinDeriveParserConsumptionCheckpointRollbackProtocol` 和
`builtin_derive_checkpoint_rollback_protocols`，
固定 `builtin_derive_parser_checkpoint_rollback_protocol_v1`、
`m23b-builtin-derive-checkpoint-rollback:<module>:<part>`、`checkpoint_count=3`、`rollback_plan_count=3`、
diagnostic replay prerequisite 和
`builtin derive checkpoint rollback protocol remains design-only and parser-blocked in M23b` blocker。
M23c 新增 `BuiltinDeriveParserPreConsumptionVerificationClosure` 和
`builtin_derive_preconsumption_verification_closures`，
固定 `builtin_derive_parser_preconsumption_verification_closure_v1`、
`m23c-builtin-derive-preconsumption-verification:<module>:<part>`、M23a/M23b/M22d/M22e/M22f 可见性闭环，
并把 result name 推进为 `M23c Builtin Derive Parser Pre-Consumption Verification Closure`。M23a-M23c 仍只是
parser consumption 前的 facts / query / fingerprint / dump / validation closure，不解析 generated part、不 merge
AST、不产生 generated source text、不要求标准库或 runtime。

M24a 新增 `BuiltinDeriveControlledParserDryRunAdapter` 和
...
diagnostic anchor counts、`prerequisite_count=5` 和
`builtin derive controlled parser dry-run adapter remains execution-blocked in M24a` blocker。
M24b 新增 `BuiltinDeriveDryRunRollbackDiagnosticReplay` 和
`builtin_derive_dry_run_rollback_replays`，固定
...
executed replay counts 和
`builtin derive dry-run rollback diagnostic replay remains execution-blocked in M24b` blocker。
M24c 新增 `BuiltinDeriveDryRunNegativeMatrixClosure` 和
...
`M24c Builtin Derive Dry-Run Negative Matrix Closure`。M24a-M24c 仍只是 parser dry-run 前的 facts / query /
fingerprint / dump / validation closure，不执行 real parser dry-run、不消费 generated token buffer、不解析
generated part、不 merge AST、不产生 generated source text、不要求标准库或 runtime。

M25a 新增 `BuiltinDeriveParserDryRunSessionBoundary` 和
`builtin_derive_parser_dry_run_sessions`，固定
`builtin_derive_parser_dry_run_session_boundary_v1`、
`m25a-builtin-derive-dry-run-session:<module>:<part>`、M24a/M24c/M21e identity 链接、compiler-owned token buffer
candidate、`parser_state_snapshot_count=1`、`committed_parse_count=0` 和
`builtin derive parser dry-run session remains check-only and uncommitted in M25a` blocker。
M25b 新增 `BuiltinDeriveTokenCursorSnapshotRollbackProof` 和
`builtin_derive_token_cursor_snapshot_proofs`，固定
...
parser state snapshot / rollback proof counts、`cursor_commit_count=0` 和
`builtin derive token cursor snapshot rollback proof keeps parser cursor unadvanced in M25b` blocker。
M25c 新增 `BuiltinDeriveDiagnosticShadowNoAstMutationClosure` 和
...
diagnostic shadow count、`executed_shadow_count=0`、`ast_mutation_count=0`、
`parser_consumable_case_count=0` 和
`builtin derive diagnostic shadow replay remains non-executing and no-AST-mutation in M25c` blocker，并把 result name
推进为 `M25c Builtin Derive Diagnostic Shadow No-AST-Mutation Closure`。M25a-M25c 仍只是 check-only parser
dry-run sandbox facts / query / fingerprint / dump / validation closure，不执行 real parser dry-run、不执行
diagnostic shadow、不执行 rollback、不提交 session、不推进 parser cursor、不消费 generated token buffer、不解析
generated part、不 merge AST、不修改 AST、不产生 sema-visible macro output、不产生 generated source text、不要求
标准库或 runtime。

M26a 新增 `BuiltinDeriveParserDryRunAdmissionGate` 和
...
`admission_prerequisite_count=5`、token buffer / token record counts、`dry_run_execution_admitted_count=0`、
`parser_consumable_case_count=0` 和
`builtin derive parser dry-run execution admission remains blocked in M26a` blocker。
M26b 新增 `BuiltinDeriveErrorRecoveryShadowDiagnosticGate` 和
...
diagnostic shadow / parser report / planned recovery counts、`executed_recovery_count=0`、
`emitted_diagnostic_count=0` 和
`builtin derive error recovery shadow diagnostics remain non-emitting in M26b` blocker。
M26c 新增 `BuiltinDeriveCursorRollbackAstMutationVerifierClosure` 和
...
`builtin derive cursor rollback execution and AST mutation verifier remain check-only in M26c` blocker，并把 result
name 推进为 `M26c Builtin Derive Cursor Rollback AST Mutation Verifier Closure`。M26a-M26c 仍只是 check-only
admission / recovery shadow / rollback verifier facts / query / fingerprint / dump / validation closure，不准入或执行
real parser dry-run、不执行 recovery、不发出 shadow diagnostic、不执行 rollback、不提交 session、不推进 parser
cursor、不消费 generated token buffer、不解析 generated part、不 merge AST、不修改 AST、不产生 sema-visible macro
output、不产生 generated source text、不要求标准库或 runtime。
历史锚点：阶段：M26c Builtin Derive Cursor Rollback AST Mutation Verifier Closure。

M27 新增 Aurex macro surface admission：parser / AST 当前可识别 `macro Name { ... }`、
`macro derive Name { ... }` 和 `macro const Name { ... }`，记录 `ItemKind::macro_decl`、
`MacroDeclKind::declarative` / `MacroDeclKind::derive` / `MacroDeclKind::compile_time`、macro body token tree、
`macro_match_clause_count` 和 `macro_body_balanced`；AST dump 会输出 `macro_kind=...`、`body_tokens=...`、
`match_clauses=...`、`balanced=yes/no` 和 `macro_body`。`expand_early_item_macros_noop()` 会为每个 macro item
生成 `AurexMacroSurfaceAdmissionGate`，summary / dump / fingerprint 记录
`aurex_macro_surface_source_items`、`aurex_macro_surface_admissions`、
`aurex_macro_declarative_surfaces`、`aurex_macro_user_derive_surfaces`、
`aurex_macro_compile_time_surfaces` 和 parser/expansion/compile-time execution 关闭计数；validation 拒绝 source
item / gate 数不匹配、body 不平衡、identity/query 漂移、错误 surface kind、AST mutation、parser consumption、
compile-time execution、standard library、runtime、external process 或 user generated code。Query 层新增
`m27_macro_expansion_plan_baseline()`、`is_valid_m27_macro_expansion_plan()`、
...
M27c 新增 Aurex macro call-site / matcher-to-call binding / user derive target schema admission：parser / AST 当前可识别
`macro call Name { ... }`，记录 `ItemKind::macro_call`、`macro_call_tokens`、`macro_call_range` 和
`macro_call_balanced`；AST dump 会输出 `macro_call_body`；tooling session 将其投影为 `macro_call`，IDE symbol
metadata 不把它当作 stable definition。`expand_early_item_macros_noop()` 会为每个 macro call 生成
`AurexMacroCallSiteAdmissionGate`，为同模块同名且已声明的 target 生成
`AurexMacroMatcherToCallBindingAdmissionGate`，并在 `#[derive(Name)]` 匹配同模块 `macro derive Name { ... }` 时生成
`AurexUserDeriveTargetSchemaAdmissionGate`。Summary / dump / fingerprint 记录
`aurex_macro_call_site_source_items`、`aurex_macro_call_site_admissions`、
`aurex_macro_matcher_to_call_bindings`、`aurex_user_derive_target_schema_source_derives`、
`aurex_user_derive_target_schemas`、target kind、field count、enum case count 和 enum payload count；Query 层新增
`m27c_macro_expansion_plan_baseline()`、`aurex_macro_call_site_admission`、
`aurex_macro_matcher_to_call_binding_admission` 和 `aurex_user_derive_target_schema_admission`。macro call-site
expansion is admission-only in M27c；matcher-to-call binding execution is admission-only in M27c；user derive target
schema is admission-only in M27c；仍不展开宏/不执行用户编译期代码/不消费 parser/不修改 AST。

M27d 新增 Aurex macro output contract admission：`expand_early_item_macros_noop()` 会为 matcher-to-call binding 和
user derive target schema 生成 `AurexMacroOutputContractAdmissionGate`，并同步生成
`AurexMacroOutputDeclaredNamePolicyAdmissionGate` 与 `AurexMacroOutputDiagnosticProjectionAdmissionGate`。
Summary / dump / fingerprint 记录 `aurex_macro_output_contracts`、
`aurex_macro_output_declared_name_policies`、`aurex_macro_output_diagnostic_projections`、declared-name
lookup/export/sema visibility 计数和 diagnostic emission 计数；Query 层新增
...
- borrowed slice 已落地：类型语法为 `[]T` / `[]mut T`，表达式语法为 `a[l:r]`、`a[:r]`、`a[l:]`、`a[:]`，当前表示为 `ptr + len`。
...
- `str` 已有内建雏形和 no-std checked UTF-8 边界；`strvalid(bytes)` / `strfromutf8(bytes)` 接受 `[]u8` 或 `[]mut u8`，`strfromutf8` 返回 `str`，失败时返回空 `str`；`text[l:r]` 按 byte offset 做 checked slicing，越界或非 UTF-8 code point boundary 返回空 `str`；`strraw(data, len)` 继续是 unsafe-only unchecked 后门。
...
函数指针类型、闭包字面量和 M20 捕获闭包核心子集已作为基础函数式能力落地：
...
variadic 标记和返回类型都是类型身份的一部分；variadic 函数类型只允许 `extern c fn`。闭包字面量当前要求显式参数类型
和显式 `-> T`，支持 `=> expr` 表达式体和 `{ ... }` 块体。
...
TypeArgs      = "<" GenericArg ("," GenericArg)* ","? ">"
...
- `id<i32>(value)` 解析为 `generic_apply` callee 后接 `call`；`Option<i32>.some` 和 `Option<T>.some` 解析为泛型类型选择后接字段/enum case 选择；`values[0].field` 和 `values[index].field` 解析为 value `index` 后接字段选择。
- `[]` 不再承担泛型职责。`name[index]` 固定是 value index 或 slice 入口；type context 中 `[]T` / `[N]T` 仍表示 slice / array；`Pair[i32]`、`id[i32](value)`、`Source[Item = i32]` 是 legacy generic syntax 诊断输入。
- 表达式中的 `<...>` 泛型后缀不是空格敏感语法；`id<i32>(value)`、`id < i32 > (value)` 和换行排版形式解析一致。只有闭合 `>` 后接 `(`、`{` 或 `.` 时，表达式后缀才构成 generic apply；普通 `a < b > c` 保持比较表达式路径。
...
- named 参数不支持 enum constructor、function value / closure 间接调用或 variadic C call。
...
- `name[index]` 固定按 index/slice 解析，不再被 parser 或 sema 改判为泛型实参。
- 显式函数泛型实参写成 `name<T>(arg)` 或 `module.name<T>(arg)`。
- generic struct literal 只有在 `Name<Args> { ... }` 形态中解析为类型实参。
- `fn f<>`、`Box<>`、`id<>(...)` 均非法，空 `<>` 不表示推导。
- `<` / `>` 同时服务泛型 delimiter 和比较 token；parser 只在声明、类型、where/builtin type operand 和合法 postfix generic suffix 上进入泛型规则，不读取空格或 identifier 大小写做判断。
...
- range pattern。
- string/byte pattern。
- 默认参数 / 命名参数的第一版已完成；仍未完成的是默认表达式引用前序参数、function value / closure 参数名元数据、
...
- shared / mutable / consuming closure capture、generic closure environment ABI、borrowed closure environment escape、
  `Fn` / `FnMut` / `FnOnce` 风格能力和 heap/runtime closure box；M20 已支持无捕获闭包字面量与 Copy-by-value
  捕获闭包核心子集。
...
- 完整 macro / proc-macro / 用户自定义 derive；当前只有编译器内建 `#[derive(Copy, Eq, Hash)]`、M21a macro
  design gate facts、M21b 通用 item attribute token-tree AST surface、M21c-M21o no-op early expansion boundary /
  generated part / hygiene-source-map-debug-trace / token materialization / parser admission / parser consumption
  contract / boundary closure facts，以及 M22a-M22f builtin derive admission、semantic plan、parser release gate、
  release hardening matrix、debug dump stability contract 和 rollback diagnostic design gate facts，M23a-M23c
  builtin derive parser consumption admission、checkpoint rollback、pre-consumption verification closure facts，以及
  M24a-M24c builtin derive controlled parser dry-run adapter、dry-run rollback diagnostic replay、dry-run negative
  matrix closure facts，M25a-M25c builtin derive parser dry-run session boundary、token cursor snapshot rollback proof
  和 diagnostic shadow no-AST-mutation closure facts。它们都保持 real parser dry-run、diagnostic shadow execution、
  rollback execution、session commit、parser cursor advance、parser consumption、AST mutation、generated source text、
  用户宏执行、标准库和 runtime 关闭。
- comptime。
- async / coroutine。
- effect system。
...
当前规则：slice 是 `ptr + len` fat value；可从 array 或 slice 产生；`[]mut T` 可赋给 `[]T`，反向不允许；`slice[i]` 沿用 index 表达式；不包含 iterator protocol 或运行时 bounds check。
...
2. function pointer / function type（已补入 M2 基线）
   ...
   无捕获闭包字面量和 C callback 已作为基础系统能力落地：
...
6. `str` 安全边界：已补 M2 no-std checked 构造和切片
   ...
   保留 `str` 语言内建，unchecked 构造 `strraw(data, len)` 放入 unsafe；checked UTF-8 构造先作为语言内建冻结，不依赖旧 std：`strvalid(bytes) -> bool`、`strfromutf8(bytes) -> str`。两者接受 `[]u8` / `[]mut u8`；失败时 `strfromutf8` 返回空 `str`，不会把无效 byte slice 包成 `str`。`text[l:r]` 已作为基础语法落地，按 byte offset 检查 bounds 和 UTF-8 boundary；owned `String`、`Bytes`、`CStr` 等继续后置到库层/资源语义之后。
