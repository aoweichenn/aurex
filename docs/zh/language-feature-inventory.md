...
- 标准库 dyn ownership surface、owning trait-object coercion、runtime ABI lowering 和 dynamic Drop runtime；M18 已把这些能力固定为 future lowering design gate，M19 已落成 IR/verifier blocked facts，M20a 已固定 admission order，M20b 已落成 compiler-owned IR shape prototype，但这些仍不是当前语言能力。
...
- owning dyn / trait-object Drop dispatch / 歧义 composition-to-supertrait 自动选择；M11a 完成
  ...
  direct supertrait method dispatch 和 `BorrowedDynViewPathFact` release closure。M17/M18/M19/M20a/M20b 已完成 dyn ownership
  runtime preparation、runtime boundary gate、IR/verifier preparation、owned dyn runtime admission gate 和 compiler-owned
  IR shape prototype gate，把 owning dyn、
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   M14 已完成唯一 borrowed dyn view path inference；M17/M18/M19/M20a/M20b 已完成 dyn ownership runtime facts、
   project-level boundary gate、IR/verifier preparation、owned dyn runtime admission gate 和 owned dyn IR shape
   prototype gate；下一步应进入 M20c drop / allocator identity prerequisite gate。owning dyn 用户值、trait-object
