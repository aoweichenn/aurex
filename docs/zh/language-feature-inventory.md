...
result name 推进为 `M21o Macro Expansion Boundary Release Closure`。真实宏展开、generated source text、
...
- 完整 macro / proc-macro / 用户自定义 derive；当前只有编译器内建 `#[derive(Copy, Eq, Hash)]`、
  M21a macro design gate facts、M21b 通用 item attribute token-tree AST surface 和 M21c no-op early expansion
  query facts。
- comptime。
- async / coroutine。
- effect system。
