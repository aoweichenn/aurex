...
状态：当前实现清单、M10d 完成面、M11a design/query gate、M11b query facts prototype、M11c frontend/sema、M11d runtime projection、M11e release closure 与后续非目标

本文记录当前仓库真实支持的 Aurex 语言表面、已具备的高级能力、未完成能力、M10d hardening/release closure 的完成范围、M11a advanced dyn design/query gate、M11b principal-set composition query facts prototype、M11c principal-set composition frontend/sema、M11d principal-set composition runtime projection 和 M11e principal-set composition release closure。本文以 `include/aurex/frontend/syntax/core/token.hpp`、`include/aurex/frontend/syntax/ast/*`、`src/frontend/parse/*`、`src/frontend/sema/*`、`src/midend/ir/*`、`src/backend/llvm/*`、`tests/samples/**` 为准；M11a gate 另以 `include/aurex/infrastructure/query/dyn_advanced_design_gate.hpp` 和 `src/infrastructure/query/dyn_advanced_design_gate.cpp` 为准；M11b/M11c/M11d/M11e principal-set facts 另以 `include/aurex/infrastructure/query/principal_set_composition_facts.hpp`、`include/aurex/infrastructure/query/dyn_abi_facts.hpp`、`src/infrastructure/query/dyn_abi_facts.cpp`、`src/infrastructure/query/lower_function_ir_query.cpp`、`src/application/tooling/ide/ide.cpp`、`src/midend/ir/core/ir_dyn_abi_facts.cpp`、`tests/gtest/frontend/sema/dyn_trait_composition_tests.cpp`、`tests/gtest/midend/ir/lower_ast_dyn_trait_tests.cpp` 和 `tests/gtest/integration/native_execution_tests.cpp` 为准。
...
- owning dyn / trait-object Drop dispatch / direct principal-qualified composition method dispatch；M11a 完成
  ...
  composition runtime facts/query/tooling/verifier hardening。
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   IR/backend runtime projection，M11e 已完成 hardening/release closure。下一步应做 M12 advanced dyn design；
