...
Aurex 现在已经具备一个小型系统语言核心：模块、import、可见性、函数、C FFI、struct、enum payload、泛型、method、pointer、array、slice、tuple、`str`、block expression、`if` expression、`match` expression、`defer`、C-style `for`、`Result` / `Option` 形状约定和 `?` 都已经存在。
...
- tuple 和 M2 pattern ergonomics 已落地：类型 `(A, B)` / `(A,)`，字面量 `(a, b)` / `(a,)`，字段访问 `value.0`，局部 `let (a, _) = value;` 解构，match tuple/struct pattern，nested enum payload destructuring，以及 `if value is pattern` / `while value is pattern` / if 表达式 pattern condition；空 tuple 暂不属于 M2。
- 顶层 item、struct field、impl method 和 import 默认 private，跨模块 API 必须显式 `pub`。
- `pub fn` 必须显式返回类型；private helper 仍可做返回类型推导。
- raw pointer、unchecked string、bit cast 等已经有最小 `unsafe` 边界；borrow checker、lifetime 和资源模型仍暂缓。
- 泛型没有 `where` / trait / capability 约束。
- `str` 已有内建雏形和 no-std checked UTF-8 边界；`strvalid(bytes)` / `strfromutf8(bytes)` 接受 `[]const u8` 或 `[]mut u8`，`strfromutf8` 返回 `(bool, str)`，失败时返回空 `str`；`strraw(data, len)` 继续是 unsafe-only unchecked 后门。
...
- tuple 是匿名 product type，字段按声明顺序编号。
- tuple literal 写作 `(1, true)` 或 `(value,)`。
- 字段访问使用零基数字字段：`pair.0`、`pair.1`。
- 局部声明可做 tuple destructuring：`let (left, _) = pair;`，`var (a, b) = pair;` 会把绑定设为 mutable。
- `match pair { (left, right) => ... }` 已支持 tuple pattern；结构化 match 需要有 irrefutable arm。
...
- `GenericParam` 只能是类型参数名；M2 不在基础泛型里混入 bound、`where`、trait、associated type 或 const generic。
...
普通 enum 默认是 ADT：base type 和 case discriminant 可省略，tag 从 0 自动分配；payload 支持单字段和多字段。显式 base/discriminant 仍保留给 C-like/repr enum。`enum Option[T]` 这类 generic enum 语法可被 parser 识别，但 M2 语义层会明确诊断不支持。
...
- generic impl block 和 generic method 在 M2 不支持；method 不能声明自己的 generic parameter。
...
其中 `ptrcast`、`bitcast`、`ptrat` 和 `strraw` 是 unsafe-only；safe context 下直接使用会诊断。`cast`、`sizeof`、`alignof`、`ptraddr`、`strptr`、`strblen`、`strvalid` 和 `strfromutf8` 仍是 safe 内建。`strvalid(bytes)` 返回 `bool`；`strfromutf8(bytes)` 返回 `(bool, str)`，`.0` 是是否成功，`.1` 成功时借用原 byte slice，失败时是空 `str`，因此 checked 调用失败时不会把无效输入包装成 UTF-8 文本。
...
4. 泛型 struct / function，采用单态化实例化。
5. 泛型函数参数推断；generic struct literal 当前需要显式 `[]` 类型实参。
...
2. ADT-first enum 语法。已补
   ...
   普通 enum 已经让 ADT 成为默认形态：base type 和 case discriminant 可省略，tag 自动分配；显式 `enum Status: u8 { ok = 0, err = 1 }` 仍保留给 C-like/repr enum。M2 当前仍不支持 generic enum，所以下面的 `Option[T]` / `Result[T, E]` 只是未来泛型 enum 方向，不是当前可通过的样例：
   ...
   当前可通过的 M2 写法应使用非泛型 ADT，例如 `OptionI32` / `ResultI32I32`。
...
6. `str` 安全边界：已补 M2 no-std checked 构造

   保留 `str` 语言内建，unchecked 构造 `strraw(data, len)` 放入 unsafe；checked UTF-8 构造先作为语言内建冻结，不依赖旧 std：`strvalid(bytes) -> bool`、`strfromutf8(bytes) -> (bool, str)`。两者接受 `[]const u8` / `[]mut u8`；失败时 `strfromutf8` 返回 `(false, "")`，不会把无效 byte slice 包成 `str`。checked slicing、owned `String`、`Bytes`、`CStr` 等继续后置到库层/资源语义之后。
...
这些都重要，但不应抢在 string、function type 这些基础语法之前。default private、ADT-first enum、array literal / repeat literal、slice type/expression、tuple/destructuring、function pointer / function type 和最小 unsafe 已完成，不再作为未完成前置项。
...
2. 继续推进 checked string slicing、`where` / 非资源类 capability；资源语义暂缓。
3. 继续完善 structural exhaustiveness、guard 和 unreachable 诊断。
