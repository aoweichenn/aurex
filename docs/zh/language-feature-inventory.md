...
- 完整 macro / proc-macro / 用户自定义 derive；当前只有编译器内建 `#[derive(Copy, Eq, Hash)]`、M21a macro
  ...
  release hardening matrix、debug dump stability contract 和 rollback diagnostic design gate facts，以及 M23a-M23c
  builtin derive parser consumption admission、checkpoint rollback、pre-consumption verification closure facts。
  它们都保持 parser consumption、generated source text、用户宏执行、标准库和 runtime 关闭。
- comptime。
- async / coroutine。
- effect system。
