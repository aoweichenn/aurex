...
item attribute token tree，但非 `derive` attribute 会在 sema 阶段报错，真实宏展开、typed expression macro、
external procedural macro、文本替换宏、宏生成用户代码和标准库仍未实现。
...
- 完整 macro / proc-macro / 用户自定义 derive；当前只有编译器内建 `#[derive(Copy, Eq, Hash)]`、
  M21a macro design gate facts 和 M21b 通用 item attribute token-tree AST surface。
- comptime。
- async / coroutine。
- effect system。
