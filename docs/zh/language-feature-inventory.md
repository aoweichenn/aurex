...
21. M13c borrowed composition-to-supertrait runtime projection：`dynproject[SourcePrincipal, TargetSupertrait](view)` 在 sema 层显式选择 composition 中的 source principal，再检查 target 是该 source 的 supertrait，并记录 `CompositionProjectionFact{kind=composition_to_supertrait}`；IR/backend 会 lowering 为 `trait_object_composition_project` + `trait_object_upcast`，不新增 runtime metadata policy。
