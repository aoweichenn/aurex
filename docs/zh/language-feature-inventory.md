...
- 泛型已有 `where` / trait predicate 约束，支持内建能力 `Sized`、`Eq`、`Ord`、`Hash`、compiler-owned `Copy` 和用户定义 nominal static trait；associated type declaration、impl assignment、projection、equality predicate 和 trait default method body 已进入 M5 release baseline；const generic、trait object 和用户 `Drop` bound 仍暂缓。M6 已保留 destructor body identity 和 drop-glue key，但还没有开放用户 `Drop` 语法。
...
- `Drop` 用户 bound、用户 destructor syntax 和 custom destructor body lowering 仍暂缓到 M7 及后续包；`Copy` 已作为 compiler-owned capability 进入 M6-WP2，destructor body identity 与 drop-glue key 已进入 M6-WP5/WP6 基线。
...
2. 继续保持 `where` capability / trait predicate 的文档、测试和诊断同步；M7 下一步进入 CFG-sensitive origin / loan / lifetime checker，用户 destructor syntax、custom destructor lowering、dynamic trait object 和完整 borrow/lifetime 体系继续作为独立后续包。
