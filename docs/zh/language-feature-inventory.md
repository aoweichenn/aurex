...
Aurex 现在已经具备一个小型系统语言核心：模块、import、可见性、函数、C FFI、struct、enum payload、泛型、method、raw pointer、safe reference、array、slice、tuple、`str`、block expression、`if` expression、`match` expression、`defer`、C-style `for`、结构化 Result/Option shape 识别、`?`、nominal static trait、显式 trait impl、trait predicate、静态 trait method dispatch、第一版 associated type model、trait default method body、compiler-owned `Copy` capability、内部 resource summary 和 whole-local move analysis 都已经存在。
...
- 泛型已有 `where` / trait predicate 约束，支持内建能力 `Sized`、`Eq`、`Ord`、`Hash`、compiler-owned `Copy` 和用户定义 nominal static trait；associated type declaration、impl assignment、projection、equality predicate 和 trait default method body 已进入 M5 release baseline；const generic、trait object 和用户 `Drop` bound 仍暂缓。
...
compiler-owned `Copy`、内部 resource summary 和 whole-local move analysis。
...
- `Drop` 用户 bound 和 destructor protocol 仍暂缓到 M6-WP5；`Copy` 已作为 compiler-owned capability 进入 M6-WP2。
...
1. 最小 `unsafe` block / `unsafe fn`。已补
   ...
   已冻结规则：unsafe-only builtin 和 raw pointer dereference 在 safe context 下诊断；调用 `unsafe fn` 或 unsafe 函数指针必须处于 unsafe context。当前不包含 borrow checker、lifetime、unsafe trait、unsafe impl、unsafe extern block 或 cleanup/destructor 资源模型。
