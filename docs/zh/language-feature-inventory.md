...
- 完整 macro / proc-macro / 用户自定义 derive；当前只有编译器内建 `#[derive(Copy, Eq, Hash)]`、M21a macro
  ...
  contract / boundary closure facts，以及 M22a-M22c builtin derive admission、semantic plan 和 parser release gate
  facts。它们都保持 parser consumption、generated source text、用户宏执行、标准库和 runtime 关闭。
- comptime。
- async / coroutine。
- effect system。
