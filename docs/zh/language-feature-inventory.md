...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   M14 已完成唯一 borrowed dyn view path inference；M17/M18 已完成 dyn ownership runtime facts 和 project-level
   boundary gate；下一步应进入 M19 IR/verifier preparation。owning dyn 用户值、trait-object Drop dispatch、allocator、
