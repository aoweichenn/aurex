...
- 泛型已有最小 `where` / capability 约束，支持内建非资源能力 `Sized`、`Eq`、`Ord`、`Hash`；用户 trait、associated type、const generic、trait object 和资源能力 `Copy` / `Drop` 仍暂缓。
...
- 用户自定义 capability / trait predicate；当前只内建 `Sized`、`Eq`、`Ord`、`Hash`。
...
2. ADT-first enum 语法。已补
   ...
   当前仍不包含 GADT、associated type、trait bound 或 const generic。
...
- octal / hex float。
- doc comment。
- 用户自定义 trait / protocol 和更完整的 capability 系统。
- resource semantics。
- borrow checker。
- package。
- 库层重建。

这些都重要，但不应抢在 M2 基础语法稳定之前。default private、ADT-first enum、generic enum、generic type alias、owner generic impl、最小 `where` capability、array literal / repeat literal、slice type/expression、checked `str` slicing、tuple/destructuring、function pointer / function type、最小 unsafe 和最小 safe reference 已完成，不再作为未完成前置项。
