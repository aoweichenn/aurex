...
状态：当前实现清单、M10d 完成面、M11a design/query gate 与后续非目标

本文记录当前仓库真实支持的 Aurex 语言表面、已具备的高级能力、未完成能力、M10d hardening/release closure 的完成范围和 M11a advanced dyn design/query gate。本文以 `include/aurex/frontend/syntax/core/token.hpp`、`include/aurex/frontend/syntax/core/ast.hpp`、`src/parse/*`、`src/sema/*`、`tests/samples/**` 为准；M11a gate 另以 `include/aurex/infrastructure/query/dyn_advanced_design_gate.hpp` 和 `src/infrastructure/query/dyn_advanced_design_gate.cpp` 为准。
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   principal-set borrowed dyn composition 并固定 `principal_set_metadata_v1` design/query gate。下一步应做 M11b query prototype；
