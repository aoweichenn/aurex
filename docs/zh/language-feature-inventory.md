...
这些都重要，但不应抢在 string、function type 这些基础语法之前。default private、ADT-first enum、array literal / repeat literal、slice type/expression、checked `str` slicing、tuple/destructuring、function pointer / function type、最小 unsafe 和最小 safe reference 已完成，不再作为未完成前置项。
...
2. 继续推进 `where` / 非资源类 capability；资源语义和完整 borrow/lifetime 体系暂缓。
