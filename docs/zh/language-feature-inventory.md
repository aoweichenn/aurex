...
- tuple 基础已落地：类型 `(A, B)` / `(A,)`，字面量 `(a, b)` / `(a,)`，字段访问 `value.0`，局部 `let (a, _) = value;` 解构；空 tuple 和 match tuple pattern 暂不属于 M2。
...
- `str` 只有内建雏形；普通数组/slice 地基已落地，但 UTF-8 boundary 和 safe/unsafe API 分层仍未冻结。
...
- match tuple pattern 暂缓；当前 tuple pattern 只用于局部 binding。
...
其中 `ptrcast`、`bitcast`、`ptrat` 和 `strraw` 是 unsafe-only；safe context 下直接使用会诊断。`cast`、`sizeof`、`alignof`、`ptraddr`、`strptr` 和 `strblen` 仍是 safe 内建。
...
- struct pattern 和 match tuple pattern。
...
3. 已补齐：tuple / destructuring declaration
   ...
   当前边界：空 tuple 不支持；tuple pattern 只用于局部 `let` / `var`，不用于 `match`；tuple struct 和 anonymous record 暂缓。

4. pattern 扩展到 `let` / `if let` / `let ... else` / struct pattern
...
2. 冻结 `str` 的 checked UTF-8 API，并明确和普通 slice 的关系。
3. 继续推进 pattern 扩展、`where` / 非资源类 capability；资源语义暂缓。
