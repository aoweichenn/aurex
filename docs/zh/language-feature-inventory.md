...
21. M13b borrowed composition-to-supertrait check-only projection：`dynproject[SourcePrincipal, TargetSupertrait](view)` 在 sema 层显式选择 composition 中的 source principal，再检查 target 是该 source 的 supertrait，并记录 `CompositionProjectionFact{kind=composition_to_supertrait}`；当前不做 IR/backend runtime lowering。
...
- owning dyn / trait-object Drop dispatch / composition-to-supertrait 隐式多步 direct dispatch；M11a 完成
  ...
  `dynproject[SourcePrincipal, TargetSupertrait](view)` frontend/query/sema check-only；IR/backend runtime lowering
  仍是后续 M13c。
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   `dynproject[...]` check-only 显式 supertrait projection。下一步应做 M13c runtime lowering；
