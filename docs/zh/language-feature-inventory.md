...
- const initializer 与运行时表达式能力不一致。
- 没有 compound assignment。
...
2. 补齐 const initializer 纯标量运算

   `/`、`%`、`<<`、`>>`、`&&`、`||` 应进入 const evaluator。系统代码需要 page size、mask、flag、alignment 这类常量表达式。第一阶段不需要函数调用或 comptime。

3. 增加 compound assignment

   `i = i + 1`、`flags = flags | mask` 在系统代码里过于啰嗦。建议支持：
...
2. trailing separator 统一

   parameter、argument、type argument、struct literal、enum case、match arm 都应有一致策略。建议列表允许 trailing comma，enum 最后一个 case 不再强制 comma。
...
1. 最小 `unsafe`
   ...
   raw pointer dereference、`ptr_cast`、`bit_cast`、`ptr_from_addr`、`str_from_bytes_unchecked` 应进入 unsafe-only 清单。先做语法和诊断框架，不必一次完成 borrow checker。
...
3. 补 const evaluator 的 `/`、`%`、shift、逻辑短路。
4. 增加 compound assignment token、AST、parser、sema、IR lowering。
5. 统一 trailing separator 策略。
6. 设计 enum ADT 的最小语法。
7. 给 `unsafe` 设计最小 AST/语义框架。
8. 再进入 `where` / capability / Drop / borrow。
