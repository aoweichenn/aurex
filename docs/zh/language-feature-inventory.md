...
source-map projection unavailable states。真实宏展开、generated source text、parser-consumable generated token buffer、
真实 hygiene resolution、declared generated names lookup、真实 expansion source map、debug trace CLI、
`--emit-expanded`、typed expression macro、external procedural macro、文本替换宏、宏生成用户代码和标准库仍未实现。
