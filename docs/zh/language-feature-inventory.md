...
状态：当前实现清单、M10d 完成面、M11 principal-set composition release、M12 direct composition dispatch release closure、M13a advanced dyn remaining policy design gate、M13b explicit dynproject frontend/query/sema、M13c runtime lowering、M13d release closure、M14 borrowed dyn view path inference release、M15 advanced dyn / const generic design baseline 与后续非目标

本文记录当前仓库真实支持的 Aurex 语言表面、已具备的高级能力、未完成能力、M10d hardening/release closure 的完成范围、M11 principal-set composition release closure、M12 direct composition dispatch release closure、M13a advanced dyn remaining policy design gate、M13b explicit dynproject frontend/query/sema 子集、M13c runtime lowering 子集、M13d release closure、M14 borrowed dyn view path release 和 M15 advanced dyn / const generic design baseline。本文以 `include/aurex/frontend/syntax/core/token.hpp`、`include/aurex/frontend/syntax/ast/*`、`src/frontend/parse/*`、`src/frontend/sema/*`、`src/midend/ir/*`、`src/backend/llvm/*`、`tests/samples/**` 为准；M11a/M13a/M15 dyn gate 另以 `include/aurex/infrastructure/query/dyn_advanced_design_gate.hpp`、`src/infrastructure/query/dyn_advanced_design_gate.cpp` 和 `tests/gtest/infrastructure/query/dyn_advanced_design_gate_tests.cpp` 为准；M15 const generic gate 另以 `include/aurex/infrastructure/query/const_generic_design_gate.hpp`、`src/infrastructure/query/const_generic_design_gate.cpp` 和 `tests/gtest/infrastructure/query/const_generic_design_gate_tests.cpp` 为准；M11/M12/M13/M14 principal-set facts 另以 `include/aurex/infrastructure/query/principal_set_composition_facts.hpp`、`include/aurex/infrastructure/query/dyn_abi_facts.hpp`、`src/infrastructure/query/dyn_abi_facts.cpp`、`src/infrastructure/query/lower_function_ir_query.cpp`、`src/application/tooling/ide/ide.cpp`、`src/midend/ir/core/ir_dyn_abi_facts.cpp`、`src/frontend/sema/facade/sema_call.cpp`、`tests/gtest/frontend/sema/dyn_trait_composition_tests.cpp`、`tests/gtest/frontend/sema/dyn_trait_composition_supertrait_tests.cpp`、`tests/gtest/midend/ir/lower_ast_dyn_trait_tests.cpp`、`tests/gtest/infrastructure/query/principal_set_composition_facts_tests.cpp`、`tests/gtest/infrastructure/query/trait_object_upcast_key_tests.cpp` 和 `tests/gtest/integration/native_execution_tests.cpp` 为准。
...
Aurex 现在已经具备一个小型系统语言核心：模块、import、可见性、函数、C FFI、struct、enum payload、泛型、method、raw pointer、safe reference、array、slice、tuple、`str`、block expression、`if` expression、`match` expression、`defer`、C-style `for`、结构化 Result/Option shape 识别、`?`、nominal static trait、显式 trait impl、trait predicate、静态 trait method dispatch、第一版 associated type model、trait default method body、compiler-owned `Copy` capability、内部 resource summary、whole-local move analysis、cleanup lowering、drop flags、稳定 drop-glue identity、target-independent drop-glue planner 和第一版 IDE/LSP resource projection 都已经存在。
...
- never type。
- 用户可写 const generic；M15 已固定 design gate，但当前 AST/parser 仍只有 type parameter 和 origin parameter。
...
- 用户可写 const generic；M15 已固定 `m15_const_generic_design_gate_baseline()`，下一步才进入 parser/query/sema check-only。
- never type。
...
2. ADT-first enum 语法。已补
  ...
  当前仍不包含 GADT、用户可写 const generic、qualified where predicate 或 generic trait predicate argument。
...
- owning dyn / trait-object Drop dispatch / 歧义 composition-to-supertrait 自动选择；M11a 完成
  ...
  direct supertrait method dispatch 和 `BorrowedDynViewPathFact` release closure。
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   M14 已完成唯一 borrowed dyn view path inference；
