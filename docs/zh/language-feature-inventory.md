...
日期：2026-06-08
阶段：M11c Principal-Set Composition Frontend / Sema Check-Only
状态：当前实现清单、M10d 完成面、M11a design/query gate、M11b query facts prototype、M11c check-only frontend/sema 与后续非目标

本文记录当前仓库真实支持的 Aurex 语言表面、已具备的高级能力、未完成能力、M10d hardening/release closure 的完成范围、M11a advanced dyn design/query gate、M11b principal-set composition query facts prototype 和 M11c principal-set composition frontend/sema check-only。本文以 `include/aurex/frontend/syntax/core/token.hpp`、`include/aurex/frontend/syntax/ast/*`、`src/frontend/parse/*`、`src/frontend/sema/*`、`tests/samples/**` 为准；M11a gate 另以 `include/aurex/infrastructure/query/dyn_advanced_design_gate.hpp` 和 `src/infrastructure/query/dyn_advanced_design_gate.cpp` 为准；M11b/M11c principal-set facts 另以 `include/aurex/infrastructure/query/principal_set_composition_facts.hpp`、`src/infrastructure/query/principal_set_composition_facts.cpp` 和 `tests/gtest/frontend/sema/dyn_trait_composition_tests.cpp` 为准。
...
- owning dyn / trait-object Drop dispatch / principal-set composition runtime；M11a 完成 principal-set
  composition design/query gate，M11b 完成 `PrincipalSetCompositionFacts` query prototype，M11c 完成
  `dyn (A + B)` frontend/sema check-only。
...
2. 继续保持 `where` capability / trait predicate 和 borrowed dyn trait 的文档、测试和诊断同步；M9 dyn ABI / tooling release closure 已完成，M10a 已选择 supertrait upcasting 并固定设计基线，M10b 已完成
   ...
   `PrincipalSetCompositionFacts` query prototype，M11c 已完成 `dyn (A + B)` frontend/sema check-only。下一步应做 M11d IR/backend runtime；
