...
日期：2026-06-05
阶段：M7d-B struct field place-state 与字段级 cleanup/drop flag 子集实现收口，建立在 M7c lifetime/dropck facts、M7b borrow contract、M7a
...
summary/contract 传播，并与 whole-local move analysis、struct 字段 place-state、cleanup 和 dropck facts 协同；
它还不是完整 Rust borrow checker 复刻，也不提供 raw pointer alias safe proof、tuple/index partial move、完整
lifetime generics 或用户级 RAII `Drop` surface。
