...
日期：2026-06-05
阶段：M7d-C RAII user surface 与 release closure 实现收口，建立在 M7d-B struct field place-state、M7c lifetime/dropck facts、M7b borrow contract、M7a
...
- 使用 M7d-C 窄 RAII destructor surface：`impl Drop for T { fn drop(self: deinit T) -> void { ... } }`。当前它进入 checked destructor facts、resource semantics、drop-glue/dropck、query fingerprint 和 IDE hover；backend custom destructor call lowering 仍未落地。
...
- 没有标准库级拥有型 `String`、容器库、用户可写 `Drop` bound、generic Drop impl、trait-object Drop dispatch 或 backend/runtime custom destructor call lowering。
...
- 没有完整 Rust-style apostrophe lifetime surface、full Polonius Datalog、raw pointer alias safe proof、tuple partial move、indexed move-out 或 `replace` / `take` / `swap` 内建。
...
- const generic
- trait object
- 用户 `Drop` bound、generic Drop impl、trait-object Drop dispatch 和 backend custom destructor call lowering
...
- backend custom destructor call lowering、trait-object Drop dispatch 和完整 RAII runtime integration。
- tuple partial move、indexed move-out、consuming pattern payload 和 non-`Copy` `?` payload transfer。
...
当前 M7d-B 已支持的 place-level resource 子集：
...
- tuple 元素 `.0` / `.1` 字段访问当前 parser 仍拒绝，因此 tuple partial move 还没有 source surface。
...
- backend 仍不会把 custom destructor step 降低成实际函数调用；当前 `drop` / `drop_if` 在 LLVM backend 仍是 cleanup
  marker。因此需要运行时副作用的资源库仍必须显式调用释放 API 或配合 `defer`，不能依赖 `drop` body 已经执行。
- 用户可写 `Drop` bound、generic Drop impl、trait-object Drop dispatch 和 async/unwind-aware drop 仍不是当前能力。
...
它还不是完整 Rust borrow checker 复刻，也不提供 raw pointer alias safe proof、tuple/index partial move、完整
lifetime generics 或 runtime-complete RAII。
...
- custom destructor body 会按普通函数体进入语义分析，但 backend 尚未把 custom destructor step 降低为实际调用。
- 没有 drop dispatch、trait object destructor、async drop、panic/unwind cleanup model 或 `may_dangle` 类 unsafe escape hatch。
- 析构执行顺序和实际释放外部资源的 ABI 仍属于后续实现包；当前资源库仍应显式释放。
...
路径。当前 `impl Drop` 已参与语义事实和 dropck，但 custom destructor call lowering 尚未落地；拥有型资源库若需要
真实运行时释放，仍需要手动 API 配合 `defer`。
...
value++              // 不支持自增
let x = tuple.0;      // 匿名 tuple 字段访问仍拒绝
