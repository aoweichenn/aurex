...
日期：2026-06-07
阶段：M7d-K array repeat resource safety closure，建立在 M7d-J cleanup marker query / tooling consumption
closure、M7d-I move rejection facts、M7d-H
...
- closure/lambda capture、async/generator borrow、HRTB/full variance、`dyn Trait` object lifetime bounds、dynamic
  Drop dispatch、marker-only cleanup 的 runtime ABI call 和 unsafe/raw pointer alias proof 仍是后续阶段，不属于当前 M7d-K。
