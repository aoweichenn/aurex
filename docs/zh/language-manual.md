...
日期：2026-06-05
阶段：M7d-E aggregate rollback codegen closure，建立在 M7d-D RAII runtime lowering、M7d-C RAII user
surface、M7d-B struct field place-state、M7c lifetime/dropck facts、M7b borrow contract、M7a
...
- 没有标准库级拥有型 `String`、容器库、用户可写 `Drop` bound、generic Drop impl、trait-object Drop dispatch、
  generic/opaque Drop ABI 或 async/unwind-aware drop。
...
- const generic
- trait object
- 用户 `Drop` bound、generic Drop impl、trait-object Drop dispatch、generic/opaque Drop ABI 和 async/unwind-aware drop
...
- trait-object Drop dispatch、generic/opaque Drop ABI、async/unwind-aware drop 和完整标准库级 RAII runtime integration。
...
lifetime generics、generic Drop dispatch 或 unwind-aware RAII。
...
- 静态可解析 custom destructor 已会在 cleanup lowering 中发出 direct call；generic/opaque Drop ABI 仍未正式化。
