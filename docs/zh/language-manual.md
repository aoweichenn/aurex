...
日期：2026-06-07
阶段：M8d/M8e borrowed dyn trait runtime dispatch closure，建立在 M8a query foundation、M8b syntax/sema、
M8c borrowed dyn coercion / checked vtable facts、M7d-K array repeat resource safety、M7d-J cleanup marker
...
- 没有 owning dyn、`Box<dyn Trait>`、trait-object Drop dispatch、supertrait upcasting、多 trait object
...
- consuming receiver、supertrait upcasting、多 principal trait object、多 trait object composition 或 auto trait composition。
...
- closure/lambda capture、async/generator borrow、HRTB/full variance、dyn object lifetime bounds、owning dyn、
  supertrait upcasting、dynamic Drop dispatch、marker-only cleanup 的 runtime ABI call 和 unsafe/raw pointer
