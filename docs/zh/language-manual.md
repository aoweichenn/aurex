...
query / tooling consumption closure、M7d-I move rejection facts、M7d-H index/slice place-state conservative
closure、M7d-G cleanup marker ABI policy、M7d-F tuple element place-state、M7d-E aggregate rollback codegen、
M7d-D RAII runtime lowering、M7d-C RAII user surface、M7d-B struct field place-state、M7c lifetime/dropck
facts、M7b borrow contract、M7a CFG-sensitive loan facts、M6 resource semantics、M5 default trait methods、
M4 trait/protocol、M3 module/generic 和 M2 language core 基线之上。
...
- 使用内建 item derive 属性：`#[derive(Copy, Eq, Hash)]` 可写在 `struct` / `enum` 上，生成 checked capability facts；
  它不是完整宏系统，也不生成标准库方法。
- 使用 M27/M27b/M27c/M27d admission-only 宏声明与调用表面：`macro Name { ... }`、`macro derive Name { ... }`、
  ...
  admission facts；matcher-to-call binding 和 user derive target schema 会形成 output contract / declared-name policy /
  diagnostic projection admission facts，但不会展开、执行或交给 parser。
...
- 使用 borrowed dyn trait view：`&dyn Trait`、`&mut dyn Trait` 和 `dyn Trait<Assoc = Type>`，在 checked
  vtable witness 上执行 runtime dynamic dispatch；impl override 和 trait default method slot 均可进入 vtable。
- 使用 borrowed dyn supertrait upcast runtime：`&dyn Child -> &dyn Parent`、
  `&mut dyn Child -> &mut dyn Parent` 和 `&mut dyn Child -> &dyn Parent` 可以在 contextual coercion 中记录 checked
  upcast facts，并 lowering 为 `trait_object_upcast` IR 和 LLVM parent vtable projection；`dyn Child` receiver 上
  inherited parent method call 可以 runtime dispatch 到 parent vtable。
...
- 使用 M7d-K array repeat resource safety：`[expr; 0]` 类型检查 repeat value 但运行期不求值/consume；
  `[expr; 1]` 允许非 `Copy` 资源转移一次；`[expr; N]` 且 `N > 1` 要求元素类型满足 `Copy`。
...
- 查询 M11b principal-set composition facts：`PrincipalSetCompositionFacts`、
  ...
  原型。
- 使用 M11c borrowed principal-set composition syntax：`&dyn (Draw + Debug)` 和
...
- 查询 M19 dyn ownership runtime IR / verifier facts：`DynOwnershipRuntimeIrVerifierFact`、
  ...
  `dyn_ownership_runtime_ir_verifier_facts_fingerprint()`、
  `summarize_dyn_ownership_runtime_ir_verifier_facts()` 和
  `dump_dyn_ownership_runtime_ir_verifier_facts()` 已固定函数级 IR/verifier 边界。IR dump/fingerprint/verifier
  会消费 `TraitObjectVTableLayout::destructor_slot_blocked`；`CleanupAbiPolicy::dynamic_erased_drop_blocked` 是
  blocked negative sentinel，用于证明 dynamic Drop runtime 没有在 M19 提前落地。
- 查询 M20a owned dyn runtime admission gate：`OwnedDynRuntimeAdmissionGate`、
  `OwnedDynRuntimeAdmissionFact`、`OwnedDynRuntimeAdmissionCapability`、`OwnedDynRuntimeAdmissionStage`、
  `OwnedDynRuntimeAdmissionPolicy`、`m20_owned_dyn_runtime_admission_gate_baseline()`、
  `owned_dyn_runtime_admission_gate_fingerprint()`、`summarize_owned_dyn_runtime_admission_gate()` 和
  `dump_owned_dyn_runtime_admission_gate()` 已固定 owned dyn runtime 的分阶段准入。它把 M17/M18/M19 facts
...
- 查询 M20c drop / allocator identity prerequisite gate：`OwnedDynDropAllocatorIdentityGate`、
  `OwnedDynDropAllocatorIdentityFact`、`OwnedDynDropAllocatorIdentitySummary`、
  `m20c_owned_dyn_drop_allocator_identity_gate_baseline()`、
  `owned_dyn_drop_allocator_identity_gate_fingerprint()` 和
  `ir::owned_dyn_drop_allocator_identity_gate()` 已固定 compiler-owned drop / allocator identity prerequisite。
...
- 没有完整 shared/mutable/consuming closure capture、borrowed closure environment escape、async/generator、语言级线程/atomic/concurrency memory model；M20 只支持 Copy-by-value 捕获闭包核心子集。可以通过 C FFI 调用外部并发 API，但 safe borrow checker 只为当前语言的本地控制流和函数 summary 建模。
...
- 泛型使用 `<...>`，`[]` 保留给数组、slice、index、attribute 和 origin 等非泛型用途。
...
DynTraitType      = "dyn" ModulePath DynTraitArgList?
DynTraitArgList   = "[" DynTraitArg ("," DynTraitArg)* ","? "]"
DynTraitArg       = Type | Identifier "=" Type
...
- 泛型实参使用 `<...>`。
- `Box<>` 会被拒绝。
- `Box[i32]` 不是 Aurex 泛型语法；`[]` 不再承担泛型职责。
- 类型路径也使用 `.`。
...
的外层局部或参数。捕获闭包的值是编译器生成的匿名环境 record，不是 `fn(...) -> T` 薄函数指针。
闭包调用会把环境地址作为隐藏参数传给编译器生成的内部 thunk：
...
fn make_adder(base: i32) {
    return [base](value: i32) -> i32 => value + base;
}
...
- 捕获外层非泛型依赖、非 borrowed-view 的 `Copy` 局部或参数，按捕获发生时的值复制进匿名环境 record。
- 支持表达式体和块体 closure。
- 支持局部存储、直接调用、嵌套 closure、match guard 内使用捕获值。
- 支持函数返回捕获闭包；返回类型可由函数 return inference 推断为内部匿名环境类型。
- 捕获环境是私有内部结构，会进入 checked/IR dump 和 LLVM lowering，但没有用户可写类型名。
...
- 捕获 generic parameter、associated projection 或包含这些类型的值会报
  `capturing a generic-dependent value in a closure is not supported yet`；M20 还没有 generic closure
  environment ABI，也不能证明实例化后不会携带 borrowed-view。
- 不支持 shared / mutable / consuming capture mode，不支持修改捕获变量后影响外层变量。
- 不支持 `Fn` / `FnMut` / `FnOnce` trait、标准库 adapter、heap/allocator closure box 或 runtime helper。
- 不支持 `extern c`、`unsafe`、variadic closure 字面量。
- 对包含 borrow/reference/slice/string view 的捕获环境，当前没有完整 closure lifetime escape 求解；M20 只把
  非泛型依赖、非 borrowed-view 的 Copy-by-value closure 路径作为稳定子集。

捕获闭包赋给薄函数指针会报 initializer type mismatch：
...
admission 和 user derive target schema admission。M27d 进一步新增 macro output contract admission，把 future output
token buffer ownership、generated part key、declared-name policy 和 diagnostic projection 固定为 facts。当前仍是
admission-only：parser、AST、dump、query 和 early expansion boundary 能识别、索引、生成稳定 facts 并验证边界，宏体
不会展开，也不会执行用户编译期代码。
...
- M27d 会为每个 matcher-to-call binding 和 user derive target schema 生成
  `AurexMacroOutputContractAdmissionGate`。
- M27d 会为每个 output contract 生成 `AurexMacroOutputDeclaredNamePolicyAdmissionGate` 和
...
- Summary/dump 会记录 `aurex_macro_output_contracts`、`aurex_macro_output_declared_name_policies` 和
...
- Output contract gate 会固定 compiler-owned future token buffer identity、generated `ModulePartKey`、source-map identity、
  hygiene mark 和 output contract identity，但 macro output parser consumption remains blocked in M27d。
- Declared-name policy gate 会预留 declared-name set，但 macro output declared names are hidden from lookup/export/sema
  in M27d。
- Diagnostic projection gate 会固定 diagnostic projection identity、source anchors 和 blocker message，但 macro output
  diagnostics are projected but parser emission remains blocked in M27d。
...
- Query 层提供 `m27d_macro_expansion_plan_baseline()`，固定 `aurex_macro_output_contract_admission`、
  `aurex_macro_output_declared_name_policy_admission` 和 `aurex_macro_output_diagnostic_projection_admission` 三类 facts。
...
- 不把宏输出交给 parser。
- 不 materialize M27d future output token buffer。
- 不让 M27d declared-name policy 对 lookup/export/sema 可见。
- 不从 M27d diagnostic projection 发出 parser consumption diagnostic。
...
也只记录目标结构，不会生成 impl、方法或其它 sema-visible item。M27d 的 output contract 已经把 future output
ownership、declared-name policy 和 diagnostic projection 固定为可查询 facts，但输出 token 仍未 materialize，parser
仍不会消费它，AST 和 sema 也看不到任何 generated item。
...
let base: i32 = 1;
let bad: Op = [base](value: i32) -> i32 => value + base; // error
```

### 8.2 泛型调用

泛型调用在 callee 后写显式泛型实参，使用 `<...>`。当前没有 turbofish 风格调用。

```text
GenericApply = ExprPath "<" GenericArg ("," GenericArg)* ","? ">" "(" ArgList? ")"
```
...
- `[a, b, c]` 按列举元素构造数组；若没有 expected array type，元素类型从第一个元素推导。
- `[]` 不能单独推导元素类型；空数组需要 expected array type。
- `[expr; count]` 是 repeat array literal；`count` 必须是整数字面量，不是任意 const expression。
- 若上下文给出 expected array type，repeat count 必须等于 expected array length。
- repeat value 必须能赋给数组元素类型；不匹配时报 `array repeat value type mismatch`。
- `[expr; 0]` 仍会类型检查 `expr`，但运行期不求值、不 consume，也不会产生运行期 borrow/move/place-state
  访问事实。
- `[expr; 1]` 求值一次，可以转移一个非 `Copy` 资源。
- `[expr; N]` 且 `N > 1` 要求元素类型满足 `Copy`；非 `Copy` 资源会被拒绝，诊断为
  `array repeat value must be Copy when repeated more than once`。
- 当前没有 `Clone`、array fill constructor 或标准库 helper，因此非 `Copy` 多元素 repeat 不会隐式构造多个资源。
...
| 写法 | 安全性 | 结果 | 说明 |
| --- | --- | --- | --- |
| `((x) as T)` | safe | `T` | 数值/布尔转换。支持整数、浮点、bool 之间转换。 |
...
| `slice.ptr` | safe | `*const T` 或 `*mut T` | 返回 slice 底层 data pointer；`[]T` 返回 `*const T`，`[]mut T` 返回 `*mut T`。 |
| `slice.len` | safe | `usize` | 返回 slice element count。 |
| `s.ptr` | safe | `*const u8` | 返回 `str` 底层 byte pointer。 |
| `s.len` | safe | `usize` | 返回 `str` byte length。 |
...
fn raw_roundtrip(text_value: str) -> str {
    return unsafe { strraw(text_value.ptr, text_value.len) };
}
...
- `slice.ptr` / `slice.len` 要求参数是 slice value，不接受 array、str、pointer 或普通 scalar；array 需要先通过 `values[:]` 形成 slice。
...
Trait 默认是 nominal static trait，不是 structural interface。显式 `dyn Trait` 是 borrowed erased view 的动态边界，
不是 owning object，也不会绕过 `impl Trait for Type` 的 nominal conformance。
...
TraitDecl     = Visibility? "trait" Identifier GenericParams? SupertraitClause? WhereClause? "{" TraitItem* "}"
SupertraitClause = ":" Type ("," Type)* ","?
...
Direct supertrait declaration 示例：
...
trait BufferedReader<T>: Reader<T> {
...
impl BufferedReader<i32> for File {
    fn remaining(self: &File) -> usize {
        return 0;
    }
}
```
...
- direct supertrait clause 写在 generic params 之后、`where` 之前，例如 `trait Child<T>: Parent<T> where T: Copy`。
- `trait Child: Parent` 是 declaration graph edge；`where Self: Parent` 仍只是 generic predicate，不作为 vtable
  supertrait metadata。
- direct/transitive supertrait edge 会进入 checked `TraitSupertraitEdgeFact`；duplicate direct parent、supertrait
  cycle 和 public trait 暴露 private supertrait 会被诊断。
- `impl Child for T` 不会自动合成 `impl Parent for T`；它要求当前 evidence system 能证明 `T: Parent`。
- 泛型 parent trait args 会按 edge 和 impl 实参替换，例如 `Child<i32>: Parent<i32>`。
- supertrait associated equality edge mapping 仍未完整实现；当前稳定能力是 parent trait args substitution。
...
### 12.1 Borrowed dyn trait view

`dyn Trait` 是类型上下文中的显式 erased view 标记。当前只支持 borrowed dyn view：

```text
DynTraitPointee = "dyn" ModulePath DynTraitArgList?
DynTraitRef     = "&" DynTraitPointee | "&mut" DynTraitPointee
```
...
fn main() -> i32 {
    ...
    return render(&file) + render(&socket);
}
```

规则：

- `&T` 可以 coercion 到 `&dyn Trait`，前提是 `T` 有可见的 nominal `impl Trait for T`。
- `&mut T` 可以 coercion 到 `&mut dyn Trait`，前提同上；dyn receiver access 会保留 shared/mutable 区分。
- bare `dyn Trait` 不能作为普通 storage type；当前只能作为 reference pointee 等显式 allowed context。
- `dyn Trait<Assoc = Type>` 支持 associated equality。所有进入 object-callable 签名的 associated type
  requirement 必须被显式约束；例如 `Self.Item` 会替换为 `Item = i32` 中的 `i32`。
- object-callable method 必须有 receiver，receiver 当前只支持 `&Self` 或 `&mut Self`。
- method-level generic、除 receiver 外的裸 `Self` 参数/返回、static associated function、unknown/duplicate
  associated equality 和缺失 associated equality 会被诊断。
- default method 可以进入 vtable slot；slot 会绑定到当前 impl override 或 trait default instance function。
...
fn demo(value: &dyn Child) -> i32 {
    return use_parent(value);
}
```

Inherited parent method 可以直接在 child dyn receiver 上调用；编译器会先产生 target parent view，再按 parent
vtable slot 动态派发：
...
- `&mut dyn Child -> &dyn Parent`

拒绝的 borrowed upcast：

- `&dyn Child -> &mut dyn Parent`，因为 shared borrow 不能升级成 mutable borrow。
- target trait 不是 source trait 的 direct/transitive supertrait。
- 泛型 parent args 不匹配，例如 source edge 证明的是 `Parent<i32>`，target 写成 `Parent<bool>`。

借用权规则：

- upcast 不创建 ownership、不复制对象、不创建 cleanup obligation。
- target reference 不能比 source reference 活得更久；origin 不被延长。
- upcast 不放宽 active loan，也不改变 M7 loan conflict matrix。
- `&mut dyn Child -> &dyn Parent` 是 shared target view，不保留 mutable target 权限。
...
- `&dyn Trait` / `&mut dyn Trait` lowering 为 `{data*, vtable*}` fat view。
- coercion 点把 concrete reference 的 data pointer 和 checked vtable witness 打包成 dyn view。
- dyn method call 从 fat view 提取 data pointer 和 vtable pointer，再按 checked slot ordinal 加载函数指针。
- supertrait upcast 复用 data pointer，从 source child vtable 的 supertrait edge table 读取 target parent vtable
  pointer，然后构造 target `{data*, parent_vtable*}` borrowed dyn view。
- principal-set composition 也复用 data pointer。Concrete reference coercion 到 `&dyn (A + B)` 会生成
  `{data*, principal_set_metadata*}` borrowed composition view；metadata global 使用 `principal_set_metadata_v1`，
...
- slot function ABI 的第一个 receiver 参数被擦成 `*const u8` 或 `*mut u8`；checked facts 和 IR verifier
  仍保留 concrete receiver type，用于验证 erased receiver ABI 与 concrete impl 签名兼容。
...
`dynproject<Child, Parent>(view)` 当前规则：

- 必须恰好写两个 type arguments 和一个 value argument。
- `Child` 和 `Parent` 必须能解析为 single trait object selector；`File` 这类 concrete type 会被拒绝。
- `view` 必须是 `&dyn (A + B)` 或 `&mut dyn (A + B)`，不能是普通 `&dyn Child`。
- `Child` 必须存在于 `view` 的 principal set 中。
- `Parent` 必须是 `Child` 的 supertrait；不能用该语法在 sibling principal 之间投影，例如
...
- full Polonius Datalog、dyn object lifetime bounds、owning dyn/upcasting 和 async/generator borrow。
...
13. M7d-K array repeat resource safety 把 `[expr; N]` 的资源语义收口：零长度 repeat 只类型检查 value、
    运行期不求值/consume；单元素 repeat 允许一次非 `Copy` transfer；多元素 repeat 必须是 `Copy`。
...
当前 M7d-B/M7d-F/M7d-H/M7d-I/M7d-J/M7d-K 已支持的 place-level resource 子集：
...
- array repeat 对资源值有明确安全边界：`[resource; 0]` 不在运行期求值/consume repeat value，`[resource; 1]`
  转移一次，`[resource; 2]` 这类多元素 repeat 必须要求元素类型是 `Copy`。

当前仍不属于 M7d-B/M7d-F/M7d-H/M7d-I/M7d-J/M7d-K 已支持范围：
...
- 非 `Copy` 多元素 array repeat 构造仍拒绝；未来必须先有 `Clone`、array fill constructor 或等价逐元素构造/
  rollback 语义，当前阶段不通过标准库实现。
...
M7d-K 增加的是 array repeat 的资源安全规则。它会拒绝非 `Copy` 资源的多元素 repeat，避免把同一个 owned value
隐式放入多个数组槽位；零长度 repeat 的 value 仍被类型检查，但运行期不求值、不 consume。它不是 `Clone` 语义，
也不是标准库填充 API。
...
- 非 `Copy` 多元素 array repeat 构造：当前已经安全拒绝；未来若要接受，需要 `Clone`、array fill constructor、
  逐元素初始化次数、失败路径、rollback cleanup 和 drop flag 精确事实。
- mutable/consuming closure capture、borrowed closure environment escape、`Fn` / `FnMut` / `FnOnce` 风格能力、
  async/generator borrow、HRTB/full variance、dyn object lifetime bounds、owning dyn、dynamic Drop dispatch、
  marker-only cleanup 的 runtime ABI call 和 unsafe/raw pointer alias proof 仍是后续阶段，不属于当前阶段。
...
- 这不是完整 Rust reborrow/lifetime subtyping；raw pointer reborrow、borrowed closure environment escape、
  generator 捕获和复杂跨函数 reborrow proof 仍不属于当前能力。
...
- `str` 内建：`text.len`、`text.ptr` 保留 UTF-8 byte offset；regex 自身按 Unicode scalar value 解析和匹配。
