...
- 没有 owning dyn、`Box<dyn Trait>`、trait-object Drop dispatch、bare `dyn A + B` parser syntax、
  ...
  boundary gate，M19 已把 boundary prerequisites 落成 IR/verifier facts 和 negative matrix；validation 仍会拒绝
  standard library、`Box` surface、allocator API、owning dyn 用户值、runtime lowering、backend runtime helper call、
  dynamic Drop dispatch 或 borrowed vtable destructor slot 被标成已实现。
