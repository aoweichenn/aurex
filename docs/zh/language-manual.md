...
日期：2026-06-08
阶段：M10b Supertrait Frontend / Query / Sema Implementation，建立在 M10a supertrait upcasting design baseline、
...
- 使用 M10b check-only borrowed dyn supertrait upcast：`&dyn Child -> &dyn Parent`、
  `&mut dyn Child -> &mut dyn Parent` 和 `&mut dyn Child -> &dyn Parent` 可以在 contextual coercion 中记录 checked
  upcast facts；runtime `trait_object_upcast` lowering 仍留给 M10c。
...
- 没有 owning dyn、`Box<dyn Trait>`、trait-object Drop dispatch、supertrait upcasting runtime lowering、多 trait object
  composition、generic associated type、associated const、specialization、const generic 或 `<T>` 风格泛型。
...
M10b 支持 borrowed dyn-to-dyn supertrait upcast 的 check-only 语义。Upcast 是 contextual coercion，不是普通子类型；
...
- M10b 会记录 `TraitObjectUpcastCoercionFact`、`TraitObjectUpcastCoercionKey` 和 `DynUpcastAbiDescriptor`。
- M10b 不生成 `trait_object_upcast` IR，不生成 `supertrait_vptr_metadata_v1` LLVM vtable global。
- inherited parent method direct dispatch on `dyn Child` 还不是 runtime-ready 能力；应通过 target parent view，或等待
  M10c/M10d 完成 runtime projection。
...
- consuming receiver、supertrait upcasting runtime lowering、多 principal trait object、多 trait object composition 或 auto trait composition。
...
- closure/lambda capture、async/generator borrow、HRTB/full variance、dyn object lifetime bounds、owning dyn、
  supertrait upcasting runtime lowering、dynamic Drop dispatch、marker-only cleanup 的 runtime ABI call 和 unsafe/raw pointer
