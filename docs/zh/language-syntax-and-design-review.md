...
Aurex 现在已经不是只支持函数和表达式的小语言。当前语言核心已经具备系统语言雏形：模块、可见性、C FFI、结构体、枚举 payload、泛型、method、标准形状 `Result` / `Option` 约定与 `?`、`defer`、C-style `for`、块表达式、`if` 表达式、`match` 表达式、指针、内建 `str` 和显式 cast 都已经落地。
