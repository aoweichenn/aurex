...
Aurex 现在已经不是只支持函数和表达式的小语言。当前语言核心已经具备系统语言雏形：模块、可见性、C FFI、结构体、枚举 payload、泛型、method、标准形状 `Result` / `Option` 约定与 `?`、`defer`、C-style `for`、基础 `for i in range(...)`、块表达式、`if` 表达式、`match` 表达式、raw pointer、safe reference、array/slice、tuple、内建 `str` 和显式 cast 都已经落地。

真正的问题不是“还缺多少语法”，而是核心模型还没有冻结。当前最危险的混乱点有五个：

1. raw pointer 和 safe reference 已经开始分层，但 method receiver、旧 address-of 兼容和 FFI 边界仍需要持续收紧。
...
- 泛型已经从“无约束单态化模板”升级到最小 capability 约束；完整 trait / protocol 后续再做。
- class / inheritance / dynamic dispatch 可以做迁移友好层，但不应抢在基础语法、`unsafe` 边界和 ADT 收口之前成为核心。

M2 的直接结论：不要复活 M1 的标准库和 selfhost 路线来掩盖语言问题。default visibility、ADT-first enum、generic enum、generic type alias、owner generic impl、array literal / repeat literal、slice type/expression、checked `str` slicing、tuple/destructuring、function type/function pointer、最小 unsafe、最小 safe reference、M2 pattern ergonomics、最小非资源类 `where` capability 和 `str` checked UTF-8 构造边界已完成。
...
M2 基础泛型包含类型参数、类型实参和最小非资源类 `where` capability：
...
- `impl[T] Box[T]` 这类 owner generic impl 支持；method-local generic parameter 当前不支持。
...
- `id[T](...)` 在 parser AST 中直接形成 `generic_apply` callee，再接 `call`；字段、调用、index、slice、struct literal 和 `?` 也都是显式 postfix AST 节点，不再保留 sema 二次 materialize 的 raw chain。
- `fn f[]`、`Box[]`、`id[](...)` 均非法。
...
- 无 guard 和字面量 `if true` arm 计入 exhaustiveness；字面量 `if false` 和动态 guard 不计入。
- const initializer 中不能使用。
...
- 语言当前支持泛型 struct / function / enum / type alias、函数参数推断、显式泛型调用和 owner generic impl。
- `where` 已支持内建非资源 capability，可表达 `K: Eq + Hash`、`T: Ord`、`T: Sized` 这类基础约束。
- 未来重新设计库层时，`Vec[T]`、`Map[K,V]`、`Result[T,E]` / `Option[T]` 的 API 约束需要继续用语言机制承接；用户 trait、associated type 和资源相关约束等后续专题成型后再进入。
...
```aurex
&T        // safe shared reference，不能写
&mut T    // safe mutable reference，可写但当前不含 borrow checker
...
短期不需要立刻改完现有 `*const` / `*mut` receiver，但设计文档应明确：safe API 优先使用 `&T` / `&mut T`，raw pointer 留给 FFI 和 unsafe 内部实现。
...
- generic enum 已纳入 M2 基线；`Option[T]` / `Result[T, E]` 可作为类型参数 ADT 使用。
...
- 隐式虚调用。
- class-only 世界观。
- 在 trait、borrow/lifetime 和资源语义固定前做复杂对象生命周期。
...
1. slice type / slice expression 已落地；`strvalid` / `strfromutf8` 已锁住 byte slice 到 `str` 的 checked UTF-8 构造边界，`text[l:r]` 已按 byte offset 和 UTF-8 boundary 规则落地。
...
目标：把泛型和非资源类核心库约束正规化，并保持 safe reference 的最小边界。

任务：

1. 最小 `where` capability 已落地；继续保持 grammar、诊断和测试矩阵同步。
2. 为未来 `Vec` / `Map` / `Result` / `Option` 这类核心库 API 保留非资源类正式约束，而不是依赖约定或特判。
3. safe reference 已有最小实现；后续只继续设计 borrow/lifetime/alias/resource 语义，不在 M2 基础语法里扩张。
...
4. bool / no-payload enum 叶子组成的 tuple、struct 和小 fixed array 已支持结构化穷尽性和 unreachable 诊断；guard 已区分无 guard、字面量 true/false 和动态表达式；dynamic slice 代表长度 witness 和 open integer literal usefulness 已进入主路径。
...
- 但完整 Rust borrow checker 一次性照搬成本很高，Aurex 当前不应推进 move-only / Drop；应先冻结 `unsafe`、ADT、slice/string 和最小 safe reference 边界。
...
- Aurex 在引入 borrow 前，应先把 `unsafe`、最小 safe reference 边界和 AIR place/lvalue 模型固定；Drop / move-only 继续暂缓。
...
3. 保持 `unsafe` block / `unsafe fn` 的语法、诊断和测试同步。
4. 浮点字面量后缀策略已冻结，后续只保持 lexer/parser 文档同步。
5. struct pattern / nested pattern 已补齐，后续继续增强复杂 match 诊断。
6. slice type/expression 和 checked `str` slicing 已落地；后续只需保持普通 borrowed slice 与 `str` byte-boundary 规则在文档和测试中同步。
7. function pointer / function type 已落地；后续补充 closure/lambda 捕获专题。
8. 最小非资源类 `where` 约束已实现；后续只保持 grammar、诊断和测试同步。
9. 最小 `&T` / `&mut T` 已实现；后续只设计 borrow/lifetime/alias/resource 语义。
10. 继续保持 `match` 的 guard、slice 和开放域 structural exhaustiveness / unreachable 回归测试与文档同步。
