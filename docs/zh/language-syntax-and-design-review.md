...
M2 的直接结论：不要复活 M1 的标准库和 selfhost 路线来掩盖语言问题。default visibility、ADT-first enum、array literal / repeat literal、slice type/expression、tuple/destructuring、function type/function pointer、最小 unsafe、M2 pattern ergonomics 和 `str` checked UTF-8 构造边界已完成；下一步继续把 checked string slicing、capability/where 和 safe reference 这些基础规则定下来。
...
1. slice type / slice expression 已落地；`strvalid` / `strfromutf8` 已锁住 byte slice 到 `str` 的 checked UTF-8 构造边界，checked string slicing 后续继续和 UTF-8 boundary 规则对齐。
...
6. slice type/expression 已落地；`str` checked UTF-8 构造边界已冻结，后续补 checked string slicing 并继续明确与普通 slice 的关系。
