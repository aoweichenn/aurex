...
2. 泛型缺少正式约束。当前 no-std 分支已经移除了标准库名字特判；恢复核心库前，约束系统需要重新设计，但不进入当前基础语法阶段。
...
M2 的直接结论：不要恢复 M1 的标准库和 selfhost 路线来掩盖语言问题。default visibility 已切换到默认 private；下一步继续把 enum ADT、unsafe、capability/where 和 safe reference 这些基础规则定下来。
...
- 一旦恢复核心库，`Vec<T>`、`Map<K,V>`、`Result<T,E>` / `Option<T>` 的 API 约束需要语言机制承接，不能继续靠约定或库名特判；资源相关约束等后续资源模型成型后再进入。
...
- bitcast 对非平凡布局应要求 unsafe 或更严格限制。
...
当前 no-std 分支已经移除了标准库 API 的名字特判，这让语言核心更干净。但约束问题并没有消失：只要核心库重新出现，`Vec<T>`、`Map<K,V>`、`Result<T,E>` 这类 API 就必须能表达 equality、hash、ordering 等能力要求。这些要求应该进入语言的泛型约束，而不是停留在文档约定。
...
4. re-export 规则冻结。
5. import alias 继续作为推荐形式。
6. selective import 可以晚点做，不需要先加 glob。
