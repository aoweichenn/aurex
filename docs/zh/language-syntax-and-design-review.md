...
Aurex 现在已经不是只支持函数和表达式的小语言。当前语言核心已经具备系统语言雏形：模块、可见性、C FFI、结构体、枚举 payload、泛型、method、标准形状 `Result` / `Option` 约定与 `?`、`defer`、C-style `for`、块表达式、`if` 表达式、`match` 表达式、指针、内建 `str`、显式 cast、`noncopy struct` 和 `move(value)` 所有权 MVP 都已经落地。
...
3. `noncopy` / `move` 已经开始解决资源所有权，但还缺正式 `Drop` / borrow / unsafe 边界。
...
- `noncopy` / `move` / `Drop` / borrow 作为资源安全模型。
- raw pointer、`ptr_cast`、`bit_cast`、`ptr_from_addr`、`str_from_bytes_unchecked` 进入 `unsafe` 边界。
...
let var if else for while break continue defer return
noncopy move true false null
void bool i8 u8 i16 u16 i32 u32 i64 u64 isize usize f32 f64 str
mut cast ptr_cast bit_cast size_of align_of
...
当前没有 `+=`、`-=`, `++`、`--`、`..` range、`?:` 三元表达式、lambda 箭头、属性列表或宏调用语法。
...
- `ptr_cast` 只用于 pointer 到 pointer。
...
- `cast` / `ptr_cast` / `bit_cast` / `ptr_addr` / `ptr_from_addr`，前提是 operand 可 const eval。
- `size_of` / `align_of`。
...
- `noncopy struct` 标记 move-only。
- struct / enum copyability 会按字段或 payload 传播。
- `move(value)` 只能移动 local 或 parameter。
- noncopy 值进入 local 初始化、赋值、return、函数实参、method receiver、struct literal 字段、enum payload、if/block/match 结果时，必须显式 move，除非该表达式是 fresh owned expression。
- 使用已 moved 的 local / parameter 会诊断。
- `move(copyable)` 允许，但不会把源 binding 标记为 moved。
- 当前还没有 borrow checker、partial move、lifetime、automatic drop。
...
- 一旦恢复核心库，`Vec<T>` 的 by-value read / move-out、`Map<K,V>` 的 key/value API、`Result<T,E>` / `Option<T>` 的 noncopy payload API 都需要这套约束系统承接，不能继续靠约定或库名特判。
...
建议在 M1 之前切换到默认 private，所有跨模块 API 必须显式 `pub`。如果担心破坏样例，可以先做迁移期：

```text
0. 当前版本继续接受默认 public。
1. 开启 warning：跨模块可访问但未写 pub 的 item/field 提示。
2. M1 结束前改为默认 private。
```
...
### 6. `noncopy` 应升级为正向能力模型

`noncopy struct` 是好用的 MVP，但长期更推荐正向能力：

- 默认值是否 `Copy` 由字段决定。
- 显式 `noncopy` 可以保留为“禁止自动 Copy”的简写。
- 有 `drop(self: &mut Self)` 或 `destroy(self: *mut Self)` 的类型自动是 `Drop`。
- generic constraint 用 `T: Copy` / `T: Drop`，而不是函数内部手写限制。

短期路线：

1. 保留当前 `destroy(self: *mut T) -> void` destructor 识别。
...
- `?` 的 control flow 可 lowering、可分析、可和 noncopy payload 结合。
- 异常会绕过当前 `defer` / ownership / Drop 的许多语义边界。

未来可以考虑 typed effects，但不建议 M1 引入用户可见 effect syntax。Koka 的 effect typing 和 effect handlers 很值得参考，但 Aurex 现阶段更需要先把 `Result`、Drop、borrow、unsafe 做稳。
...
5. iterator / range / slice 语法在 borrow 基线后再定。
