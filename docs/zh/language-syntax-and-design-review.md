...
4. bool / no-payload enum 叶子组成的 tuple、struct 和小 fixed array 已支持结构化穷尽性和 unreachable 诊断；guard 已区分无 guard、字面量 true/false 和动态表达式，后续继续改进 slice 和开放域 diagnostics。
...
10. 继续增强 `match`：guard、slice 和开放域 structural exhaustiveness / unreachable 诊断。
