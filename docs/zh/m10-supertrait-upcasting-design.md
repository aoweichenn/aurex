...
状态：**M10a Supertrait Upcasting Design Baseline**、**M10b Supertrait Frontend / Query / Sema Implementation**、
**M10c Supertrait IR / Backend Runtime Implementation** 和 **M10d Supertrait Hardening / Release Closure** 已完成。
M10b 已实现 parser/AST、checked facts、query key、ABI DTO 和 sema check-only borrowed dyn-to-dyn coercion；
M10c 已实现 `trait_object_upcast` IR、`supertrait_vptr_metadata_v1` vtable metadata、LLVM parent vtable projection 和
native inherited parent dispatch；M10d 已完成 query/cache/tooling projection polish、negative sample matrix、
documentation tests、coverage gate 和 release baseline。
M10 仍不实现标准库、owning dyn、`Box<dyn Trait>`、dynamic Drop dispatch 或 multi trait composition。M10 建立在
[Aurex M9 Dyn ABI / Tooling Release Baseline](m9-release-baseline.md) 之上。
...
- 只处理 borrowed dyn-to-dyn coercion：`&dyn Child -> &dyn Parent`、
  `&mut dyn Child -> &mut dyn Parent` 和 `&mut dyn Child -> &dyn Parent`。
...
- M10b 已实现 `TraitSupertraitEdgeFact`、`TraitObjectUpcastCoercionKey` 和 `DynUpcastAbiDescriptor`。
- M10c 已实现 vtable supertrait edge layout fact、`trait_object_upcast` IR 和 LLVM runtime parent vtable projection。
- M10d 已实现 upcast query/cache/tooling release closure，见
  [Aurex M10 Supertrait Upcasting Release Baseline](m10-release-baseline.md)。
- M10 不实现标准库，不实现 `Box<dyn Trait>`，不实现 owning dyn，不实现 dynamic Drop dispatch，不实现
...
M10 以当前仓库代码为准，不从文档或外部语言倒推。本节先保留 M10a 的输入事实，再说明 M10b 已经落地的改动。
...
```text
trait Name[Params]? (":" Supertrait ("," Supertrait)*)? where ... { ... }
```

`src/frontend/parse/grammar/parser_trait.cpp` 现在解析 direct supertrait list，`TraitSupertraitDecl` 保存 parent type、
source range 和 direct edge ordinal，`ItemNode::trait_supertraits` / `TraitItemPayload::supertraits` 保存 AST 事实；
AST dump、module store/load、copy/move 均已接入。
...
M10b 已新增 `TraitSupertraitInfo` 和 `TraitSignature::supertraits`，保存 child/parent `DefKey`、parent trait args、
edge fingerprint、direct edge ordinal、closure depth、source range 和 part index。Associated equality edge mapping
仍未完整实现；当前 mature path 是 parent trait args substitution。

### 1.3 Trait object query key

M8/M9 已有三类 dyn query key：
...
M10b 新增第四类 key：`TraitObjectUpcastCoercionKey`。`src/infrastructure/query/trait_object_key.cpp` 对
`TraitObjectCoercionKey` 的 validation 明确要求
`vtable_layout.concrete_type == source_type`，因此它只能表达 concrete-to-dyn borrowed coercion，不能表达
`dyn Child` 到 `dyn Parent` 的 upcast；upcast 必须通过新的 `TraitObjectUpcastCoercionKey` 表达。

### 1.4 Sema coercion

`src/frontend/sema/facade/sema_types.cpp` 继续保留 M8/M9 concrete-to-dyn coercion 的保守边界，同时新增
borrowed dyn-to-dyn upcast checker。M10b 不放宽 `TraitObjectCoercionKey`，而是在 contextual expected type /
assignment path 上发现 source/target 都是 borrowed trait object reference 时，查找已验证的 supertrait edge path，
再记录 `TraitObjectUpcastCoercionFact`。

### 1.5 Checked facts 和 ABI DTO

M8/M9 `CheckedModule` 中与 dyn 有关的事实包括：
...
- `TraitSupertraitEdgeFact`
- `TraitObjectUpcastCoercionFact`
- `TypeCheckBodyAuthority::trait_supertrait_edge_count`
- `TypeCheckBodyAuthority::trait_object_upcast_coercion_count`

M9 新增的 `FunctionDynAbiFacts` 原本能把 object/vtable/slot/coercion/dispatch 投影为 library-independent facts；
M10b 现在还可以投影 upcast descriptors。它目前承认：

- ABI policy：`borrowed_view_v1`
- metadata policy：`borrowed_methods_only_v1`
- upcast metadata policy：`supertrait_vptr_metadata_v1`

`DynAdvancedDesignGate` 已把 `supertrait_upcasting` 标成 `design_gate`，并要求新 metadata policy
`supertrait_vptr_metadata_v1`。M10a 把这个 gate 变成完整设计基线；M10b 把 frontend/query/sema 侧的 check-only
facts 落地。
...
M10b 现在新增的 check-only 能力：

- 可以写 `trait Child: Parent { ... }` 和 `trait Child[T]: Parent[T] where ... { ... }`。
- 可以在 checked facts 中表达 direct/transitive `TraitSupertraitEdgeFact`。
- 可以诊断 duplicate direct parent、supertrait cycle、public trait exposes private supertrait。
- `impl Child for T` 会要求 parent trait evidence；泛型 parent args 会按 impl 实参替换。
- 可以从 `&dyn Child` contextual coercion 到 `&dyn Parent`，从 `&mut dyn Child` coercion 到
  `&mut dyn Parent` 或 `&dyn Parent`，并记录 `TraitObjectUpcastCoercionFact`。
- 可以用 `TraitObjectUpcastCoercionKey` 和 `DynUpcastAbiDescriptor` 表达 upcast fact、summary、fingerprint 和 dump。

M10c 继续新增的 runtime 能力：

- 可以在 child dyn vtable 中 runtime 读取 parent vtable metadata。
- 可以把 inherited parent method direct dispatch on `dyn Child` 当成 runtime-ready capability。
- 可以生成 `trait_object_upcast` IR。
- 可以生成带 supertrait vptr metadata 的 LLVM vtable global。

当前仍不能做：

- 不能完整表达 supertrait associated equality edge mapping / ambiguity solver；M10b 已支持泛型 parent args substitution。
...
- M10b Sema 已经可以证明 `Parent` 是 `Child` 的 supertrait，并能在显式/contextual target parent view 上记录 upcast
  fact。
- M10c 已把直接在 `&dyn Child` receiver 上调用 inherited parent method 接成 runtime-ready 能力：lowering 会先生成
  parent view projection，再按 parent vtable slot dispatch。
- M10c IR lowering 显式产生 `trait_object_upcast` / parent vtable projection，不把 parent method slot 复制进 child
  method table。
...
Supertrait edge 允许携带 parent trait args；associated equality mapping 是设计目标但不是 M10b 的完整实现。规则是：

- M10b 已支持 `trait Child[T]: Parent[T]` 这类 parent trait args substitution，并把 transitive generic args
  实例化到 edge/path fingerprint。
...
- 如果多条 edge path 到同一个 parent 产生不一致 equality，后续 M10c/M10d 或专门 hardening 阶段必须诊断
  ambiguity，不生成 upcast key。
...
struct TraitSupertraitEdgeFact {
    ...
    IdentId child_trait_name_id;
    syntax::ModuleId child_trait_module;
    InternedText parent_trait_name;
    IdentId parent_trait_name_id;
    syntax::ModuleId parent_trait_module;
...
- Edge path 是否可由 checked graph 证明。
- Parent trait args 在当前 edge/path 下是什么。
- Parent vtable metadata 在 source vtable 中的 edge ordinal/path 是什么。

### 7.2 `TraitObjectTypeKey`

`TraitObjectTypeKey` 继续表示单 principal trait object，不变成 principal set。M10b 没有把 schema 升级到 v2；它
继续使用 `object_callability_schema` 表达当前 principal object callability，supertrait edge/path 独立进入 checked
facts 和 upcast key。
...
Concrete-to-dyn 的 `TraitObjectCoercionKey` 不应被复用。M10b 已新增：
...
- source origin、edge path fingerprint 和 global id 非空。
- source 与 target principal trait 不同。
- source 和 target ABI policy 一致，当前为 `borrowed_view_v1`。
- borrow kind 是 valid enum value。
- target principal trait 是 source principal trait 的 supertrait，这一点由 sema checked fact 证明，不由 query key
  单独重跑 graph validation。
- borrow kind 不把 shared 升级成 mutable；该权限规则由 sema upcast checker 保证。
...
M10c 已在 IR/backend ABI facts 中表达 vtable parent edge。当前代码中的等价 runtime layout fact 是
`TraitObjectVTableSupertraitEdge`，其语义对应设计中的 `VTableSupertraitEdgeDescriptor`：
...
它的职责是证明：给定 source layout 的 vtable pointer，可以取得 target layout 的 parent vtable pointer。M10c 已从
checked supertrait edge/path 和 upcast coercion fact 生成 runtime source/target vtable layout projection，并要求每个
concrete child vtable 都携带同序 supertrait edge。

### 9.2 `DynUpcastAbiDescriptor`

`FunctionDynAbiFacts` 已新增 upcast descriptor。当前实现形状是：

```cpp
struct DynUpcastAbiDescriptor {
    query::TraitObjectUpcastCoercionKey upcast;
...
M9c 的 `supertrait_upcasting` candidate 是 design gate。M10a/M10b 没有回写 M9c release baseline DTO；它应该保持
可回溯。后续如果需要一个 M10-specific stage fact，可以新增 `DynSelectedAdvancedDesign`，记录：
...
M10b 当前没有新增该 stage fact；实际可查询事实已经通过 checked module 和 `FunctionDynAbiFacts::upcasts` 暴露。

## 10. Sema Pipeline

M10b 的 frontend/query 实现顺序已经完成到 check-only 边界，M10c 已在其后追加 runtime lowering/backend 链路：
...
6. Parent trait args substitution 为每条 edge/path 生成 stable fingerprint；associated equality mapping 仍留后续。
7. Trait impl checking 把 supertrait predicates 加入 obligation：`impl Child for T` 需要可证明 `T: Parent`。
8. Borrowed dyn-to-dyn coercion checker 生成 `TraitObjectUpcastCoercionKey`。
9. Checked facts / dump / fingerprint / IDE semantic facts 投影 upcast edge。
10. M10c lowering 生成 `trait_object_upcast` IR，并在 child vtable layout 上生成 supertrait edge。
11. M10c verifier 校验 source/target object、layout、edge index、upcast key、borrow kind 和 metadata policy。
12. M10c LLVM backend 生成 `{ [methods x ptr], [supertraits x ptr] }` vtable global，并从 source vtable 投影 parent
    vtable pointer。
...
## 11. IR 和 Backend Runtime

M10c 已实现 runtime lowering。IR 新增 value：
...
- parent vtable value 来自 source vtable 的 `TraitObjectVTableSupertraitEdge`。
...
- `borrowed_methods_only_v1` layout 可保持 methods-only metadata。
- `supertrait_vptr_metadata_v1` layout 使用 explicit metadata shape：`{ [methods x ptr], [supertraits x ptr] }`。
- `vtable_slot` emission 从 vtable struct 的 method table field 读取 slot。
- `trait_object_upcast` emission 复用 data pointer，并从 supertrait edge table 读取/投影 parent vtable pointer。
...
M10b/M10c 的测试必须按模块拆，不再把所有 dyn trait case 塞进一个巨型测试文件。M10b 已新增：
...
- `tests/gtest/midend/ir/lower_ast_dyn_trait_tests.cpp`：`trait_object_upcast` IR、supertrait edge verifier、
  inherited parent dispatch、多 concrete child vtable edge 和 LLVM parent projection 覆盖。
- `tests/gtest/integration/native_execution_tests.cpp`：native inherited supertrait dispatch 和多 concrete parent vtable
  projection 覆盖。

M10d 已新增/更新：

- `tests/gtest/application/tooling/ide_tooling_tests.cpp`：supertrait upcast-only function 的 dyn ABI semantic fact、
  lower-IR query surface 和 hover projection 覆盖。
- `tests/gtest/infrastructure/query/trait_object_upcast_key_tests.cpp`：upcast summary、edge/borrow fingerprint drift
  和 lower-IR result invalidation 覆盖。
- `tests/samples/negative/traits/trait_dyn_upcast_non_supertrait_target.ax`
- `tests/samples/negative/traits/trait_dyn_upcast_shared_to_mut.ax`
- `tests/samples/negative/traits/trait_dyn_upcast_missing_parent_impl.ax`

Coverage 要求按当前仓库门槛保持 90% 以上。M10b/M10c/M10d 需要 focused parser/sema/query/IR/backend/runtime tests
固定 positive、negative、fingerprint、dump 和 validation matrix；documentation tests 必须固定本设计中的关键词和
非目标，防止后续漂移。
...
| M10b frontend/query/sema | 已完成。supertrait syntax/AST、trait graph facts、cycle/visibility diagnostics、impl parent evidence、generic parent args substitution、`TraitObjectUpcastCoercionKey`、checked dump/fingerprint facts；associated equality edge mapping 留后续 | 1,800-3,000 行，实际以 diffstat 为准 |
| M10c IR/backend runtime | 已完成。`supertrait_vptr_metadata_v1` vtable layout、`VTableSupertraitEdgeDescriptor` 等价 IR fact、`trait_object_upcast` IR、verifier、LLVM lowering、runtime native coverage | 预估 1,600-2,800 行；实际 `37 files changed, 1316 insertions(+), 255 deletions(-)` |
| M10d hardening/release | 已完成。negative matrix、query/cache invalidation、IDE hover polish、coverage closure、language manual/sample/release docs | 700-1,300 行，实际以 M10d diffstat 为准 |

合计预估：M10 完整 supertrait upcasting 需要约 4,800-8,200 行新增/修改。M10b 的实际代码量如果偏离
1,800-3,000 行，优先分析 parser/AST storage、checked-module copy/fingerprint/dump、query validation 和 focused
tests 的拆分成本，而不是把 runtime 代码提前塞进 M10b。

M10c 实际低于预估，原因是 M10b 已提前完成 checked/query DTO，M8/M9 已有 borrowed dyn ABI/tooling 基线，
LLVM lowering 复用现有 fat-view/indirect-call 路径，且标准库、owning dyn、allocator、dynamic Drop dispatch 和
multi trait composition 都明确排除在 M10c/M10d 之外。该偏差是范围控制造成的正向偏差，不是功能漏做。

## 15. M10a/M10b 非目标

M10a/M10b/M10c/M10d 明确不做：
...
M10d 已完成。M10 的退出标准不是继续追加新 dyn 语义，而是证明 M10c 已有 runtime 链路在 checked fact、IR、
verifier、backend global layout、runtime execution、query/cache/fingerprint、documentation 和 coverage 上都稳定；
该标准已通过 M10d release closure 固定。

M10 结束后，下一步应进入 **M11 Advanced Dyn Design Baseline**：从 M9c gate 的剩余候选中选择下一条 advanced dyn
主线，先设计 policy/schema、query DTO、verifier/backend negative matrix、tooling/cache 投影和非目标。M11 不应直接
实现标准库、owning dyn、`Box<dyn Trait>`、allocator 或 dynamic Drop dispatch。
