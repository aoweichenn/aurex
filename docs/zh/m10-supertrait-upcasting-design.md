...
状态：**M10a Supertrait Upcasting Design Baseline** 已完成；**M10b Supertrait Frontend / Query / Sema Implementation**
已完成。M10b 已实现 parser/AST、checked facts、query key、ABI DTO 和 sema check-only
borrowed dyn-to-dyn coercion，不实现 IR lowering、LLVM backend runtime 或标准库代码。M10 建立在
...
- `VTableSupertraitEdgeDescriptor`、`trait_object_upcast` IR 和 LLVM runtime parent vtable projection 留给 M10c。
...
- 不能在 child dyn vtable 中 runtime 读取 parent vtable metadata。
- 不能把 inherited parent method direct dispatch on `dyn Child` 当成 runtime-ready capability；需要显式 parent view
  或等待 M10c/M10d 接入。
...
- M10b Sema 已经可以证明 `Parent` 是 `Child` 的 supertrait，并能在显式/contextual target parent view 上记录 upcast
  fact。
- 直接在 `&dyn Child` receiver 上调用 inherited parent method 还不是 runtime-ready 能力；M10c/M10d 需要先把 parent
  view projection 和 parent vtable slot dispatch 接上。
- M10c IR lowering 应显式产生 parent view 或 parent vtable projection，不把 parent method slot 复制进 child method
  table。
...
M10b 尚未实现 runtime vtable parent edge descriptor；M10c 需要在 IR/backend ABI facts 中表达 vtable parent edge：
...
它的职责是证明：给定 source layout 的 vtable pointer，可以取得 target layout 的 parent vtable pointer。M10b 只记录
checked supertrait edge/path 和 upcast coercion fact，不生成 runtime source/target vtable layout projection。
...
M10b 的 frontend/query 实现顺序已经完成到 check-only 边界：
...
Coverage 要求按当前仓库门槛保持 90% 以上。M10b 需要 focused parser/sema/query tests 固定 positive、negative、
fingerprint、dump 和 validation matrix；documentation tests 必须固定本设计中的关键词和非目标，防止后续漂移。
...
- 不实现 `trait_object_upcast` IR。
- 不实现 LLVM `supertrait_vptr_metadata_v1` vtable global。
- 不把 inherited parent method direct dispatch on `dyn Child` 当成 runtime-ready 能力。
...
M10b 完成后，下一步应进入 **M10c Supertrait IR / Backend Runtime Implementation**：

- 新增 `trait_object_upcast` IR value / operation。
- 新增 `VTableSupertraitEdgeDescriptor` 或等价 layout fact。
- 让 verifier 覆盖 source/target object、borrow kind、origin、edge target、metadata policy 和 parent vtable pointer
  type negative matrix。
- 让 LLVM backend 生成 `supertrait_vptr_metadata_v1` explicit metadata shape，并在 upcast lowering 中复用 data pointer、
  投影 parent vtable pointer。
- 补 runtime positive/negative samples。
- 继续保持“不实现标准库”的阶段边界。

M10c 的退出标准不是“能在一个 sample 中手写投影成功”，而是 checked fact、IR、verifier、backend global layout、
runtime execution、query/cache/fingerprint 和 coverage 都能证明 borrowed supertrait view 的 runtime 链路稳定。
