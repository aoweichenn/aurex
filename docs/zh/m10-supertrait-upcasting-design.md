...
和 **M10c Supertrait IR / Backend Runtime Implementation** 已完成。M10b 已实现 parser/AST、checked facts、
query key、ABI DTO 和 sema check-only borrowed dyn-to-dyn coercion；M10c 已实现 `trait_object_upcast` IR、
`supertrait_vptr_metadata_v1` vtable metadata、LLVM parent vtable projection 和 native inherited parent dispatch。
...
Coverage 要求按当前仓库门槛保持 90% 以上。M10b/M10c 需要 focused parser/sema/query/IR/backend/runtime tests
...
| M10c IR/backend runtime | 已完成。`supertrait_vptr_metadata_v1` vtable layout、`VTableSupertraitEdgeDescriptor` 等价 IR fact、`trait_object_upcast` IR、verifier、LLVM lowering、runtime native coverage | 1,600-2,800 行，实际以 M10c diffstat 为准 |
...
M10c 完成后，下一步应进入 **M10d Supertrait Hardening / Release Closure**：

- 补 query/cache/tooling projection polish，确保 checked/IR dyn ABI facts、hover、dump 和 fingerprint 一致。
- 补 runtime/diagnostic negative matrix：invalid target、borrow mutability upgrade、missing parent evidence、
  layout/edge mismatch 和 associated equality edge ambiguity。
- 收口 language manual、usage、progress、version、README、requirements 和 documentation tests。
- 跑完整 tests、90% coverage gate、diff check 和 release self-review。
- 继续保持“不实现标准库”的阶段边界。

M10d 的退出标准不是继续追加新 dyn 语义，而是证明 M10c 已有 runtime 链路在 checked fact、IR、verifier、backend
global layout、runtime execution、query/cache/fingerprint、documentation 和 coverage 上都稳定。
