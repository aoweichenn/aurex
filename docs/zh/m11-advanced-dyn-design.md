...
状态：**M11e Principal-Set Composition Hardening / Release Closure 已完成**。M11a/M11b 已固定
compiler/query facts baseline；M11c 已接入 parser/AST/type/sema；M11d 已接入显式 composition-to-principal
IR/backend runtime projection；M11e 已完成 query/cache/tooling/verifier/docs release closure。当前仍不是标准库、
owning dyn、allocator 或 dynamic Drop dispatch 实现阶段。M11 release 入口见
...
M11c 在 M11b 地基上落地 frontend/sema：当前用户可写 spelling 是 `dyn (A + B)`，支持
`&dyn (Draw + Debug)` / `&mut dyn (Draw + Debug)` annotation，以及 `&T` / `&mut T` 到 borrowed composition
view 的 contextual coercion。M11d 在此基础上落地显式 runtime projection：`&dyn (A + B) -> &dyn A` 和
`&mut dyn (A + B) -> &mut dyn A` 会 lowering 为 `trait_object_composition_project`，从
`principal_set_metadata_v1` metadata 中取出目标 principal vtable。M11d 仍不实现 bare `dyn A + B` parser syntax、
direct principal-qualified dispatch 或标准库。
...
M11a/M11b 新增的是设计和 query facts 能力；M11c 新增的是用户可写 composition 语法和 sema facts；M11d 新增的是显式 runtime projection；M11e 新增的是 release-quality query/cache/tooling/verifier closure：
...
- 可以把 borrowed composition view 显式赋给某个 principal dyn view，例如
  `let draw: &dyn Draw = combo;`，再复用 single-trait dyn dispatch 调用 `draw.draw()`。
- IR dump 中可以看到 `dyn.composition.pack`、`dyn.composition.project` 和
  `principal_set_metadata @__aurex_principal_set_metadata_...`。
- LLVM backend 会生成 `{ [N x ptr] }` principal-set metadata global，projection 按 canonical principal index 加载目标 vtable。
- `FunctionDynAbiFacts` 会暴露 `principal_sets`、`composition_projections`、principal-set witnesses 和 projection
  borrow kind。
- lower-IR query fingerprint、IDE semantic fact 和 hover 会消费 composition runtime descriptors。
- IR verifier negative matrix 已覆盖 duplicate witness、metadata identity drift、missing pack metadata、invalid
...
- direct principal-qualified method dispatch、auto trait composition、structural conformance、
  Swift-style owning existential value buffer。

M11 release 当前也不能做：

- principal-set method call resolution。
- `combo.draw()` 形式的 direct composition receiver dispatch；必须先显式 projection 到 `&dyn Draw`。
...
M11d 对该 policy 的 runtime 解释已经固定为 borrowed-only metadata array：

- Composition borrowed view 是 `{data*, principal_set_metadata*}`。
- Metadata global shape 是 `{ [N x ptr] }`，entries 按 principal-set 类型的 canonical principal order 排列。
- 每个 entry 指向一个 single-trait vtable witness；同一个 principal set 对不同 concrete type 生成不同 metadata
  global。
- Composition-to-principal projection 不修改 data pointer，只从 metadata array 加载目标 principal vtable，再构造
  `{data*, vtable*}` single dyn view。
- Metadata 中仍没有 destructor slot、allocator slot、owner/value-buffer slot、size/align/type metadata。
...
M11b/M11c/M11d validation 固定以下边界：
...
- aggregate facts 必须引用已记录的 identity，summary 必须与 facts 重新计算结果一致，非空 fingerprint 必须与当前 facts
  重新计算结果一致。
- IR verifier 要求 principal-set metadata layout 以 `(principal_set_identity, concrete_type)` 唯一，object type 必须是
  同 identity 的 principal-set trait object，witness 必须覆盖 canonical principal index，composition pack 的 data
  必须是 concrete pointer/reference，composition project 的 target 必须是 principal-set 内的 single-trait object。
...
multiple concrete witness sets。M11d focused tests 覆盖 frontend composition-to-principal projection、mutable
projection borrow kind、IR `dyn.composition.pack` / `dyn.composition.project` lowering、multiple concrete metadata
layout、LLVM principal vtable load 和 native execution dispatch。

M11d 仍明确不实现标准库、不实现 bare `dyn A + B` parser syntax、不实现 direct principal-qualified method
dispatch、不实现 owning dyn、不实现 `Box<dyn Trait>`、不实现 allocator API/policy、不实现 dynamic Drop dispatch。
...
M11d 已实现 IR/backend runtime projection；M11e 已继续扩展 release negative matrix：

- duplicate witness index 被拒绝。
- metadata identity drift 被拒绝。
- composition pack 找不到 matching metadata layout 被拒绝。
- composition projection principal index 越界被拒绝。
- composition projection 缺失 principal object 被拒绝。
- composition projection principal object 与 target principal 不匹配被拒绝。

仍未打开的 matrix 包括 direct composition method dispatch、principal-qualified slot namespace lowering、owning
metadata、allocator metadata 和 dynamic Drop slot。它们必须留到后续阶段，不能混入 M11 的 borrowed
`principal_set_metadata_v1` release baseline。
...
M11c/M11d/M11e 已继续按功能模块拆测试，不把所有 dyn 测试塞回一个巨大文件：
...
- negative samples。
- IR/verifier focused tests；M11d 已加入 composition pack/project 和 metadata layout 覆盖。
- backend/native tests；M11d 已覆盖 LLVM principal vtable load 和显式 projection native execution。
- M11e 已补 query descriptor validation、lower-IR fingerprint invalidation、IDE hover/semantic fact、IR facts boundary
  和 verifier negative matrix。

## 12. 明确非目标

M11 release baseline 明确不做：
...
M11b 预估是 800-1,400 行，M11c 预估是 1,800-3,200 行，M11d 早期预估是 1,600-2,800 行。实际完成后应以
commit diffstat 为准；若偏离预估，主要原因通常是文档入口、documentation tests、query validation 强度、
focused tests 数量，以及 M8/M10/M11b/M11c 已有地基可复用程度。M11d 实际选择了更稳的显式 projection runtime，
没有直接打开 principal-qualified composition receiver dispatch，因此比“完整 direct dispatch”方案少了语法和 method
namespace lowering 代码。
...
| M11c Principal-Set Composition Frontend / Sema Check-Only | 已完成。`dyn (A + B)` parser/AST、type identity、coercion check、method-call guard、associated equality merge check、checked dump/fingerprint、negative samples、历史 IR runtime guard | 实际以本次 diffstat 为准 |
| M11d Principal-Set Composition IR / Backend Runtime | 已完成。IR composition pack/project、principal-set metadata layout、verifier、LLVM metadata global、显式 principal projection、native runtime tests；仍不实现 owning dyn/stdlib/direct composition method dispatch | 实际以本次 diffstat 为准 |
| M11e Hardening / Release Closure | 已完成。query/cache/tooling polish、IDE hover/workspace index、verifier negative matrix、docs/tests release closure、代码量偏差分析 | 实际以本次 diffstat 为准 |
| M12 design baseline | direct principal-qualified dispatch、owning dyn、dynamic Drop dispatch、allocator policy 或 auto trait composition 等候选选择一条独立主线 | 600-1,200 行文档/测试 |
...
M11 已结束。下一步应进入 M12 Advanced Dyn Design Baseline，继续在不实现标准库和 owning dyn 的前提下先做设计、
policy/schema、query facts、diagnostics、verifier/backend test plan 和代码量估算。
