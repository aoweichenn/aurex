...
状态：**M11d Principal-Set Composition IR / Backend Runtime 已完成**。M11a/M11b 已固定
compiler/query facts baseline；M11c 已接入 parser/AST/type/sema；M11d 已接入显式 composition-to-principal
IR/backend runtime projection。当前仍不是标准库、owning dyn、allocator 或 dynamic Drop dispatch 实现阶段。
...
M11a/M11b 新增的是设计和 query facts 能力；M11c 新增的是用户可写 composition 语法和 sema facts；M11d 新增的是显式 runtime projection：
...
M11d 已实现 IR/backend runtime projection；M11e hardening 仍必须继续扩展 negative matrix：
...
这些 verifier/backend tests 应在 M11e hardening / release closure 中继续分批补齐，并作为 release gate 固定。
...
M11c/M11d/M11e 应继续按功能模块拆测试，不把所有 dyn 测试塞回一个巨大文件：
...
M11e 是 M11d 之后最合适的下一步，因为 parser/AST/type/sema facts 和显式 runtime projection 已经固定，下一步应在
仍不实现标准库和 owning dyn 的前提下把 query/cache/tooling、negative matrix、stress/perf gate、文档和代码量偏差分析
收口成 release baseline。
