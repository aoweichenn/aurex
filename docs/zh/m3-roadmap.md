...
`m3`，当前最高优先级切到 M3.2 Query-backed Sema。
所有 M3 实现必须复用
...
状态：2026-05-29，WP-1 Sema Query Authority Inventory、WP-2 Item And Body Provider Boundary 和
WP-3 Checked Fact Materialization 已完成。非泛型 item/body query 已使用 authority-backed provider input
和共享 result helper，incremental cache subject / provider-skip replay 能解释 item signature、function body
syntax 和 type-check body 的 sema 级复用。下一步进入 WP-4 Sema Service Boundary Split。
