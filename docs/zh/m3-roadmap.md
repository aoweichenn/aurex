...
Phase 9A-D 已完成收口，当前最高优先级切到 M3.1 泛型闭环。但所有 M3 实现必须复用
...
M3.1 的后续执行入口见 [Aurex M3.1 泛型闭环执行计划](m3.1-generics-plan.md)。后续按该文档中的
work package 推进，避免每次重新读取全部 M3 历史，同时保留跨 sema / query / lowering / backend 的全局不变量。
...
已完成的 M3.0 模块顺序作为历史验收保留：文档收束、module AST/parser、ModuleLoader、sema 可见性、
module graph/export query、package visibility、source-root topology、ModulePartKey、IDE part recovery
和 selective re-export 已闭环。当前从 M3.1 开始按下列顺序推进：

1. 1A 泛型 ABI 稳定化。
2. 1B 泛型实例身份贯通。
3. 2 泛型 query 权威化。
4. 3 泛型 body/lowering 闭环。
5. 4 `sizeof[T]` / `alignof[T]` 闭环。
