...
M4-WP1 到 WP8 已完成。M4 release baseline 见
[Aurex M4 Trait / Protocol Release Baseline](m4-release-baseline.md)。后续工作应作为独立设计流启动，
而不是重新打开 M4 baseline。
...
WP8 已完成 release baseline 收口：文档集、语言表面、unsupported matrix、验证门禁、常规仓库测试、coverage、
query/cache/profile stress gates 和后续入口已经作为 M4 完成契约记录下来。M4 完成面继续保持 nominal、explicit、
