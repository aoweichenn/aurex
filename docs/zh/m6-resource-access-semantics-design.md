...
| 地基 | 当前状态 | 代码入口 |
| --- | --- | --- |
| 引用和值类别基础 | `TypeKind` 已区分 `pointer`、`reference`、`slice`、`str`、aggregate 和 generic param | `include/aurex/frontend/sema/type.hpp` |
...
| 稳定 query 身份 | `BodySlotKind::destructor_drop`、`GenericParamKind::resource` 和 `lifetime` 已预留 | `include/aurex/infrastructure/query/query_key.hpp` |
