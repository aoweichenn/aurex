...
Mojo 关注 Python 生态、性能和系统级控制，公开设计中强调 owned/borrowed/inout、参数 convention 和 origin/lifetime
概念。它给 Aurex 的价值不在语法复制，而在“先让常见调用写法自然，再让高级用户能表达所有权边界”。

对 Aurex 的启发：

- `borrowed` / `inout` / `owned` 是可读性很强的 API 术语，未来 surface 可以参考。
...
FunctionBorrowSummary {
  ...
  return_origin_dependencies: [ReturnSlot -> OriginParam/static/unknown]
