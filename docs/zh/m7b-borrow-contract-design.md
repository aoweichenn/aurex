...
### 3.3 `@borrow(...)` decorator surface

M7b 选择复用 `@name("...")` 的函数声明前装饰器位置，引入一个窄 borrow contract 装饰器：
...
fn choose_ref[T](left: &T, right: &T, take_left: bool) -> &T {
    if take_left {
        return left;
    }
    return right;
...
extern c {
    @borrow(return = [bytes])
    fn view_bytes(bytes: []const u8) -> str;
}
```
...
- 函数声明前装饰器位置已经存在，适合 ABI 和 contract metadata。
...
- `BodyLoan::parent_loan` 已落地；child loan 的有效 place 会通过 parent 底层 place 归一化。
- `&*carrier` / `&mut *carrier` 形态的 local carrier reborrow 进入 checker facts。
- child 活跃期间使用 mutable parent carrier 会产生 `reborrow_parent_use` conflict。
- child effective place 保留 field projection，因此 `value.field` 与 `value.other` 的 known-disjoint 放宽仍成立。
- raw pointer reborrow、跨闭包/generator 捕获、完整参数 origin reborrow proof 仍后置到 M7c/M8。
...
- `FunctionCallBinding` / `TraitMethodCallBinding` 记录 `receiver_access`、`receiver_auto_borrow` 和
  `receiver_two_phase_eligible`。
- body flow 只为语义确认的 mutable reference receiver auto-borrow 生成 `call_receiver_reserve` /
  `call_receiver_activate` action。
- reservation 期间拒绝 write/reinit/move/drop/cleanup、mutable borrow 和 nested mutable receiver reservation；
  shared read / shared receiver call 保持可用。
- activation 点会与活跃 loan 做 mutable conflict check，并报告 reservation / activation notes。
...
- 不新增关键字。
- 扩展函数 decorator 解析，支持 `@borrow(return = [name, self, static, unknown])`。
- AST 新增 `BorrowContractDecl` 或在现有 function decorator payload 中保存结构化 contract。
...
| `@borrow(return = [x])` | 不新增 keyword，适合 ABI/API metadata，易渐进 | decorator 参数 parser 要扩展，语义需严格校验 | M7b 采用 |
