...
- object-callability diagnostics 完整。
- 不做 backend dispatch。

验收：

- parser positive/negative。
- sema positive/negative：object-safe trait、generic method rejection、unconstrained associated type rejection。
- canonical type key 重新引入时必须结构化。
...
- borrow/lifetime conflict matrix。
- trait method binding whitebox。
- checked dump / IDE facts / query invalidation。
...
- coverage source lines/functions/regions 均达 95%。
