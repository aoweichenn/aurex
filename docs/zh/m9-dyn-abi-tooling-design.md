...
状态：M9 release closure 已完成。M8 release closure 已完成；M9 从 `m9` 分支开始，第一包已完成 borrowed dyn
ABI/tooling facts、advanced dyn 设计准入 gate 和 release baseline。当前仍不实现标准库、`Box<dyn Trait>`、
owning dyn、allocator、dynamic Drop dispatch、supertrait upcasting runtime 或多 trait object composition runtime。
最终收口记录见 [Aurex M9 Dyn ABI / Tooling Release Baseline](m9-release-baseline.md)。
...
| M9d release closure | M9 release baseline、文档入口、documentation tests、coverage/release gate；已完成，继续不实现标准库或 runtime | 300-700 行 |
...
- 新增 M9 release baseline。已完成。
- README / progress / version / next-steps 顶部状态收口到 M9 release closure。已完成。
- documentation tests 固定 M9 release contract 和非目标。已完成。
- release gate 保持 90% 以上。
...
- [Aurex M9 Dyn ABI / Tooling Release Baseline](m9-release-baseline.md) 作为 M9 最终收口入口。
- 文档测试防漂移。

M9 release 完成后，仓库仍不会新增：
