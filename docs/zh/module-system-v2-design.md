...
2. **支撑 M2.5 query / lossless / IDE 架构**。后续 parse、module graph、exports、item signature、body typecheck 都要变成 query，模块身份必须稳定，不能继续把“物理文件路径”当成“逻辑模块身份”。
...
`pub(package)` / `pub(crate)` 这类可见性都依赖 `PackageKey`。
...
static trait、资源语义、owned containers、class-like sugar 都需要稳定模块边界。否则 trait impl、Drop impl、associated type、generic instance key 都容易被临时路径和 display name 污染。
...
### Phase 4：package-private visibility
...
### Phase 5：ModuleExports query

把 public / package-public / re-export 表结构化。

交付：

- exports fingerprint。
- re-export cycle diagnostics。
- ambiguous re-export diagnostics。
- IDE definition target 可从 re-export 跳到真实定义。
...
5. [Kotlin documentation: visibility modifiers](https://kotlinlang.org/docs/visibility-modifiers.html)
6. [Kotlin specification: packages, modules and imports](https://kotlinlang.org/spec/kotlin-spec.html)
7. [Python reference: import system](https://docs.python.org/3.10/reference/import.html)
...
包边界：PackageKey / pub(package) / public API
```
...
3. 新增 `pub(package)` / `pub(crate)` 防止内部 API 被迫 public。
