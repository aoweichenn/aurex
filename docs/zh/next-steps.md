...
当前 selfhost 自举处在“可度量的 Stage1 前端切片 + TAC snapshot 输出”阶段，还不是能替代 Stage0 的完整自举编译器。

- Stage0 仍是 C++20 编译器，是当前生产编译器。
- selfhost 已包含 M0 编写的 lexer core、token dump、parser seed、ID-backed AST、Stage1 CLI 和 Stage1 TAC snapshot emitter。
- Stage1 能读取 `.ax` 文件，解析 seed 覆盖范围内的语法，并输出 `aurex_tac v0` 文本快照。
...
- Stage1 没有真正 TAC verifier：目前主要是 snapshot 输出，不是完整可执行 IR 合法性验证。
...
4. 让 Stage1 lowering 从 snapshot 走向真实 TAC 子集  
   先覆盖 function、block、value、terminator、locals、return、call、binary/unary、`if` 和 `while` CFG。

5. 实现 Stage1 TAC verifier
   先检查临时值引用范围、function 参数和返回类型一致、call 目标和参数数量一致。
