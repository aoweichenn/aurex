...
| perf baseline benchmark harness | 基础线已接入，跨前端对比通道和泛型 RSS/time 压测入口已接入 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；新增 `tools/generic_stress.py` / `make perf-stress` 固化 200/500/1000/2000 泛型实例的 elapsed time + peak RSS 基线；`--check` / checked dump 路径不再保留后端 lowering 专用的泛型实例 side table；暂不强制阈值 | 2M AST 节点 RSS、跨机器稳定阈值和 CI perf lane 仍后置 |
