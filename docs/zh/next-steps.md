...
| AST 多次整树复制 / 胖节点 | 已关闭当前主爆点 | module loader append 使用 compact payload move；driver 持有 parser/module AST，并把 mutable 引用交给 sema 和 IR lowering；`SemanticAnalyzer(const AstModule&)` 已删除，避免隐式整树复制；`CheckedModule::normalized_ast` 默认不再保留 AST snapshot；sema 构造期不再对 `exprs/types` 做 `size+4096` reserve；postfix materialization 不再按值复制胖 `ExprNode` / `TypeNode`；`TypeNode` / `ExprNode` / `PatternNode` 主存储已改成 32B compact header + per-kind payload arena，legacy fat node 只作为边界 materialized value | global bump allocator、AST 原生 `IdentId`、Stmt/Item 进一步压缩和 2M 节点跨机器 RSS 阈值仍是后续性能任务 |
...
| lookup / registry reserve | 已关闭当前 rehash 热点 | sema 全局表、generic 实例表、module visibility/export cache、函数/enum/const/ABI 临时表和局部 scope 按已知规模 reserve；struct field 注册避免逐字段重复查表 | 完整 typed identifier key / global interner 仍后置 |
| Identifier interner / typed lookup key | sema 热路径已阶段性关闭 | sema 新增 session 级 `IdentifierInterner`；type/function/value/generic template/enum case lookup 使用 `{ModuleId, IdentId}` 或 `{ModuleId, TypeHandle, IdentId}` typed 索引；生产索引完整时 visible/export module lookup miss 不再拼接 `module:name` / method string；诊断/dump 仍保留原始 display string | 跨阶段 global interner、stable hash、AST token/name 原生 `IdentId` 仍后置 |
...
| perf baseline benchmark harness | 基础线已接入，跨前端对比通道和泛型/AST RSS 压测入口已接入 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema、AST bulk lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；新增 `tools/generic_stress.py` 和 `tools/ast_stress.py`，`make perf-stress` 固化 200/500/1000/2000 泛型实例和 10000/50000/100000 AST bulk statements 的 elapsed time + peak RSS 基线；compact AST 后本机 100000 AST bulk statements 为约 200-215 MiB RSS / 118-127 ms；`--check` / checked dump 路径不再保留后端 lowering 专用的泛型实例 side table；暂不强制阈值 | 2M AST 节点跨机器稳定阈值和 CI perf lane 仍后置 |

当前仍不应声称已经完成的内容：跨阶段 global identifier interner / stable hash、AST token/name 原生 `IdentId`、global bump allocator、Stmt/Item compact storage、完整 pattern matrix / witness search、完整 `analyze_expr` 分层重写、did-you-mean/previous declaration note/warning-help 诊断增强、以及 2000 generic instance / 2M AST 节点的跨机器 RSS/耗时阈值和 CI perf lane。这些已从当前爆点中解耦，并已有 generic/AST stress baseline 入口，但仍属于 M2.1 后续架构和 perf lane 工作。
...
7. Identifier interner 和 typed lookup key。
   ...
   - 已完成 sema 热路径阶段：type/function/value/generic template/enum case lookup 使用 session `IdentifierInterner` + typed lookup key；`CheckedModule` 字符串 key 保留给 ABI、dump 和跨阶段边界，完整 global/stable interner 与 AST 原生 `IdentId` 继续后置。
8. Sema 不再复制整棵 AST。
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   - 当前已经消除 sema/checked 主路径整树复制和 side table 爆炸；`TypeNode`、`ExprNode` 和 `PatternNode` 已落为 32B header + per-kind payload arena。
   - 本机 `tools/ast_stress.py --skip-build --counts 10000,50000,100000` baseline 中，100000 AST bulk statements 约为 200-215 MiB RSS / 118-127 ms，较 compact 前约 575 MiB RSS / 135 ms 明显收敛。
   - 后续目标：global bump allocator、AST token/name 原生 `IdentId`、Stmt/Item compact storage、2M AST 节点跨机器 RSS/耗时阈值。
