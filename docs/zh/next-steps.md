...
| AST 多次整树复制 | 已关闭当前整树复制爆点 | module loader append 使用 move 节点；driver 持有 parser/module AST，并把 mutable 引用交给 sema 和 IR lowering；`SemanticAnalyzer(const AstModule&)` 已删除，避免隐式整树复制；`CheckedModule::normalized_ast` 默认不再保留 AST snapshot；sema 构造期不再对 `exprs/types` 做 `size+4096` reserve；postfix materialization 不再按值复制胖 `ExprNode` / `TypeNode` | compact AST layout、payload arena / bump allocator 和 2M 节点跨机器 RSS 阈值仍是后续性能任务 |
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   - 当前已经消除 sema/checked 主路径整树复制和 side table 爆炸；下一步再做 compact AST layout。
