...
| perf baseline benchmark harness / stress gate | 已接入 baseline、轻量 CI gate、默认 Release+LTO 发布 gate、阶段 profile 和跨机器校准机制 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema、AST bulk lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；`tools/generic_stress.py`、`tools/ast_stress.py` 和 `tools/diagnostic_stress.py` 统一通过 `tools/perf_thresholds.py` 支持 `--threshold-profile` / `--threshold-scale`，JSON 同时记录 raw thresholds、effective thresholds、profile、scale、machine info、Release/LTO build options、进程级 wall/user/sys/RSS/page fault 指标和 `aurexc --profile-output` 阶段 profile；`make perf-stress-threshold` 固化 100/200 generic + 1000/5000 AST + 100/500 errors 的 mixed-feature 轻量阈值质量门，GitHub Actions `stress-thresholds` job 覆盖三条 lane 并标记 `github-ubuntu-24.04-fedora` profile；`make perf-release-threshold` 默认使用独立 `build-perf-lto` Release+LTO 跑 5000 generic、2M 高复杂 mixed AST statements、5000 errors 的发布 lane，`perf-release-lto-threshold` / `perf-release-all-threshold` 为兼容别名；三条 stress 生成器默认 `--shape=mixed`，generic 覆盖 generic constraints / impl / pointer alias / tuple-pair / slice-pattern，AST 覆盖 extern、type alias、struct/enum、impl/method、generic、tuple、slice、pattern、try/defer/unsafe/loop，diagnostic 覆盖多类 sema 错误；AST mixed 已恢复高复杂 case 分布，2M 源码约 106820 KiB，发布 AST RSS 阈值为 8192 MiB；compact AST + AST 原生 `IdentId` + 删除旧胖 `ExprNode` 生产路径 + bump-backed AST header/payload vectors 后，本机 100000 mixed AST bulk statements 约 96.3 MiB RSS / 77.9 ms；表达式 arena 现在在 parser 初始化时按 token 形态 reserve 每类 backing storage + page pre-touch，表达式节点创建走字段级 emplace，不再先落大 payload 临时，也不再靠 vector 热路径自动扩容；lexer token arena 用 token-count estimator 一次性 reserve 且不预触未写入估算余量页，避免大源文件 bump vector 过量预留或扩容留下旧 backing storage；token 本体已压缩为 pointer + range，import/module parse 完成后释放 token arena；sema 持久 value-payload list 和 generic constraint bucket 已接入 bump arena；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr；`tools/frontend_compare.py` 本机 baseline 中 Aurex lookup/96 约 10.1 ms、generics/96 约 9.6 ms，Clang++ 分别约 21.2 ms / 24.3 ms，G++ 分别约 25.1 ms / 24.3 ms；当前默认 Release+LTO 发布 lane 本机 5000 generic 约 450.5 MiB / 13073.0 ms，2M 高复杂 AST statements 约 4325.9 MiB / 2841.3 ms，5000 errors 约 32.9 MiB / 66.7 ms；2M AST 阶段 profile 显示 module.read 约 27.2 ms / 227.1 MiB、module.lex 约 247.7 ms / 1291.3 MiB、module.parse 约 1130.0 ms / 3468.1 MiB、sema.analyze 约 1141.8 ms / 4325.9 MiB；`--check` 路径不再保留泛型实例 side table；`--emit=typed` 可单独压测 retained side table | 后续只保留新增机器 profile 数据，不再缺 2M/5000/5000 默认 Release+LTO 发布 gate、diagnostic stress、阶段 profile 或校准机制 |
...
8. Sema 不再复制整棵 AST。
   ...
   - 验收：默认 Release+LTO 2M 高复杂 mixed AST statements 源码约 106820 KiB，RSS 当前约 4325.9 MiB，低于发布阈值 8192 MiB；parser token buffer 已压缩为 pointer + range，ModuleLoader parse 完成后释放 token arena，阶段 profile 可直接看到 module.read/lex/parse 和 sema.analyze 的耗时/RSS-after。若继续收紧阈值，后续优先评估 streaming parser 或进一步减少 source/token/AST 同峰持有。
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   ...
   - 本机 `tools/ast_stress.py --skip-build --counts 100000 --shape mixed` baseline 中，100000 mixed AST bulk statements 约为 96.3 MiB RSS / 77.9 ms，较 compact 前约 575 MiB RSS / 135 ms 明显收敛；当前默认 Release+LTO 2M 高复杂 mixed AST statements 源码约 106820 KiB，约 4325.9 MiB RSS / 2841.3 ms；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr。
