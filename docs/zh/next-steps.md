...
| perf redline benchmark harness | 基础线已接入 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema；`tools/bench.py` 输出 Google Benchmark 报告，`tools/check_perf_redlines.py` / `make perf` 解析 `--benchmark_format=json` 并提供可由环境变量收紧的轻量红线 | 200/500/1000/2000 generic instance RSS、2M AST 节点 RSS、跨机器稳定阈值和 CI perf lane 仍后置 |
