...
- 字符串基础类型需要继续冻结到 public API：`str` 已按和 `int` 同层级的借用 UTF-8 文本切片推进，普通文本、原始 bytes、拥有型 `String`、C FFI 字符串和平台 `Path` 已在核心 std 类型上拆开；`std.fs.file` / `std.fs.dir` 已有 `Path` / `str` 新入口，目录 suffix 和 M1 axbuild 目录扫描已收口到 `str` / `Path`，后续风险主要是进程和剩余 FFI 场景中的底层 `c"..."` / `*const u8` 兼容入口还要继续收口。
