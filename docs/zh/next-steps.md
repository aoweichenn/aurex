...
| match exhaustiveness | 结构化 pattern 笛卡尔积可指数爆炸 | 用 pattern matrix / usefulness witness search 构造反例，不枚举全空间；guard、slice/open-domain 仍按当前 M2 边界保守处理 |
...
| contextual expression cache | 已关闭 | checked/generic side table 已拆出 `expr_intrinsic_types`、`expr_types` final table、`expr_expected_types` final-cache key 和 `CoercionRecord` overlay；integer/float/null、unary/binary、slice、array/tuple literal、if/block/match 在 expected type 下会保留 intrinsic type，并把 contextual final type 与 coercion/adjustment 单独记录 | 后续只保留 `analyze_expr` 责任拆分和更丰富 coercion kind，不再是 P0 缓存污染问题 |
...
| match exhaustiveness 笛卡尔积 | 已关闭正解主线 | `src/sema/match.cpp` 已替换为 pattern matrix / usefulness witness search：按 constructor specialization 和 default matrix 检查 bool、enum payload、tuple、struct、小/中 fixed array 的覆盖性和 unreachable arm；guarded arm 不计入穷尽覆盖；不再枚举笛卡尔积字符串，也不再使用 4096 组合上限 | guard 逻辑精确可用性、动态 slice 长度/open integer domain 的细粒度 witness 仍按当前 M2 边界保守处理 |
...
| identifier 临时 string | 已关闭当前主热路径 | syntax 层 `IdentifierInterner` 使用 global bump allocator 保存稳定文本；AST name-bearing 字段携带原生 `IdentId`；`SymbolTable` scope lookup、函数/类型/值/generic/enum case/method/member lookup 均使用 typed `IdentId` key；sema typed lookup key 复用 `AstModule` identifier arena，不再维护第二套私有 interner，也不再在 lookup miss 时回扫 string map；`FunctionSignature`、`Symbol`、`StructInfo`、`StructFieldInfo`、`EnumCaseInfo`、`TypeAliasInfo`、`TypeInfo` 和 generic template 的持久文本字段改为 `InternedText`，普通 source name 直接借用 AST identifier interner，生成的 ABI/display/dump 名进入 checked bump-backed interner，复制时重新 intern 到目标 arena，不再复制堆 `std::string` buffer；ABI symbol 校验使用 `std::string_view` key，不再临时复制一整套 C symbol interner | 跨模块 stable identifier hash、并行全局 ID 和 delayed display string 仍后置 |
| AST 多次整树复制 / 胖节点 | 已关闭当前主爆点 | module loader append 按 compact payload remap 并重新 intern 到目标 `AstModule`；driver 持有 parser/module AST，并把 mutable 引用交给 sema 和 IR lowering；`SemanticAnalyzer(const AstModule&)` 已删除，避免隐式整树复制；`CheckedModule::normalized_ast` 已改成轻量 normalization overlay，彻底不拥有 `AstModule` snapshot；节点级 C symbol side table 已从 `std::vector<std::string>` 改为 `expr_c_name_ids` / `pattern_c_name_ids` / `item_c_name_ids` 的 `IdentId` 表，文本只进入 checked module C-name interner 去重；sema 构造期不再对 `exprs/types` 做 `size+4096` reserve；postfix materialization 不再按值复制胖 `ExprNode` / `TypeNode`；`TypeNode` / `ExprNode` / `PatternNode` / `StmtNode` / `ItemNode` 主存储已改成 32B compact header + per-kind payload arena；旧胖 `ExprNode` 生产类型已删除，parser 创建、`AstModule` 存储、module loader append 和 postfix materialization 都直接写 compact expression header + per-kind payload；parser 表达式创建和 postfix materialization 进一步改为字段级 append/set，不再先构造 name/call/cast/postfix/array 等 payload 大临时对象；parser 启动按 token 形态估算 expression header 和每类 payload 的容量上界，直接从 bump arena 取好 vector backing storage 并对表达式 arena page pre-touch；解析中表达式创建只顺序 emplace，不再触发 vector 自动扩容或逐节点首次触页；lexer token 输出改为 bump-backed `TokenBuffer`，一次性 reserve 且不再被 262144 token 上限截断，也不再预触不会写入的估算余量页；sema 的 `CheckedModule`、generic side table、pattern case table、type/symbol table、analyzer lookup/cache 主表、函数签名参数/泛型实参、struct 字段、enum payload、`TypeInfo` tuple/function/generic args、generic constraint bucket 和持久 sema 文本字段均使用 bump-backed storage / interner；generic function instance 列表改为 bump-backed deque 保持嵌套实例化期间 side table 地址稳定；generic method / enum-case / visible-module cache bucket 不再由默认 heap vector 生成；sema、IR lowering 和 AST dump 的 `ExprNode` 热路径使用 compact view / payload 直读，literal 分析不再重建胖节点；sema item owner 查询改为显式 `ItemId`，不再靠 `items.data()` 反推；module/import/path 元数据 finalize 后同样进入 AST identifier arena | 2M 节点跨机器 RSS 阈值、generic/AST stress 阈值、跨模块 stable hash 和 CI perf lane 仍是后续性能任务 |
...
| record / enum case lookup 索引 | 已关闭当前退化路径 | struct/enum lookup 命中后做 type identity 校验；enum case 索引 miss 不再静默全表扫描通过；method/member、enum case 和 associated function 名字查找已统一走 `IdentId` typed key | 跨模块 stable member key / 增量失效 key 仍后置 |
...
| diagnostics line table / 输出上限 / 高价值 note-help | 已关闭当前主线 | `SourceFile` 建 line starts 表，诊断位置查询不再线性扫描；driver 输出最多 128 条诊断并打印 summary；`Diagnostic::Severity` 支持 error/warning/note/help；lookup miss 对变量/函数/类型给 `did you mean` help；let/const/assignment/return/call/enum payload/array/tuple/struct literal mismatch 输出 expected/actual note；duplicate function/type/const/local/field/enum case 等主路径输出 previous declaration note；parser 对表达式/type/pattern/call/index/slice 等成对 delimiter 缺失输出 opening delimiter note | 多行 span、颜色输出、module alias/field/enum case 更细 suggestion 仍是低风险增强项 |
...
| perf baseline benchmark harness / stress gate | 已接入 baseline 和轻量阈值 gate | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema、AST bulk lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；`tools/generic_stress.py` 和 `tools/ast_stress.py` 继续固化 500/1000/2000/5000 泛型实例、`--shape=templates` 和 10000/50000/100000 AST bulk statements 的 elapsed time + peak RSS 基线，并新增 `--max-elapsed-ms` / `--max-rss-mib` 以及对应 env；`make perf-stress-threshold` 固化 100/200 generic + 1000/5000 AST 的轻量阈值质量门，GitHub Actions 新增 `stress-thresholds` job；compact AST + AST 原生 `IdentId` + 删除旧胖 `ExprNode` 生产路径 + bump-backed AST header/payload vectors 后，本机 100000 AST bulk statements 约 140.8 MiB RSS / 72.4 ms；表达式 arena 现在在 parser 初始化时按 token 形态 reserve 每类 backing storage + page pre-touch，表达式节点创建走字段级 emplace，不再先落大 payload 临时，也不再靠 vector 热路径自动扩容；lexer token arena 一次性 reserve 且不预触未写入估算余量页，避免大源文件 bump vector 扩容留下旧 backing storage；sema 持久 value-payload list 和 generic constraint bucket 已接入 bump arena；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr；`tools/frontend_compare.py` 本机 baseline 中 Aurex lookup/96 约 10.1 ms、generics/96 约 9.6 ms，Clang++ 分别约 21.2 ms / 24.3 ms，G++ 分别约 25.1 ms / 24.3 ms；2000 generic instance stress 约 148.4 MiB RSS / 439.8 ms，仍低于 M2.1 约 150 MiB 目标；`--check` 路径不再保留泛型实例 side table；`--emit=typed` 可单独压测 retained side table | 2M AST 节点、5000 generic 和 5000 errors 的跨机器/release 阈值仍需继续校准；跨模块 stable hash 仍后置 |

当前仍不应声称已经完成的内容：跨阶段 stable identifier hash / parallel global ID、`analyze_expr` 内部职责进一步拆小、module alias/field/enum case suggestion 的全覆盖、多行 span / 颜色输出，以及 2M AST 节点、5000 generic、5000 errors 的跨机器/release RSS/耗时阈值。轻量 generic/AST stress 已进入本地和 CI 质量门，但重型 perf lane 仍需要继续校准。
...
2. match exhaustiveness 防指数爆炸。
   ...
   - guarded arm 不计入穷尽覆盖；动态 slice 长度、integer/open domain 仍要求 wildcard 或 irrefutable arm。
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   ...
   - 本机 `tools/ast_stress.py --skip-build --counts 10000,50000,100000` baseline 中，100000 AST bulk statements 约为 140.8 MiB RSS / 72.4 ms，较 compact 前约 575 MiB RSS / 135 ms 明显收敛；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr。
   - 后续目标：跨模块 stable hash / parallel global ID、2M AST 节点和 2000/5000+ generic stress 的跨机器 RSS/耗时阈值。
...
1. `did you mean` 建议已接入主 lookup。
   ...
   - module alias、field、enum case 的 suggestion 仍可继续细化。
...
2. 建立 stress/perf 测试分层。
   - 常规单测只跑轻量 baseline case；`make perf-stress-threshold` / CI `stress-thresholds` 跑 100/200 泛型实例和 1000/5000 AST bulk 的阈值门；release/perf lane 继续跑 2000/5000 泛型实例、AST bulk / 2M AST 节点、二进制输入、5000 errors、深括号和长操作链。
...
| match stress | 小结构精确 exhaustiveness；大结构 witness search 不枚举全空间；guard/open-domain/slice 边界保守诊断 |
