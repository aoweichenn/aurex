...
| lexer 二进制输入 DoS | 已关闭 | 连续 invalid byte 聚合为单个 range 诊断；lexer error diagnostics capped at 128 + summary；全局 diagnostic 输出上限和 line table 已落地 | 后续只保留诊断 category/code 和 LSP 结构化输出 |
