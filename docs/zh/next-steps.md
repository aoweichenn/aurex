...
M7d-E 之后本阶段不要把标准库作为工程入口。最直接的 compiler-only 候选是把 generic/opaque cleanup marker 的
ABI 策略正式化，或者继续补 tuple/index place-state gap；`Drop` bound、generic Drop surface、trait-object dispatch
