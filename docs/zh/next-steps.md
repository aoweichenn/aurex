...
| Identifier interner / typed enum case key | 第一阶段已落地 | sema 新增 session 级 `IdentifierInterner`；enum case 二级索引用 `{TypeHandle, IdentId}`，lookup miss 不再构造 `type:case` 临时字符串；诊断/dump 仍保留原始 display string | module/qualified/member key、跨模块 global interner 和 stable hash 仍后置 |
...
当前仍不应声称已经完成的内容：跨阶段完整 identifier interner / stable hash、compact AST、global bump allocator、完整 pattern matrix / witness search、完整 `analyze_expr` 分层重写、did-you-mean/previous declaration note/warning-help 诊断增强、以及 2000 generic instance / 2M AST 节点的跨机器 perf 红线。这些已从当前爆点中解耦，但仍属于 M2.1 后续架构和 perf lane 工作。
...
7. Identifier interner 和 typed lookup key。
   ...
   - 已完成第一阶段：enum case lookup 使用 session `IdentifierInterner` + typed `{TypeHandle, IdentId}` key，失败查找不再分配或拼接临时字符串；完整 module/qualified/member key 迁移继续后置。
8. Sema 不再复制整棵 AST。
