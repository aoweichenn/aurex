...
当前下一步应进入 **M14 Advanced Dyn Post-M13 Design Baseline**。M14 起点不应直接实现标准库、owning dyn、
`Box<dyn Trait>`、allocator 或 dynamic Drop dispatch；应先从 M13 后剩余 advanced dyn 候选中重新做 policy
selection，明确下一条主线、非目标、query gate、ABI/runtime 边界和测试计划。M13 已把 borrowed dyn、
supertrait upcast、principal-set composition、direct composition dispatch 和显式 composition-to-supertrait
projection 收口；下一步不要在没有设计基线时把 owning/runtime/stdlib 能力混进当前 borrowed view metadata。

M12 后续候选不应混在同一阶段一次性实现：

- borrowed composition-to-supertrait policy：需要决定是否允许 direct call 穿过 `dyn (Child + Debug)` 到
  `Parent`，以及是否要求显式 projection syntax、query facts 和 ambiguity diagnostics。
...
| M14 design baseline | post-M13 advanced dyn 候选重新评估，明确下一条主线、query gate、ABI/runtime non-goals、测试计划；不直接实现标准库或 owning dyn | 500-900 行 |
