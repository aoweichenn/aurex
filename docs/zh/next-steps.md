...
| perf baseline benchmark harness / stress gate | 已接入 baseline、轻量 CI gate、release gate、Release+LTO gate 和跨机器校准机制 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema、AST bulk lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；`tools/generic_stress.py`、`tools/ast_stress.py` 和 `tools/diagnostic_stress.py` 统一通过 `tools/perf_thresholds.py` 支持 `--threshold-profile` / `--threshold-scale`，JSON 同时记录 raw thresholds、effective thresholds、profile、scale、machine info 和 Release/LTO build options；`make perf-stress-threshold` 固化 100/200 generic + 1000/5000 AST + 100/500 errors 的 mixed-feature 轻量阈值质量门，GitHub Actions `stress-thresholds` job 覆盖三条 lane 并标记 `github-ubuntu-24.04-fedora` profile；`make perf-release-threshold` 固化 5000 generic、2M AST statements、5000 errors 的 release lane；`make perf-release-lto-threshold` 使用独立 `build-perf-lto` 跑同规格 Release+LTO lane；三条 stress 生成器默认 `--shape=mixed`，generic 覆盖 generic constraints / impl / pointer alias / tuple-pair / slice-pattern，AST 覆盖 extern、type alias、struct/enum、impl/method、generic、tuple、slice、pattern、try/defer/unsafe/loop，diagnostic 覆盖多类 sema 错误；compact AST + AST 原生 `IdentId` + 删除旧胖 `ExprNode` 生产路径 + bump-backed AST header/payload vectors 后，本机 100000 mixed AST bulk statements 约 96.3 MiB RSS / 77.9 ms；表达式 arena 现在在 parser 初始化时按 token 形态 reserve 每类 backing storage + page pre-touch，表达式节点创建走字段级 emplace，不再先落大 payload 临时，也不再靠 vector 热路径自动扩容；lexer token arena 用 token-count estimator 一次性 reserve 且不预触未写入估算余量页，避免大源文件 bump vector 过量预留或扩容留下旧 backing storage；token 本体已压缩为 pointer + range，import/module parse 完成后释放 token arena；sema 持久 value-payload list 和 generic constraint bucket 已接入 bump arena；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr；`tools/frontend_compare.py` 本机 baseline 中 Aurex lookup/96 约 10.1 ms、generics/96 约 9.6 ms，Clang++ 分别约 21.2 ms / 24.3 ms，G++ 分别约 25.1 ms / 24.3 ms；当前 mixed release lane 本机 5000 generic 约 450.9 MiB / 30275.8 ms，2M AST statements 约 1465.3 MiB / 1196.7 ms，5000 errors 约 33.1 MiB / 103.7 ms；Release+LTO lane 本机 5000 generic 约 450.6 MiB / 18144.0 ms，2M AST statements 约 1466.9 MiB / 950.8 ms，5000 errors 约 34.3 MiB / 70.5 ms；`--check` 路径不再保留泛型实例 side table；`--emit=typed` 可单独压测 retained side table | 后续只保留新增机器 profile 数据，不再缺 2M/5000/5000 release gate、Release+LTO gate、diagnostic stress 或校准机制 |

当前增量编译缓存文件格式和 driver read/write/reuse 路径已完成：`--incremental-cache <path>` 会写入 schema/version、root/import path、source content fingerprint、module row 和 checked stable/incremental def row；`--check` 在 root、import path 和所有 source fingerprint 全部匹配时直接命中返回，不再重新 parse/sema。当前仍不应声称已经完成的内容：LSP 结构化诊断 protocol，以及 M2.5 query / lossless syntax / IDE-native 细粒度增量架构。跨阶段 stable identifier hash / parallel global ID / incremental fingerprint、module alias/field/enum case suggestion、多行 span / 颜色输出、CLI JSON diagnostics、跨机器 perf threshold profile/scale 校准机制，以及 2M AST 节点、5000 generic、5000 errors 的普通 Release 与 Release+LTO gate 已落地。
...
8. Sema 不再复制整棵 AST。
   ...
   - 验收：2M mixed AST statements RSS 当前约 1465.3 MiB（Release）/ 1466.9 MiB（Release+LTO），parser token buffer 已压缩为 pointer + range，ModuleLoader parse 完成后释放 token arena。若继续收紧阈值，后续优先评估 streaming parser 或进一步减少 source/token/AST 同峰持有。
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   ...
   - 本机 `tools/ast_stress.py --skip-build --counts 100000 --shape mixed` baseline 中，100000 mixed AST bulk statements 约为 96.3 MiB RSS / 77.9 ms，较 compact 前约 575 MiB RSS / 135 ms 明显收敛；当前 release 2M mixed AST statements 约 1465.3 MiB RSS / 1196.7 ms，Release+LTO 约 1466.9 MiB RSS / 950.8 ms；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr。
   - release gate 已覆盖 2M AST statements、5000 generic 和 5000 diagnostic errors，Release+LTO gate 使用独立 `build-perf-lto` 覆盖同规格场景；`AUREX_PERF_THRESHOLD_PROFILE` / `AUREX_PERF_THRESHOLD_SCALE` 已作为跨机器阈值校准入口，并随 stress JSON 记录 raw/effective thresholds、机器信息和 build options。
...
2. 建立 stress/perf 测试分层。
   - 常规单测只跑轻量 baseline case；`make perf-stress-threshold` / CI `stress-thresholds` 跑 100/200 mixed 泛型实例、1000/5000 mixed AST bulk 和 100/500 mixed diagnostic errors 的阈值门；`make perf-release-threshold` 跑 5000 mixed 泛型实例、2M mixed AST statements 和 5000 mixed errors；`make perf-release-lto-threshold` 用 Release+LTO 跑同规格场景，`make perf-release-all-threshold` 串行跑普通 Release 和 Release+LTO。
...
| perf | 500/1000/2000/5000 mixed 泛型实例 RSS/耗时；10000/50000/100000 mixed AST bulk RSS/耗时；2M mixed AST 节点 RSS；5000 mixed errors RSS/耗时；Release+LTO 同规格 gate |
