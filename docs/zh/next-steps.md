...
generic/opaque Drop ABI 和标准库拥有型资源封装继续后移。
...
M7d-F 之后本阶段不要把标准库作为工程入口。最直接的 compiler-only 候选是把 generic/opaque cleanup marker 的
ABI 策略正式化，或者继续补 index/slice place-state gap；`Drop` bound、generic Drop surface、trait-object dispatch
和标准库资源 API 都应等 ABI 与 capability 设计稳定后再作为独立阶段推进。
