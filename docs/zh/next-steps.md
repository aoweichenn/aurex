...
| identifier 临时 string | 已关闭当前主热路径 | syntax 层 `IdentifierInterner` 使用 global bump allocator 保存稳定文本；AST name-bearing 字段携带原生 `IdentId`；`SymbolTable` scope lookup、函数/类型/值/generic/enum case/method/member lookup 均使用 typed `IdentId` key；sema typed lookup key 复用 `AstModule` identifier arena，不再维护第二套私有 interner，也不再在 lookup miss 时回扫 string map；`FunctionSignature`、`Symbol`、`StructInfo`、`EnumCaseInfo`、`TypeAliasInfo`、`TypeInfo` 和 generic template 的持久文本字段改为 `InternedText`，文本进入 bump-backed interner，复制时重新 intern 到目标 arena，不再复制堆 `std::string` buffer | 跨模块 stable identifier hash、并行全局 ID 和 delayed display string 仍后置 |
...
| perf baseline benchmark harness | 基础线已接入，跨前端对比通道和泛型/AST RSS 压测入口已接入 | 增加基于 Google Benchmark 的 `aurex_frontend_bench`，覆盖 lexer mixed、lookup-heavy lex→parse→sema、generic-instantiation-heavy lex→parse→sema、AST bulk lex→parse→sema；`tools/perf_report.py` / `make perf` 解析 `--benchmark_format=json` 并输出轻量 baseline；新增 `aurex_frontend_compare_bench` 和 `tools/frontend_compare.py`，用 Google Benchmark real_time 对比 Aurex `--check`、Clang/G++ `-fsyntax-only`、rustc stable metadata emission 的 lookup/generic frontend/check 基线；新增 `tools/generic_stress.py` 和 `tools/ast_stress.py`，`make perf-stress` 固化 500/1000/2000/5000 泛型实例；`tools/generic_stress.py --shape=templates` 覆盖大量不同泛型模板和 10000/50000/100000 AST bulk statements 的 elapsed time + peak RSS 基线；compact AST + AST 原生 `IdentId` + 删除旧胖 `ExprNode` 生产路径 + bump-backed AST header/payload vectors 后，本机 100000 AST bulk statements 约 140.8 MiB RSS / 72.4 ms；表达式 arena 现在在 parser 初始化时按 token 形态 reserve 每类 backing storage + page pre-touch，表达式节点创建走字段级 emplace，不再先落大 payload 临时，也不再靠 vector 热路径自动扩容；lexer token arena 一次性 reserve 且不预触未写入估算余量页，避免大源文件 bump vector 扩容留下旧 backing storage；sema 持久 value-payload list 和 generic constraint bucket 已接入 bump arena；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr；`tools/frontend_compare.py` 本机 baseline 中 Aurex lookup/96 约 10.1 ms、generics/96 约 9.6 ms，Clang++ 分别约 21.2 ms / 24.3 ms，G++ 分别约 25.1 ms / 24.3 ms；2000 generic instance stress 约 148.4 MiB RSS / 439.8 ms，仍低于 M2.1 约 150 MiB 目标；`--check` 路径不再保留泛型实例 side table；`--emit=typed` 可单独压测 retained side table；暂不强制阈值 | 2M AST 节点跨机器稳定阈值、generic/AST stress 阈值、跨模块 stable hash 和 CI perf lane 仍后置 |

当前仍不应声称已经完成的内容：跨阶段 stable identifier hash / parallel global ID、完整 `analyze_expr` 分层重写、did-you-mean/previous declaration note/warning-help 诊断增强、以及 2M AST 节点的跨机器 RSS/耗时阈值和 CI perf lane。这些已从当前爆点中解耦，并已有 generic/AST stress baseline 入口，但仍属于 M2.1 后续架构和 perf lane 工作。
