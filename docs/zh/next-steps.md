...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   ...
   - release gate 已覆盖 2M AST statements、5000 generic 和 5000 diagnostic errors；`AUREX_PERF_THRESHOLD_PROFILE` / `AUREX_PERF_THRESHOLD_SCALE` 已作为跨机器阈值校准入口，并随 stress JSON 记录 raw/effective thresholds 和机器信息。
