...
| record / enum case lookup 索引 | 已关闭当前退化路径 | struct/enum lookup 命中后做 type identity 校验；enum case 索引 miss 不再静默全表扫描通过；method/member、enum case 和 associated function 名字查找已统一走 `IdentId` typed key；struct field、enum case、method/function、const/type alias 均写入 stable member/def/incremental key | 后续只保留增量编译文件格式和跨进程缓存落盘 |
...
8. Sema 不再复制整棵 AST。
   ...
   - 验收：2M AST statements RSS 从约 2.4 GiB 降到约 1.7 GiB；剩余大头是当前 parser 仍需在 parse 峰值同时持有完整 token buffer 与 compact AST，后续若继续收紧阈值应优先做 token text/range 压缩或 streaming parser。
...
12. Compact AST 和 bump allocator 作为 M2.1 后半段优化。
   ...
   - 本机 `tools/ast_stress.py --skip-build --counts 10000,50000,100000` baseline 中，100000 AST bulk statements 约为 140.8 MiB RSS / 72.4 ms，较 compact 前约 575 MiB RSS / 135 ms 明显收敛；当前 release 2M AST statements 约 1687.0 MiB RSS / 1123.5 ms；Google Benchmark `sema_ast_bulk/1024` 当前约 128 ns/expr。
