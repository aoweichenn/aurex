...
`src/driver/incremental_cache_format.hpp` 也已经把 incremental cache 的 wire format、record 结构和
query kind 表独立成内部格式层。`module_loader.cpp` 现在只做文件读取、import 解析、模块校验和递归编排；
`incremental_cache.cpp` 则保留 query 采样、reuse、pruning 与写盘调度。相关 driver / query / sample
回归与 `tools/check_coverage.sh` 已重新跑过，source totals lines/regions/functions 覆盖率仍保持在 95% 以上。
