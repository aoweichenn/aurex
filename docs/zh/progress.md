...
parser dry-run execution、AST mutation 或 macro-generated user code lowering；它只把 item-level macro call-site、
matcher-to-call binding 和 user derive target schema 固定为可验证 admission facts。

新增 `ItemKind::macro_call`、`MacroCallItemPayload`、`ItemNode::macro_call_tokens`、
`ItemNode::macro_call_range` 和 `ItemNode::macro_call_balanced`。Parser 现在支持 `macro call Name { ... }`，
把调用体保存为 flat token tree；AST dump 输出 `macro_call ... call_tokens=... balanced=yes/no` 和
`macro_call_body`；tooling session 将该 item 投影为 `macro_call`，IDE symbol metadata 不把 call-site 当成 stable
definition。`expand_early_item_macros_noop()` 会为每个 macro call 生成
`AurexMacroCallSiteAdmissionGate`，并在同模块同名 macro surface 存在时生成
`AurexMacroMatcherToCallBindingAdmissionGate`；binding 当前只连接第一个 recognized typed matcher，不执行 matcher。
如果 `#[derive(Name)]` 匹配同模块 `macro derive Name { ... }`，会生成
`AurexUserDeriveTargetSchemaAdmissionGate`，记录 target kind、struct field count、enum case count 和 enum payload
count。

Summary / dump / fingerprint 新增 `aurex_macro_call_site_source_items`、
`aurex_macro_call_site_admissions`、`aurex_macro_call_site_targets_declared`、
`aurex_macro_call_site_balanced`、`aurex_macro_call_site_expansion_enabled`、
`aurex_macro_matcher_to_call_bindings`、`aurex_macro_matcher_to_call_bindings_admitted`、
`aurex_user_derive_target_schema_source_derives`、`aurex_user_derive_target_schemas`、
`aurex_user_derive_target_schema_structs`、`aurex_user_derive_target_schema_enums`、
`aurex_user_derive_target_schema_fields`、`aurex_user_derive_target_schema_enum_cases` 和
`aurex_user_derive_target_schema_enum_payloads`。Validation 会拒绝 macro call source item / gate 数不匹配、
已声明目标的 call-site 缺少 binding、binding 串错 call identity / surface identity / matcher identity、user
derive schema 源计数与 gate 数不匹配、schema 串错 derive surface、identity/query 漂移、parser consumption、AST
mutation、standard library、runtime、external process 或 user generated code 被打开。macro call-site expansion is
admission-only in M27c；matcher-to-call binding execution is admission-only in M27c；user derive target schema is
admission-only in M27c；仍不展开宏/不执行用户编译期代码/不消费 parser/不修改 AST。
稳定 blocker 文案：macro call-site expansion is admission-only in M27c；matcher-to-call binding execution is admission-only in M27c；user derive target schema is admission-only in M27c。

Query 层新增 `aurex_macro_call_site_admission`、
`aurex_macro_matcher_to_call_binding_admission` 和 `aurex_user_derive_target_schema_admission` 三类
`MacroExpansionFactKind`，对应 `aurex_macro_call_site_admission_v1`、
`aurex_macro_matcher_to_call_binding_admission_v1` 和 `aurex_user_derive_target_schema_admission_v1` policy，并新增
`m27c_macro_expansion_plan_baseline()` 与 `is_valid_m27c_macro_expansion_plan()`。M27c plan 在 M27b 十三个 facts 上叠加
三类 call-site / matcher binding / user derive target schema facts，共 16 个 facts；默认 early expansion 仍兼容
M21c baseline，不替换 M21-M26 builtin derive 链条。新增说明见
[Aurex M27 Aurex Macro Surface Admission](m27-aurex-macro-surface-admission.md)。
...
resolution、真实 expansion source map、debug trace CLI、`--emit-expanded`、generated source text、generated module
part parse / merge、declared generated names lookup、generated item visibility / export、parser-consumable generated
token buffer、parser consumption execution、real parser dry-run execution 或 macro-generated user code lowering；它把
M23c pre-consumption verification closure 之后的 controlled parser dry-run adapter、rollback diagnostic replay 和
dry-run negative matrix closure 固定成可验证 facts。

新增 `BuiltinDeriveControlledParserDryRunAdapter`、
`BuiltinDeriveDryRunRollbackDiagnosticReplay` 和 `BuiltinDeriveDryRunNegativeMatrixClosure`，并给
`EarlyItemExpansionResult` 增加 `builtin_derive_controlled_dry_run_adapters`、
`builtin_derive_dry_run_rollback_replays` 和 `builtin_derive_dry_run_negative_matrices`。M24a controlled dry-run
adapter 绑定 M23c `verification_closure_identity`、M23a `admission_protocol_identity` 和 M23b
`checkpoint_protocol_identity`，固定 `builtin_derive_controlled_parser_dry_run_adapter_v1`、
`m24a-builtin-derive-controlled-parser-dry-run:<module>:<part>`、token record / diagnostic anchor counts、
`prerequisite_count=5` 和 execution-blocked blocker。M24b rollback replay 绑定 M24a
`dry_run_adapter_identity`、M23b `checkpoint_protocol_identity` 和 M22f `rollback_gate_identity`，固定
`builtin_derive_dry_run_rollback_diagnostic_replay_v1`、
`m24b-builtin-derive-dry-run-rollback-replay:<module>:<part>`、diagnostic anchor / report entry / planned replay /
executed replay counts 和 replay execution blocker。M24c negative matrix 绑定 M24a `dry_run_adapter_identity`、
M24b `replay_protocol_identity` 和 M23c `verification_closure_identity`，固定
`builtin_derive_dry_run_negative_matrix_closure_v1`、
`m24c-builtin-derive-dry-run-negative-matrix:<module>:<part>`、dry-run adapter / rollback replay / verification
closure counts、`negative_case_count=8`、`parser_consumable_case_count=0`，并把 `EarlyItemExpansionResult::name`
推进为 `M24c Builtin Derive Dry-Run Negative Matrix Closure`。summary / dump / fingerprint / validation 会拒绝
M24 identity 或 query 漂移、M24 count 漂移、上游 M22/M23/M24 identity 串线、dry-run execution / replay execution /
parser admission / parser consumption 被打开、generated part parsed / merged / sema-visible 被打开、standard
library/runtime/external process requirement 被打开、emit/debug/source-map/user-code flag 被打开。新增说明见
[Aurex M24a Builtin Derive Controlled Parser Dry-Run Adapter](m24a-builtin-derive-controlled-parser-dry-run-adapter.md)、
[Aurex M24b Builtin Derive Dry-Run Rollback Diagnostic Replay](m24b-builtin-derive-dry-run-rollback-diagnostic-replay.md) 和
...
LLVM global emission，以及 verifier 对 metadata policy、concrete type、principal-set object type、witness coverage、
canonical principal index 和 projection target 的检查。Runtime representation 仍是 borrowed two-pointer view：
composition view 是 `{data*, principal_set_metadata*}`，projection 后的 single dyn view 是 `{data*, vtable*}`。
Metadata global shape 为 `{ [N x ptr] }`，每个 entry 指向对应 principal 的 checked vtable witness，不携带 owner、
allocator、drop、size 或 align metadata。
...
M11a 是 design/query gate baseline；M11b 是 compiler/query facts prototype gate；M11c 是 frontend/sema
check-only gate。当前稳定可用的用户语言能力包含 M8 borrowed dyn trait、M9 dyn ABI/tooling facts、M10
borrowed dyn supertrait upcast，以及 M11c borrowed principal-set composition type annotation/coercion 的
check-only 子集；
...
`dump_principal_set_composition_facts()`。M11c 新增的是用户可写 `dyn (A + B)` borrowed composition spelling、
AST/type identity、borrowed concrete-to-composition coercion check、principal-set associated equality merge
diagnostics、method-call guard、checked dump/fingerprint、`TypeCheckBodyAuthority` principal-set facts 和
frontend/query/IR negative tests。
...
`sliceptr`、`slicelen`、`str.ptr`、`str.len`、`strvalid`、`strfromutf8` 和 `strraw` 已通过泛型正样例覆盖
