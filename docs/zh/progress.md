...
当前开发线先切到架构与耦合重构。新语言表面继续保留在计划中，但第一优先级是把已经落地的泛型、for-in、closure、borrow/drop 和 IR lowering 路径整理成更低耦合、更容易验证、更适合后续扩展的结构。目录/文件组织、测试代码组织和 CMake/测试脚本拆分也属于第一优先级，不再把超大文件和万能测试入口当作可接受状态。
