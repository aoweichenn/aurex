...
- `aurex-profile-v1` 的 driver 主阶段 phase 附带可选 `stage` 元数据对象，包含 stage id、input、output、
  ...
  `PipelineStageId` / `PipelineProfileSubeventId` 进入 profiler，避免后续 IDE/LSP 阶段视图重新维护诊断阶段表。
