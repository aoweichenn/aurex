...
四个内部域，`io` 又继续拆出 fingerprint、reader、writer 三个内部域，避免 query subject 构造、
cache parser / writer 和 source fingerprint 继续集中在大文件里。`module_loader.cpp` 现在只做文件读取、import 解析、模块校验和递归编排；
`incremental_cache.cpp` 则降为 public facade，不再持有协议、I/O、query subject 生成、reuse plan
或 profile 明细。相关 cache/query/incremental gtest、完整 ctest、改动 C++ 文件格式检查和
`tools/check_coverage.sh` 已经重新跑过，source totals lines/functions/regions 覆盖率分别为
95.22% / 97.84% / 95.17%，`aurexc` entrypoint 三项为 100.00%。
