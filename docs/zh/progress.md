...
2026-05-17 正则性能/测试线补上 `RegexSet` exact-literal prefix trie：纯字面量 set 不再逐 pattern 复制公共前缀 NFA，而是构建临时 trie 后输出普通 set `State` program，保持 text/bytes、重复 literal、pattern id、all-span callback 和 database roundtrip 语义兼容；同时新增 `tools/regex_differential.py`，用 Python `re` safe subset 做差分，并从 Unicode 17.0 `CaseFolding.txt` / `GraphemeBreakTest.txt` 生成 full case-fold 与 UAX #29 `\X` conformance 程序，作为 CTest slow conformance 入口。
