...
当前实现基线是 M6 Resource And Access Semantics。M6-WP2 到 M6-WP7 已落地资源分类、whole-local move
...
- dynamic trait object、object safety、vtable ABI、associated const、default associated type、generic associated type、
  specialization、minimal implementation annotation、完整 borrow checker、partial move、region、用户 destructor
  syntax、custom destructor lowering、aggregate rollback codegen、async drop 和标准库重建。
