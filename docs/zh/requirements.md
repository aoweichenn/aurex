...
- `let` / `var`、pattern、let-else、block expression、if/match expression、`while`、C-style `for`、`for i in range(...)`、一等 `range(...)` value、array/slice value for-in、str byte for-in、用户 protocol iterator for-in、`break`、`continue`、`defer`、`?`。
...
- mutable/reference item iteration、range literal、标准库 iterable adapter、Unicode scalar / char iteration adapter。
