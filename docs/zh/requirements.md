...
当前实现基线是 M14 Borrowed Dyn View Path Inference / Dispatch Release。M6-WP2 到 M6-WP7 已落地资源分类、whole-local move
...
- M11a/M11b advanced dyn design/query facts：`m11a_dyn_advanced_design_gate_baseline`、
  ...
  M13c 已支持 `dynproject[Child, Parent](view)` 这类 borrowed composition-to-supertrait 显式投影 runtime lowering；M14 已支持唯一 path 的 `let parent: &dyn Parent = view;` 和 `view.parent()`。
...
- owning dyn、`Box<dyn Trait>`、allocator、trait-object Drop dispatch、歧义 composition-to-supertrait 自动选择、bare `dyn A + B` parser syntax、associated const、default associated type、generic associated type、
