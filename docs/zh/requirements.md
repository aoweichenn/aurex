...
- `let` / `var`、pattern、let-else、block expression、if/match expression、`while`、C-style `for`、`for i in range(...)`、一等 `range(...)` value、array/slice value for-in、用户 protocol iterator for-in、`break`、`continue`、`defer`、`?`。
...
- mutable/reference item iteration、str iteration、range literal、标准库 iterable adapter。
