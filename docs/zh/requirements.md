...
当前文档基线是 M4。较早的 M2 `language-core-no-std` 阶段用于隔离语言核心验证：
...
- nominal static trait、显式 `impl Trait for Type`、static trait method dispatch 和第一版 associated type model。
...
- dynamic trait object、object safety、vtable ABI、associated const、generic associated type、default method、specialization 和资源语义。
