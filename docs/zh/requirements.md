...
当前实现基线是 M16 Const Generic Frontend / Query / Sema Check-Only。M6-WP2 到 M6-WP7 已落地资源分类、whole-local move
...
- owning dyn、`Box<dyn Trait>`、allocator、trait-object Drop dispatch、歧义 composition-to-supertrait 自动选择、
  bare `dyn A + B` parser syntax、generic const arithmetic、const where
