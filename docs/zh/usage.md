...
本文描述当前 **M15 Advanced Dyn Ownership / Const Generic Boundary Design Baseline** 下已经落地的用法。标准库仍保持冻结并移除，所有示例和测试都应围绕语言语法、语义、IR、后端、static trait、borrowed dyn trait、borrowed dyn supertrait upcast 和 borrowed dyn composition 表面本身展开。
...
gate，用于固定 owning dyn/runtime boundary 与 const generic 后续实现路线。当前仍不实现 bare `dyn A + B`、
标准库、owning dyn、`Box<dyn Trait>`、allocator、dynamic Drop dispatch、歧义 composition-to-supertrait 多步自动选择或
用户可写 const generic 语法。
...
generic associated type、associated const 或用户可写 const generic。

## Const Generic 状态

M15 只固定 const generic 的后续实现边界，不打开用户语法。当前可以继续写普通 type generic 和 origin generic：
...
M15 design gate 已经选择后续路线：typed scalar const parameter、canonical const value key、generic instance
const arg key 和 `[N]T` array length integration。下一阶段实现时必须让 `Array[i32, 4]` 与 `Array[i32, 5]`
成为不同 generic instance，不能把 const argument 只拼进 display name，也不能复用 type parameter 的
`TypeHandle` placeholder。generic const arithmetic、`N + 1`、const where predicate、const associated value、
dyn const equality 和 runtime/standard-library const generic 都继续后移。
