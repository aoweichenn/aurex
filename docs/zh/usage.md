...
本文描述当前 **M5 default trait methods release baseline** 的用法。标准库仍保持冻结并移除，所有示例和测试都应围绕语言语法、语义、IR、后端和 static trait 表面本身展开。
...
`aurex/tooling/ide.hpp`，会复用这条 lossless/query 主线暴露 diagnostics、hover、definition、
...
M5 支持 nominal static trait、显式 impl、generic trait predicate、static trait method call、associated-type
equality constraint，以及 trait 内部的 default method body：
...
default method body 会在 trait context 中检查；被选中的 inherited default 会在单态化后 lowering 为 internal
direct-call function。显式 impl method 在替换后的签名匹配时仍是 override。这仍然是 static-dispatch 表面。
dynamic trait object、vtable ABI、object safety、specialization、default associated type、generic associated
type、associated const 和 RAII/resource semantics 仍是未来设计流。
