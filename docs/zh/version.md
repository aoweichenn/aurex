...
- M1 axbuild target graph diagnostic/message baseline，包括 `GraphDiagnostic`、status、target index、related index、message，以及 duplicate / invalid dependency / cycle back-edge 定位。
...
- M1 axbuild 从 source/stamp mtime、source discovery、target name lookup、target graph smoke 和结构化 graph diagnostic/message 推进到完整目录项列表、递归遍历、完整 cycle path diagnostic、cwd/env、stderr capture 和结构化输出报告。
- M3 之后以新语言特性重新设计自举链路。
