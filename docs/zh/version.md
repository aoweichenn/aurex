...
- 当前语言路线以泛型和语法修正为主。
- 闭包 capture-list、一等 `range(...)` value、array/slice for-in 和 protocol iterator for-in 已经进入当前语言边界。
...
- `for item in expr` 当前支持 range value、array/slice 按值迭代和用户 protocol iterator。array/slice 元素类型必须满足 `Copy`；protocol iterator 的 `next()` item 不要求 `Copy`。
