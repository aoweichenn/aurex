...
- M1 axbuild 从 source/stamp mtime、source discovery、target name lookup 和 target graph smoke 推进到完整目录项列表、递归遍历、cycle path diagnostic、cwd/env、stderr capture 和结构化输出报告。
- M3 之后以新语言特性重新设计自举链路。
