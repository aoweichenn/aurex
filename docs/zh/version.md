...
- M1 axbuild 从 source/stamp mtime、source discovery、target name lookup、target graph smoke 和结构化 graph diagnostic/message/name/cycle-path 推进到完整目录项列表、递归遍历、cwd/env、stderr capture、dependency value diagnostic 和结构化输出报告。
- M3 之后以新语言特性重新设计自举链路。
