...
当前仍未完成的 RAII runtime 闭环：backend 还没有把 custom destructor step lowering 成实际函数调用；
LLVM backend 中 `drop` / `drop_if` 仍是 cleanup marker。因此需要真实运行时副作用的资源库仍要显式调用释放
API 或配合 `defer`。用户可写 `Drop` bound、generic Drop impl、trait-object Drop dispatch、async/unwind-aware
drop、panic cleanup ABI 和标准库拥有型资源封装继续作为后续设计项。
...
当前仍未完成的 M7d-B 设计项：tuple `.0` source surface 仍由 parser 拒绝，因此 tuple partial move 未落地；
indexed move-out、consuming pattern payload、non-`Copy` `?` payload transfer 仍保守拒绝；`replace` / `take` /
...
或用户级析构器语法。M7d-C 后续已补上窄 `impl Drop` / `deinit` semantic surface，但 backend custom destructor
call lowering 仍未完成。
M7d-B 后续已补上本地 owned struct field partial move/reinit/drop flag 子集，但 tuple/index partial move 和
replace/take/swap 仍不属于当前已完成范围。
...
`impl Drop` / `deinit` semantic surface；backend custom destructor call lowering 仍是未来项。
...
字段级 drop flag；M7d-C 后续已开放窄 `impl Drop` / `deinit` semantic surface。indexed move-out、tuple `.0`
partial move、consuming pattern payload 和 non-`Copy` `?` payload transfer 仍然拒绝；backend custom
destructor call lowering 继续后移。
