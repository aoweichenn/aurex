...
- 当前只记录 checked facts / query DTO / ABI descriptor，不生成 `trait_object_upcast` IR，也不生成
  `supertrait_vptr_metadata_v1` LLVM vtable global。
- 直接在 `dyn Child` receiver 上调用 inherited parent method 还没有作为 runtime-ready dispatch 接入；应通过
  target parent view 或留到 M10c/M10d。
...
下一步是 **M10c Supertrait IR / Backend Runtime Implementation**：实现 `trait_object_upcast` IR、
`VTableSupertraitEdgeDescriptor`、IR verifier negative matrix、LLVM `supertrait_vptr_metadata_v1` vtable global 和
runtime parent vtable projection。标准库仍不属于 M10c。
...
当前下一步是 M10c IR/backend runtime；标准库仍不属于本阶段。
