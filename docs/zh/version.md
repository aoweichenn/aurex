...
- `std.core.map` Vec-backed `noncopy` 泛型 `Map<K, V>` 和 borrowed C string -> usize 的 `noncopy` `CStringUsizeMap` baseline；当前按值 `Map<K,V>` key/value API 已有 copyable key/value 语义护栏。
- 字符串基础类型方向已冻结为 `str` = borrowed UTF-8 text slice、`String` = owned UTF-8 buffer、`Bytes` / `Span<u8>` = raw bytes、`CStr` / `CString` = C FFI、`Path` = platform path bytes；当前已落地字符串字面量 UTF-8/escape 诊断、`std.core.str` borrowed API 和 scalar API、`std.core.string.String` 的 `from_str/from_utf8/as_str/append(str)/push_scalar/insert_scalar/pop_scalar/remove_scalar_at/slice_bytes_checked/truncate_bytes_checked` UTF-8 surface、`String.as_mut_span` 移除、`std.core.bytes.Bytes` raw bytes 容器、bytes-backed `std.fs.path.Path`、`std.fs.file` 和 `std.fs.dir` 的 `Path` / `str` 新入口、bytes-backed `DirectoryEntry` raw bytes / checked UTF-8 视图，以及 `std.ffi.c.string.CStr` borrowed 视图 / `CString` noncopy 拥有型 FFI 边界类型；`String`、`Bytes`、`Path`、`DirectoryEntry`、`Vec<T>`、`Map<K,V>` 和 `CStringUsizeMap` 作为拥有型资源均已进入 noncopy 所有权边界，`Vec`、`Map`、`Result` / `Option` 的部分 copy-only API 已有语义诊断护栏；析构 MVP 已识别 `destroy(self: *mut T) -> void`，并提供 `Vec<T>.destroy_deep/clear_deep/truncate_deep` 给拥有型元素做显式深销毁。
...
- M1 axbuild `Path` / owned container 风险收口 baseline，包括 `Target.sources: Vec<Path>`、`Project.stamp_path: Path`、source/stamp metadata、stamp 写入、clean、目录扫描和临时 source cleanup 的 `Path` / `str` 入口，以及 `Project.add_target(&target)` 通过 `Vec.take()` / field reset 转移 owned 字段。
