...
inline constexpr std::string_view DRIVER_OUTPUT_OPEN_FAILED =
    "failed to open output file";

inline constexpr std::string_view DRIVER_OUTPUT_WRITE_FAILED =
    "failed to write output file";

inline constexpr std::string_view DRIVER_NATIVE_OUTPUT_REQUIRES_OUTPUT_PATH =
...
inline constexpr std::string_view DRIVER_INPUT_READ_FAILED =
...
inline constexpr std::string_view DRIVER_CLANG_FORK_FAILED =
    "failed to fork clang process";

inline constexpr std::string_view DRIVER_CLANG_WAIT_FAILED =
    "failed to wait for clang process";

inline constexpr std::string_view DRIVER_CLANG_INVOCATION_FAILED =
    "clang invocation failed";

inline constexpr std::string_view DRIVER_MODULE_LOADING_FAILED =
    "module loading failed";

inline constexpr std::string_view DRIVER_IMPORT_DEPTH_EXCEEDED =
...
[[nodiscard]] inline std::string driver_module_import_mismatch_message(
    ...
    return "module declaration '" + std::string(declared) +
           "' does not match import '" + std::string(expected) + "'";
}

[[nodiscard]] inline std::string driver_duplicate_module_name_message(
    const std::string_view module_name,
    const std::string_view loaded_path
) {
    return "duplicate module name '" + std::string(module_name) +
           "' already loaded from " + std::string(loaded_path);
}
...
[[nodiscard]] inline std::string driver_import_resolve_failed_message(
    ...
    return "failed to resolve import: " + std::string(module_name) +
           " (searched: " + std::string(candidates) + ")";
}
...
inline constexpr std::string_view DRIVER_MULTIPLE_INPUT_FILES_UNSUPPORTED =
    "multiple input files are not supported";

inline constexpr std::string_view DRIVER_ERROR_PREFIX =
    "aurexc: ";
