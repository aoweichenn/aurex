...
inline constexpr std::string_view SYNTAX_TYPE_PAYLOAD_ID_CONTEXT = "syntax type payload id";
inline constexpr std::string_view SYNTAX_TYPE_NODE_ID_CONTEXT = "syntax type node id";
...
struct ReferenceTypePayload {
    PointerMutability mutability = PointerMutability::const_;
    TypeId pointee = INVALID_TYPE_ID;
    AstArenaVector<std::string_view> origin_names;
    AstArenaVector<IdentId> origin_name_ids;
    AstArenaVector<base::SourceRange> origin_ranges;
    base::SourceRange origin_range{};
...
struct TypeNodePayloadArena {
    ...
    explicit TypeNodePayloadArena(base::BumpAllocator& arena)
        : primitives(base::BumpAllocatorAdapter<PrimitiveTypeKind>{arena}),
          ...
          references(base::BumpAllocatorAdapter<ReferenceTypePayload>{arena}),
    ...
    AstArenaVector<ReferenceTypePayload> references;
...
private:
    ...
    {
        const base::u32 index = base::checked_u32(payloads.size(), SYNTAX_TYPE_PAYLOAD_ID_CONTEXT);
