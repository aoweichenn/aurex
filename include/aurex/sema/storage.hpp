...
[[nodiscard]] SemaMap<Key, Value, Hash, KeyEqual> make_sema_map(
    base::BumpAllocator& arena, Hash hash = Hash{}, KeyEqual equal = KeyEqual{})
{
    return SemaMap<Key, Value, Hash, KeyEqual>(
        0, std::move(hash), std::move(equal), base::BumpAllocatorAdapter<std::pair<const Key, Value>>{arena});
}
...
[[nodiscard]] SemaSet<Key, Hash, KeyEqual> make_sema_set(
    base::BumpAllocator& arena, Hash hash = Hash{}, KeyEqual equal = KeyEqual{})
{
    return SemaSet<Key, Hash, KeyEqual>(0, std::move(hash), std::move(equal), base::BumpAllocatorAdapter<Key>{arena});
}
