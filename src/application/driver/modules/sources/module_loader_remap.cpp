...
#include <application/driver/modules/private/module_loader_remap.hpp>
...
void remap_item_node(syntax::ItemNode& node, const IdMap& map)
{
    ...
    node.trait_type = remap_type(node.trait_type, map);
    for (syntax::ItemId& item : node.trait_items) {
