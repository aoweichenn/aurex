...
{
    return suffix.empty() || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_I8
        || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_I16 || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_I32
        || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_I64 || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_ISIZE
        || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_U8 || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_U16
        || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_U32 || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_U64
        || suffix == LLVM_BACKEND_UTIL_INTEGER_SUFFIX_USIZE;
}

[[nodiscard]] bool float_suffix_is_valid(const std::string_view suffix) noexcept
{
    return suffix.empty() || suffix == LLVM_BACKEND_UTIL_FLOAT_SUFFIX_F32
...
[[nodiscard]] bool integer_digit_value(const char c, const int base, std::uint64_t& value) noexcept
{
    ...
    if (c >= LLVM_BACKEND_UTIL_HEX_LOWER_FIRST && c <= LLVM_BACKEND_UTIL_HEX_LOWER_LAST) {
        value = static_cast<std::uint64_t>(
            LLVM_BACKEND_UTIL_HEX_DIGIT_OFFSET + static_cast<std::uint64_t>(c - LLVM_BACKEND_UTIL_HEX_LOWER_FIRST));
        return value < static_cast<std::uint64_t>(base);
    }
    if (c >= LLVM_BACKEND_UTIL_HEX_UPPER_FIRST && c <= LLVM_BACKEND_UTIL_HEX_UPPER_LAST) {
        value = static_cast<std::uint64_t>(
            LLVM_BACKEND_UTIL_HEX_DIGIT_OFFSET + static_cast<std::uint64_t>(c - LLVM_BACKEND_UTIL_HEX_UPPER_FIRST));
        return value < static_cast<std::uint64_t>(base);
    }
    return false;
}
...
std::string decode_string_literal(const std::string_view literal, const bool has_c_prefix)
{
    base::StringLiteralDecode decoded = base::decode_string_literal(
        literal, has_c_prefix ? base::StringLiteralKind::c_string : base::StringLiteralKind::string);
    return std::move(decoded.decoded);
}

std::string decode_raw_string_literal(const std::string_view literal)
{
    base::StringLiteralDecode decoded = base::decode_string_literal(literal, base::StringLiteralKind::raw_string);
    return std::move(decoded.decoded);
}
