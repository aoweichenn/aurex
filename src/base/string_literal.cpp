...
StringLiteralDecode decode_string_literal(const std::string_view literal, const StringLiteralKind kind) {
    ...
    if (is_c_string && literal.size() >= STRING_LITERAL_C_STRING_PREFIX_LENGTH && literal[0] == STRING_LITERAL_C_PREFIX && literal[1] == STRING_LITERAL_ESCAPE_DOUBLE_QUOTE) {
        content_begin = STRING_LITERAL_C_STRING_PREFIX_LENGTH;
    } else if (!literal.empty() && literal.front() == STRING_LITERAL_ESCAPE_DOUBLE_QUOTE) {
        content_begin = 1;
    }
