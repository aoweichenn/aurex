...
#include <frontend/lex/private/keyword.hpp>
#include <frontend/lex/private/lexeme.hpp>
#include <frontend/lex/private/punctuator.hpp>
...
constexpr void mark_token_start(std::array<TokenStartAction, LEX_BYTE_CHAR_CLASS_COUNT>& table, const char c,
    const TokenStartAction action) noexcept
{
    table[char_class_index(c)] = action;
}

constexpr void mark_token_start_range(std::array<TokenStartAction, LEX_BYTE_CHAR_CLASS_COUNT>& table, const char first,
    const char last, const TokenStartAction action) noexcept
{
...
[[nodiscard]] bool estimate_two_byte_prefix(
    const std::string_view source, const base::usize index, const std::string_view prefix) noexcept
{
    return index + prefix.size() <= source.size() && source.substr(index, prefix.size()) == prefix;
}

[[nodiscard]] base::usize estimate_quoted_token_end(
    const std::string_view source, base::usize index, const char quote) noexcept
{
...
[[nodiscard]] base::usize estimate_line_comment_end(const std::string_view source, base::usize index) noexcept
{
...
[[nodiscard]] base::usize estimate_block_comment_end(const std::string_view source, base::usize index) noexcept
{
...
[[nodiscard]] base::usize estimate_identifier_token_end(const std::string_view source, base::usize index) noexcept
{
...
[[nodiscard]] base::usize estimate_number_token_end(const std::string_view source, base::usize index) noexcept
{
    while (index < source.size() && is_ident_continue(source[index])) {
        ++index;
    }
...
[[nodiscard]] bool estimate_prefixed_quote(
    const std::string_view source, const base::usize index, const std::string_view prefix) noexcept
{
    return source[index] == prefix.front() && index + LEXER_TOKEN_PREFIX_TAIL_OFFSET < source.size()
    ...
    const std::string_view source, const bool include_trivia_tokens) noexcept
{
    ...
    while (index < source.size()) {
        ...
        if (is_decimal_digit(current)
            || (current == LEXEME_DOT && index + LEXEME_SINGLE_BYTE_WIDTH < source.size()
...
Lexer::Lexer(const base::SourceId source_id, const std::string_view source_text, base::DiagnosticSink& diagnostics,
...
base::Result<TokenBuffer> Lexer::tokenize()
{
    ...
    if (this->diagnostics_.has_error()) {
        return base::Result<TokenBuffer>::fail({base::ErrorCode::lex_error, std::string(LEXEME_LEXING_FAILED_MESSAGE)});
    }
...
{
    while (!this->is_at_end() && TOKEN_START_ACTIONS[char_class_index(this->peek())] == TokenStartAction::INVALID) {
        this->advance();
    }
...
{
    const auto match =
        match_punctuator(first, this->peek_next(), this->cursor_.peek_at(detail::PUNCTUATOR_THIRD_BYTE_OFFSET));
    if (!match.matched()) {
        return false;
    }
...
void Lexer::add_nonempty_token(const syntax::TokenKind kind, const base::usize begin, const base::usize end)
{
    this->tokens_.push_back(syntax::Token{
        kind,
