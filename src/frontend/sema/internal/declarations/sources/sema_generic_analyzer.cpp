...
bool SemanticAnalyzerCore::GenericAnalyzer::has_lifetime_origin_params(const syntax::ItemNode& item) const noexcept
{
    return std::ranges::any_of(item.generic_params, [](const syntax::GenericParamDecl& param) {
...
void SemanticAnalyzerCore::GenericAnalyzer::record_lifetime_origin_params(
    const syntax::ItemNode& item, const syntax::ItemId item_id)
{
    if (!this->has_lifetime_origin_params(item)) {
        return;
    }
    ...
    for (const syntax::GenericParamDecl& param : item.generic_params) {
        ...
        LifetimeOriginParamInfo fact;
        fact.module = owner;
        fact.item = item_id;
        fact.name = this->core_.source_name_text(param.name_id, param.name);
        fact.name_id = param.name_id;
        fact.range = param.range;
        fact.ordinal = ordinal;
        fact.part_index = part_index;
        this->core_.state_.checked.lifetime_origin_params.push_back(std::move(fact));
        ++ordinal;
    }
}
    ...
    const syntax::ItemNode& item, const syntax::ItemId item_id)
{
    ...
    this->record_lifetime_origin_params(item, item_id);
    const syntax::ModuleId owner = this->core_.item_module(item_id);
...
bool SemanticAnalyzerCore::has_lifetime_origin_params(const syntax::ItemNode& item) const noexcept
{
    return GenericAnalyzer(const_cast<SemanticAnalyzerCore&>(*this)).has_lifetime_origin_params(item);
}
...
void SemanticAnalyzerCore::record_lifetime_origin_params(const syntax::ItemNode& item, const syntax::ItemId item_id)
{
    GenericAnalyzer(*this).record_lifetime_origin_params(item, item_id);
}
