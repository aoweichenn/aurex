...
void ItemNodeList::reserve(const base::usize size)
{
    ...
    this->payloads_.macro_calls.reserve(rare);
...
AttributeDecl ItemNodeList::copy_or_move_attribute(AttributeDecl&& attribute)
{
    AttributeDecl copy;
    copy.name = attribute.name;
    copy.name_id = attribute.name_id;
    copy.range = attribute.range;
    copy.token_tree_range = attribute.token_tree_range;
    copy.token_tree = this->copy_or_move_list(std::move(attribute.token_tree));
    copy.has_token_tree = attribute.has_token_tree;
    return copy;
}
...
AttributeDecl ItemNodeList::detach_attribute(const AttributeDecl& attribute) const
{
    AttributeDecl copy;
    copy.name = attribute.name;
    copy.name_id = attribute.name_id;
    copy.range = attribute.range;
    copy.token_tree_range = attribute.token_tree_range;
    copy.token_tree = copy_detached_ast_vector(attribute.token_tree);
    copy.has_token_tree = attribute.has_token_tree;
    return copy;
}
...
base::u32 ItemNodeList::store_payload(ItemNode node)
{
    switch (node.kind) {
        ...
        case ItemKind::macro_call:
            return this->push_payload(this->payloads_.macro_calls,
                MacroCallItemPayload{
                    node.name,
                    node.name_id,
                    this->copy_or_move_attributes(std::move(node.attributes)),
                    this->copy_or_move_derives(std::move(node.derives)),
                    this->copy_or_move_list(std::move(node.macro_call_tokens)),
                    node.macro_call_range,
                    node.macro_call_balanced,
                });
    }
...
ItemNode ItemNodeList::load(const base::usize index) const
{
    ...
    switch (node.kind) {
        ...
        case ItemKind::opaque_struct_decl:
            ...
            node.attributes = this->detach_attributes(this->payloads_.opaque_structs[header.payload].attributes);
        ...
        case ItemKind::extern_block:
            node.attributes = this->detach_attributes(this->payloads_.extern_blocks[header.payload].attributes);
        ...
        case ItemKind::macro_call: {
            const MacroCallItemPayload& payload = this->payloads_.macro_calls[header.payload];
            node.name = payload.name;
            node.name_id = payload.name_id;
            node.attributes = this->detach_attributes(payload.attributes);
            node.derives = this->detach_derives(payload.derives);
            node.macro_call_tokens = copy_detached_ast_vector(payload.tokens);
            node.macro_call_range = payload.token_range;
            node.macro_call_balanced = payload.token_tree_balanced;
            loaded_known_payload = true;
            break;
        }
    }
...
ItemNode ItemNodeList::load_moved(const base::usize index)
{
    ...
    switch (node.kind) {
        ...
        case ItemKind::macro_call: {
            MacroCallItemPayload& payload = this->payloads_.macro_calls[header.payload];
            node.name = payload.name;
            node.name_id = payload.name_id;
            node.attributes = this->detach_attributes(payload.attributes);
            node.derives = this->detach_derives(payload.derives);
