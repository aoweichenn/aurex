        ...
        case TypeKind::reference:
            return this->push_payload(this->payloads_.references,
                ReferenceTypePayload{
                    node.pointer_mutability,
                    node.pointee,
                    this->copy_list(node.reference_origin.names),
                    this->copy_list(node.reference_origin.name_ids),
                    this->copy_list(node.reference_origin.ranges),
                    node.reference_origin.range,
                    node.reference_origin.explicit_,
                });
        case TypeKind::array:
...
TypeNode TypeNodeList::load(const base::usize index) const
{
    ...
    switch (header.kind) {
        ...
        case TypeKind::reference: {
            const ReferenceTypePayload& payload = this->payloads_.references[header.payload];
            node.pointer_mutability = payload.mutability;
            node.pointee = payload.pointee;
            node.reference_origin.names = copy_std_vector(payload.origin_names);
            node.reference_origin.name_ids = copy_std_vector(payload.origin_name_ids);
            node.reference_origin.ranges = copy_std_vector(payload.origin_ranges);
            node.reference_origin.range = payload.origin_range;
            node.reference_origin.explicit_ = payload.has_explicit_origin;
            break;
        }
        case TypeKind::array: {
