...
constexpr void mark_char_class(
    std::array<std::uint8_t, LEX_BYTE_CHAR_CLASS_COUNT>& table, const char c, const std::uint8_t flags) noexcept
{
    const std::size_t index = char_class_index(c);
    table[index] = static_cast<std::uint8_t>(table[index] | flags);
}

constexpr void mark_char_class_range(std::array<std::uint8_t, LEX_BYTE_CHAR_CLASS_COUNT>& table, const char first,
    const char last, const std::uint8_t flags) noexcept
{
...
[[nodiscard]] inline bool has_char_class_flags(const std::uint8_t classes, const std::uint8_t flags) noexcept
{
