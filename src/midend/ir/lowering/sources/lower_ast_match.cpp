...
IrTextId Lowerer::enum_case_symbol(const std::string_view name) noexcept
{
    ...
    if (const auto found = this->enum_cases_by_c_name_.find(existing); found != this->enum_cases_by_c_name_.end()) {
        return existing;
    }
...
const sema::EnumCaseInfo* Lowerer::enum_case_by_type_and_case(
    const sema::TypeHandle enum_type, const std::string_view case_name) const noexcept
{
...
{
    if (!sema::is_valid(enum_type) || enum_type.value >= this->module_.types.size()
    ...
    if (ok_case != nullptr && err_case != nullptr && sema::is_valid(ok_case->payload_type)
    ...
    if (some_case != nullptr && none_case != nullptr && sema::is_valid(some_case->payload_type)
...
{
    if (syntax::is_valid(id) && this->active_side_tables_.generic != nullptr
    ...
    if (syntax::is_valid(id) && this->active_side_tables_.pattern_c_name_ids != nullptr
        && id.value < this->active_side_tables_.pattern_c_name_ids->size()) {
        const std::string_view c_name =
            this->checked_.c_name_text((*this->active_side_tables_.pattern_c_name_ids)[id.value]);
        if (!c_name.empty()) {
            return this->module_.intern(c_name);
        }
    }
...
bool Lowerer::is_irrefutable_pattern(const syntax::PatternId id, const sema::TypeHandle matched_type) const
{
    struct PatternFrame {
        ...
        if (frame.expanded) {
            ...
            switch (pattern->kind) {
                ...
                case syntax::PatternKind::struct_:
                    result = sema::is_valid(frame.type)
                        && this->module_.types.get(frame.type).kind == sema::TypeKind::struct_;
        ...
        switch (pattern->kind) {
            ...
            case syntax::PatternKind::slice: {
                ...
                const sema::TypeHandle element_type =
                    info.kind == sema::TypeKind::array ? info.array_element : info.slice_element;
...
ValueId Lowerer::append_pattern_source_length(const ValueId source_address, const sema::TypeHandle source_type)
{
...
ValueId Lowerer::append_pattern_element_address(const ValueId source_address, const sema::TypeHandle source_type,
    const ValueId index, const sema::TypeHandle element_type)
{
...
ValueId Lowerer::append_pattern_condition(
    const syntax::PatternId id, const ValueId source_address, const sema::TypeHandle source_type)
{
    ...
    if (pattern->kind == syntax::PatternKind::or_pattern) {
        ...
        for (const syntax::PatternId alternative : pattern->alternatives) {
            const ValueId alternative_condition =
                this->append_pattern_condition(alternative, source_address, source_type);
            if (!is_valid(condition)) {
                condition = alternative_condition;
                continue;
            }
            condition = this->append_binary_value(BinaryOp::logical_or,
                this->module_.types.builtin(sema::BuiltinType::bool_), condition, alternative_condition);
        }
    ...
    if (pattern->kind == syntax::PatternKind::const_) {
        ...
        return this->append_binary_value(BinaryOp::equal, this->module_.types.builtin(sema::BuiltinType::bool_), source,
            this->append_value(const_ref));
    }
    if (pattern->kind == syntax::PatternKind::literal) {
        const ValueId source = this->append_load(source_address, source_type);
        Value literal = this->module_.make_value();
        literal.kind = (pattern->case_name == "true" || pattern->case_name == "false") ? ValueKind::bool_literal
    ...
    if (pattern->kind == syntax::PatternKind::tuple) {
        ...
        for (base::usize i = 0; i < count; ++i) {
            ...
            const ValueId element_condition = this->append_pattern_condition(
                pattern->elements[i], this->append_value(field), tuple.tuple_elements[i]);
            condition = is_valid(condition)
                ? this->append_binary_value(BinaryOp::logical_and,
                      this->module_.types.builtin(sema::BuiltinType::bool_), condition, element_condition)
                : element_condition;
        }
        return is_valid(condition) ? condition : this->append_true_value();
    }
    if (pattern->kind == syntax::PatternKind::slice) {
        ...
        const sema::TypeHandle element_type =
            source.kind == sema::TypeKind::array ? source.array_element : source.slice_element;
        const sema::TypeHandle usize_type = this->module_.types.builtin(sema::BuiltinType::usize);
        ...
        ValueId condition = this->append_binary_value(
            pattern->has_slice_rest ? BinaryOp::greater_equal : BinaryOp::equal, bool_type, length, fixed_count);
        ...
        const base::usize suffix_count = pattern->has_slice_rest ? pattern->elements.size() - rest_index : 0;
    ...
    if (pattern->kind == syntax::PatternKind::struct_) {
        ...
        for (const syntax::FieldPattern& field_pattern : pattern->field_patterns) {
            ...
            condition = is_valid(condition)
                ? this->append_binary_value(BinaryOp::logical_and,
                      this->module_.types.builtin(sema::BuiltinType::bool_), condition, field_condition)
                : field_condition;
        }
        return is_valid(condition) ? condition : this->append_true_value();
    }
    ...
    const ValueId source = payload_enum
        ? this->append_load(this->enum_field_addr(source_address, this->module_.intern(IR_ENUM_TAG_FIELD_NAME)),
              enum_tag_type(this->module_.types, source_type), this->module_.intern("pattern.tag"))
        : this->append_load(source_address, source_type);
    const ValueId case_id = payload_enum ? this->append_enum_tag_literal(this->module_.text(case_symbol),
                                               enum_tag_type(this->module_.types, source_type))
                                         : this->append_enum_case_ref(this->module_.text(case_symbol), source_type);
    ValueId condition = this->append_binary_value(
        BinaryOp::equal, this->module_.types.builtin(sema::BuiltinType::bool_), source, case_id);
    ...
    const BlockId payload_block = add_block(this->module_, *this->current_function_,
        "pattern.payload" + std::to_string(this->current_function_->blocks.size()));
    const BlockId exit_block = add_block(this->module_, *this->current_function_,
        "pattern.join" + std::to_string(this->current_function_->blocks.size()));
    ...
    const ValueId storage_addr =
        this->enum_field_addr(source_address, this->module_.intern(IR_ENUM_PAYLOAD_FIELD_NAME));
    ...
    if (info->payload_types.size() == 1 && pattern->payload_patterns.size() == 1) {
        payload_condition =
            this->append_pattern_condition(pattern->payload_patterns.front(), payload_addr, info->payload_type);
    } else {
        ...
        for (base::usize i = 0; i < count; ++i) {
            ...
            const ValueId field_condition = this->append_pattern_condition(
                pattern->payload_patterns[i], this->append_value(field), info->payload_types[i]);
            payload_condition = is_valid(payload_condition)
                ? this->append_binary_value(BinaryOp::logical_and,
                      this->module_.types.builtin(sema::BuiltinType::bool_), payload_condition, field_condition)
                : field_condition;
        }
    }
...
ValueId Lowerer::lower_match_expr(const syntax::ExprId expr_id, const ExprView& expr)
{
    ...
    const BlockId join_block =
        add_block(this->module_, *current_function_, "match.join" + std::to_string(current_function_->blocks.size()));
    ...
    for (base::usize i = 0; i < expr.match_arms.size(); ++i) {
        ...
        const BlockId arm_block = add_block(
            this->module_, *current_function_, "match.arm" + std::to_string(current_function_->blocks.size()));
        ...
        if (implicit_fallback) {
            ...
            next_test_block = add_block(
                this->module_, *current_function_, "match.next" + std::to_string(current_function_->blocks.size()));
            if (fallback) {
                append_branch_if_open(arm_block);
            } else {
        ...
        if (guarded) {
            arm_body_block = add_block(this->module_, *current_function_,
                "match.guard.pass" + std::to_string(current_function_->blocks.size()));
...
ValueId Lowerer::lower_try_expr(const syntax::ExprId expr_id, const ExprView& expr)
{
    ...
    if (source_shape.kind == TryShapeKind::none || source_shape.kind != return_shape.kind
        || source_shape.success_case == nullptr || source_shape.failure_case == nullptr
    ...
    const ValueId tag = append_load(enum_field_addr(source_slot, this->module_.intern(IR_ENUM_TAG_FIELD_NAME)),
        tag_type, this->module_.intern("try.tag"));
    ...
    const BlockId success_block =
        add_block(this->module_, *current_function_, "try.ok" + std::to_string(current_function_->blocks.size()));
    const BlockId failure_block =
        add_block(this->module_, *current_function_, "try.err" + std::to_string(current_function_->blocks.size()));
    const BlockId continue_block =
        add_block(this->module_, *current_function_, "try.join" + std::to_string(current_function_->blocks.size()));
    ...
    const ValueId success_value = append_enum_payload_load(source_slot, result_type, this->module_.intern("try.ok"));
...
{
    const sema::TypeHandle expected_payload_type =
        enum_case.payload_types.empty() ? enum_case.payload_type : enum_case.payload_types.front();
    const ValueId payload_value =
        syntax::is_valid(payload_expr) ? lower_expr(payload_expr, expected_payload_type) : INVALID_VALUE_ID;
    return append_enum_constructor(enum_case, payload_value);
}

ValueId Lowerer::lower_enum_constructor_call(const sema::EnumCaseInfo& enum_case, const ExprView& expr)
{
    ...
    for (base::usize i = 0; i < field_count; ++i) {
        const ValueId value =
            this->coerce_value(this->lower_expr(expr.args[i], enum_case.payload_types[i]), enum_case.payload_types[i]);
...
ValueId Lowerer::append_enum_payload_load(
    const ValueId enum_slot, const sema::TypeHandle payload_type, const IrTextId name)
{
...
ValueId Lowerer::append_binary_value(
    const BinaryOp op, const sema::TypeHandle type, const ValueId lhs, const ValueId rhs)
{
    Value value = this->module_.make_value();
...
ValueId Lowerer::enum_field_addr(const ValueId object, const IrTextId field_name)
{
    ...
    value.type = module_.types.pointer(
        sema::PointerMutability::mut, aggregate_field_type(enum_type, this->module_.text(field_name)));
    return append_value(value);
}

void Lowerer::bind_pattern_locals(
    const syntax::PatternId pattern_id, const ValueId source_address, const sema::TypeHandle source_type)
{
    this->bind_pattern_locals_with_mutability(pattern_id, source_address, source_type, false);
}

void Lowerer::collect_pattern_binding_slots(const syntax::PatternId pattern_id, const sema::TypeHandle source_type,
    const bool is_mutable, std::unordered_map<sema::IdentId, PatternBindingSlot, sema::IdentIdHash>& slots)
{
        ...
        switch (pattern->kind) {
            ...
            case syntax::PatternKind::struct_:
                for (auto field_pattern = pattern->field_patterns.rbegin();
                    field_pattern != pattern->field_patterns.rend(); ++field_pattern) {
                    pending.push_back(PatternTypeFrame{
...
void Lowerer::store_pattern_bindings(const syntax::PatternId pattern_id, const ValueId source_address,
    ...
    while (!pending.empty()) {
        ...
        switch (pattern->kind) {
            ...
            case syntax::PatternKind::binding: {
                ...
                if (slot != slots.end()) {
                    this->append_store(
                        slot->second.slot, this->append_load(frame.address, frame.type, slot->second.name));
                }
                break;
            }
            case syntax::PatternKind::tuple: {
                ...
                for (base::usize i = count; i > 0; --i) {
                    ...
                    field.type =
                        this->module_.types.pointer(sema::PointerMutability::mut, tuple.tuple_elements[element_index]);
                    pending.push_back(PatternFrame{
                        pattern->elements[element_index],
            ...
            case syntax::PatternKind::enum_case: {
                ...
                for (base::usize i = count; i > 0; --i) {
                    ...
                    field.kind = ValueKind::field_addr;
                    field.type =
                        this->module_.types.pointer(sema::PointerMutability::mut, info->payload_types[payload_index]);
                    field.name = this->module_.intern(
                        std::string(IR_ENUM_SYNTHETIC_PAYLOAD_FIELD_PREFIX) + std::to_string(payload_index));
                    field.object = payload_addr;
                    pending.push_back(PatternFrame{
                        pattern->payload_patterns[payload_index],
                        this->append_value(field),
...
void Lowerer::bind_pattern_locals_with_mutability(const syntax::PatternId pattern_id, const ValueId source_address,
    const sema::TypeHandle source_type, const bool is_mutable)
{
    ...
    const BlockId join_block = add_block(this->module_, *this->current_function_,
        "pattern.bind.join" + std::to_string(this->current_function_->blocks.size()));
    for (base::usize i = 0; i < pattern->alternatives.size(); ++i) {
        const syntax::PatternId alternative = pattern->alternatives[i];
        const BlockId alternative_block = add_block(this->module_, *this->current_function_,
            "pattern.bind.alt" + std::to_string(this->current_function_->blocks.size()));
        BlockId next_block = INVALID_BLOCK_ID;
        if (i + 1 == pattern->alternatives.size()) {
            this->append_branch_if_open(alternative_block);
        } else {
            next_block = add_block(this->module_, *this->current_function_,
                "pattern.bind.next" + std::to_string(this->current_function_->blocks.size()));
