...
{
    if (lhs.template_def != rhs.template_def || lhs.const_args != rhs.const_args || lhs.param_env != rhs.param_env
        || lhs.global_id != rhs.global_id || lhs.type_args.size() != rhs.type_args.size()) {
        return false;
    }
...
GenericInstanceKey generic_instance_key(const DefKey template_def, const std::span<const CanonicalTypeKey> type_args,
    const std::span<const StableFingerprint128> const_args, const ParamEnvKey param_env)
{
...
std::string debug_string(const ParamEnvKey key)
{
    ...
    out << "ParamEnvKey{global=" << key.global_id << ",predicates=" << debug_string(key.predicates) << '}';
    return out.str();
}
...
{
    std::ostringstream out;
    out << "GenericInstanceKey{global=" << key.global_id << ",fingerprint=" << debug_string(stable_key_fingerprint(key))
        << '}';
    return out.str();
}
