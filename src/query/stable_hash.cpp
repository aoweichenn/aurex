...
base::u64 stable_mix(const base::u64 seed, const base::u64 value) noexcept {
    return seed ^ (value + QUERY_STABLE_HASH_MIX_INCREMENT + (seed << QUERY_STABLE_HASH_MIX_LEFT_SHIFT) + (seed >> QUERY_STABLE_HASH_MIX_RIGHT_SHIFT));
}
...
void StableHashBuilder::mix_u64(const base::u64 value) noexcept {
    for (base::usize index = 0; index < QUERY_STABLE_U64_BYTES; ++index) {
        this->mix_raw_byte(static_cast<base::u8>(
            (value >> (index * QUERY_STABLE_BYTE_SHIFT)) & QUERY_STABLE_BYTE_MASK));
    }
}
...
std::string debug_string(const StableFingerprint128 fingerprint) {
    ...
    out << std::hex << std::setfill(QUERY_FINGERPRINT_HEX_FILL)
        << std::setw(QUERY_FINGERPRINT_HEX_WIDTH) << fingerprint.primary
        << ':'
        << std::setw(QUERY_FINGERPRINT_HEX_WIDTH) << fingerprint.secondary
        << std::dec << ':' << fingerprint.byte_count;
    return out.str();
}

StableKeyWriter::StableKeyWriter()
    : bytes_(),
      hash_() {
}
