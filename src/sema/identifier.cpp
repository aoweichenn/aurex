...
[[nodiscard]] base::u64 pack_lookup_key_parts(const base::u32 high, const base::u32 low) noexcept
{
    return (static_cast<base::u64>(high) << SEMA_LOOKUP_KEY_U32_SHIFT) | static_cast<base::u64>(low);
}
...
{
    return static_cast<std::size_t>(identity.value ^ (identity.value >> SEMA_GENERIC_PARAM_IDENTITY_HASH_SHIFT));
}

std::size_t MethodLookupKeyHash::operator()(const MethodLookupKey key) const noexcept
{
    const std::size_t module_name_hash = std::hash<base::u64>{}(pack_lookup_key_parts(key.module, key.name.value));
