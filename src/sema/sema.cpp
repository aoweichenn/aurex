...
SemanticAnalyzer::SemanticAnalyzer(
    ...
    : module_(module), diagnostics_(diagnostics), options_(options), arena_(std::make_unique<base::BumpAllocator>()),
      named_types_(
          make_sema_map<ModuleLookupKey, TypeHandle, ModuleLookupKeyHash>(*this->arena_, ModuleLookupKeyHash{})),
      generic_struct_templates_(make_sema_map<ModuleLookupKey, GenericTemplateInfo, ModuleLookupKeyHash>(
          *this->arena_, ModuleLookupKeyHash{})),
      generic_enum_templates_(make_sema_map<ModuleLookupKey, GenericTemplateInfo, ModuleLookupKeyHash>(
          *this->arena_, ModuleLookupKeyHash{})),
      generic_type_alias_templates_(make_sema_map<ModuleLookupKey, GenericTemplateInfo, ModuleLookupKeyHash>(
          *this->arena_, ModuleLookupKeyHash{})),
      generic_function_templates_(make_sema_map<ModuleLookupKey, GenericTemplateInfo, ModuleLookupKeyHash>(
          *this->arena_, ModuleLookupKeyHash{})),
      ...
      generic_function_instances_(
          make_sema_map<FunctionLookupKey, base::usize, FunctionLookupKeyHash>(*this->arena_, FunctionLookupKeyHash{})),
      generic_placeholder_functions_(make_sema_map<FunctionLookupKey, FunctionSignature, FunctionLookupKeyHash>(
          *this->arena_, FunctionLookupKeyHash{})),
      resolved_type_aliases_(
          make_sema_map<ModuleLookupKey, TypeHandle, ModuleLookupKeyHash>(*this->arena_, ModuleLookupKeyHash{})),
      resolving_type_aliases_(make_sema_vector<ModuleLookupKey>(*this->arena_)),
      global_values_(
          make_sema_map<FunctionLookupKey, Symbol, FunctionLookupKeyHash>(*this->arena_, FunctionLookupKeyHash{})),
      function_definition_items_(make_sema_map<FunctionLookupKey, syntax::ItemId, FunctionLookupKeyHash>(
          *this->arena_, FunctionLookupKeyHash{})),
      function_body_states_(make_sema_map<FunctionLookupKey, FunctionBodyState, FunctionLookupKeyHash>(
          *this->arena_, FunctionLookupKeyHash{})),
      struct_infos_by_type_(make_sema_map<base::u32, const StructInfo*>(*this->arena_)),
      named_types_by_name_(
          make_sema_map<ModuleLookupKey, IndexedTypeInfo, ModuleLookupKeyHash>(*this->arena_, ModuleLookupKeyHash{})),
      ...
      methods_by_name_(make_sema_map<MethodLookupKey, const FunctionSignature*, MethodLookupKeyHash>(
          *this->arena_, MethodLookupKeyHash{})),
      global_values_by_name_(
          make_sema_map<ModuleLookupKey, const Symbol*, ModuleLookupKeyHash>(*this->arena_, ModuleLookupKeyHash{})),
      method_global_values_by_name_(
          make_sema_map<MethodLookupKey, const Symbol*, MethodLookupKeyHash>(*this->arena_, MethodLookupKeyHash{})),
      ...
      enum_cases_by_type_and_case_(make_sema_map<EnumCaseLookupKey, const EnumCaseInfo*, EnumCaseLookupKeyHash>(
          *this->arena_, EnumCaseLookupKeyHash{})),
...
SemanticAnalyzer::GenericTemplateList& SemanticAnalyzer::generic_method_template_bucket(const ModuleLookupKey& key)
{
...
{
    if (const auto found = this->enum_cases_by_type_.find(enum_type.value); found != this->enum_cases_by_type_.end()) {
        return found->second;
    }
...
SemanticAnalyzer::CapabilitySet& SemanticAnalyzer::capability_bucket(CapabilityMap& map, const IdentId key) const
{
...
base::Result<CheckedModule> SemanticAnalyzer::analyze()
{
    ...
    this->checked_.reserve_side_table_storage(this->module_.exprs.size(), this->module_.patterns.size(),
        this->module_.types.size(), this->module_.stmts.size(), this->module_.items.size());
    ...
    const base::usize expected_identifier_count =
        this->module_.identifiers.size() + item_counts.items + item_counts.enum_cases + item_counts.modules;
...
bool SemanticAnalyzer::validate_ast_contract() const
{
    ...
    for (base::usize i = 0; i < count; ++i) {
        ...
        if (!syntax::is_valid(owner) || owner.value >= this->module_.modules.size()) {
            this->report_internal_contract(this->module_.items.range(i), std::string(SEMA_AST_ITEM_MODULE_INVALID));
            valid = false;
        }
    }
    return valid;
