    ...
    if (result == nullptr && report_unknown) {
        if (inaccessible_result != nullptr) {
            this->report_visibility(
                range, sema_private_method_message(this->state_.checked.types.display_name(owner_type), name));
            return nullptr;
        }
        this->report_lookup(
    ...
    const base::SourceRange& range, const bool opaque_allowed_as_pointee, const bool report_unknown)
{
    ...
    if (is_valid(lookup_key)) {
        ...
        if (const auto found = this->state_.names.type_aliases_by_name.find(lookup_key);
            found != this->state_.names.type_aliases_by_name.end() && found->second != nullptr) {
    ...
    const bool report_unknown)
{
    ...
    for (const syntax::ModuleId candidate_module : this->module_export_modules(module)) {
        ...
        if (is_valid(lookup_key)) {
            ...
            } else if (const auto alias_found = this->state_.names.type_aliases_by_name.find(lookup_key);
                alias_found != this->state_.names.type_aliases_by_name.end() && alias_found->second != nullptr) {
    ...
    const IdentId name_id, const std::string_view name, const base::SourceRange& range, const bool report_unknown)
{
    const ModuleLookupKey lookup_key = this->find_module_lookup_key(this->state_.flow.current_module, name_id);
    if (is_valid(lookup_key)) {
        if (const auto found = this->state_.names.functions_by_name.find(lookup_key);
            found != this->state_.names.functions_by_name.end()) {
            return found->second;
        }
    }
    ...
    const IdentId name_id, const std::string_view name, const base::SourceRange& range, const bool report_unknown)
{
    const ModuleLookupKey lookup_key = this->find_module_lookup_key(this->state_.flow.current_module, name_id);
    if (is_valid(lookup_key)) {
        if (const auto found = this->state_.names.enum_cases_by_module_name.find(lookup_key);
            found != this->state_.names.enum_cases_by_module_name.end()) {
            return found->second;
        }
    }
    ...
    const base::SourceRange& range, const bool report_unknown)
{
    const ModuleLookupKey enum_lookup_key =
        this->find_module_lookup_key(this->state_.flow.current_module, enum_name_id);
    const bool enum_type_name_exists = is_valid(enum_lookup_key)
        && (this->state_.names.named_types_by_name.contains(enum_lookup_key)
            || this->state_.names.type_aliases_by_name.contains(enum_lookup_key));
...
{
    if (!syntax::is_valid(callee_id) || callee_id.value >= this->ctx_.module.exprs.size()) {
        return nullptr;
    }
    const syntax::FieldExprPayload* const callee = this->ctx_.module.exprs.field_payload(callee_id.value);
    if (callee == nullptr || !syntax::is_valid(callee->object)
        || callee->object.value >= this->ctx_.module.exprs.size()) {
        return nullptr;
    }
    ...
    if (this->state_.checked.types.get(enum_type).kind != TypeKind::enum_) {
        if (report_unknown) {
            this->report_general(
                this->ctx_.module.exprs.range(callee_id.value), std::string(SEMA_ENUM_CASE_SCOPE_TYPE));
        }
        return nullptr;
    }
    ...
    if (report_unknown) {
        this->report_lookup(this->ctx_.module.exprs.range(callee_id.value),
            sema_unknown_scoped_enum_case_message(
                this->state_.checked.types.display_name(enum_type), callee->field_name));
        this->report_lookup_suggestion(this->ctx_.module.exprs.range(callee_id.value),
...
{
    if (const Symbol* local = this->state_.names.symbols.find(name_id); local != nullptr) {
        return local;
    }
