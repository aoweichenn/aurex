...
ExprId AstModule::push_literal_expr(
    const ExprKind kind,
...
ExprId AstModule::push_call_expr(
    const ExprKind kind,
    const base::SourceRange& range,
...
ExprId AstModule::push_index_expr(
    const base::SourceRange& range,
    const ExprId object,
    const ExprId index) {
...
void AstModule::set_generic_apply_expr(
    ...
    const base::SourceRange& range,
    GenericApplyExprPayload payload) {
...
void AstModule::set_struct_literal_expr(
    const base::usize index,
    const base::SourceRange& range,
    StructLiteralExprPayload payload) {
...
void AstModule::reserve_for_estimate(const AstReserveEstimate& estimate) {
    ...
    this->types.reserve(ast_reserve_at_least(
        INITIAL_CAPACITY,
        ast_reserve_larger(type_capacity, ast_reserve_fraction(estimate.tokens, SYNTAX_AST_RESERVE_TYPE_TOKEN_DIVISOR))));
    this->exprs.reserve_touched(expr_capacity);
    this->patterns.reserve(ast_reserve_at_least(
        INITIAL_CAPACITY,
        ast_reserve_larger(pattern_capacity, ast_reserve_fraction(estimate.tokens, SYNTAX_AST_RESERVE_PATTERN_TOKEN_DIVISOR))));
    this->stmts.reserve(ast_reserve_at_least(
        INITIAL_CAPACITY,
        ast_reserve_larger(estimate.statements, ast_reserve_fraction(estimate.tokens, SYNTAX_AST_RESERVE_STMT_TOKEN_DIVISOR))));
    const base::usize item_capacity = ast_reserve_at_least(
        INITIAL_CAPACITY,
        ast_reserve_larger(estimate.items, ast_reserve_fraction(estimate.tokens, SYNTAX_AST_RESERVE_ITEM_TOKEN_DIVISOR)));
