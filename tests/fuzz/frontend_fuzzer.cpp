...
void exercise_source(const std::string_view source) {
    ...
    static_cast<void>(ir::run_pass_pipeline(lowered.value(), ir::PassPipelineOptions {
...
extern "C" int LLVMFuzzerTestOneInput(const std::uint8_t* data, const std::size_t size) {
    ...
    const std::string generated = aurex::test::generated_source_from_seed(
        aurex::test::seed_from_bytes(data, size)
    );
