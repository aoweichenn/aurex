...
TEST_F(AurexIntegrationTest, TryExpressionDiagnostics)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(non_result)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(return_mismatch)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(error_mismatch)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(malformed_result)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(malformed_option)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(extra_result_case)).output,
        "try expression requires result-like ok/err enum or option-like some/none enum");

    const fs::path extra_option_case = negative_sample("error_handling", "try_option_extra_case.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(extra_option_case)).output,
        "try expression requires result-like ok/err enum or option-like some/none enum");
}
