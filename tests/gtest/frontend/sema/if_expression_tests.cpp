...
TEST_F(AurexIntegrationTest, IfExpression)
{
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(condition)).output, "if expression condition must be bool");
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(missing_else)).output, "if expression requires an else branch");
