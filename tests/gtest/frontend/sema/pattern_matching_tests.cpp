...
TEST_F(AurexIntegrationTest, MatchExpression)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(non_enum)).output,
    ...
    const fs::path bool_wildcard_unreachable =
        negative_sample("pattern_matching", "bool_match_wildcard_unreachable.ax");
    expect_contains(
        require_failure(aurexc() + " --check " + q(bool_wildcard_unreachable)).output, "match arm is unreachable");
...
TEST_F(AurexIntegrationTest, EnumPayloadAndMatchBinding)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(missing_arg)).output,
        "enum payload constructor requires exactly one argument");

    const fs::path scoped_missing_arg = negative_sample("pattern_matching", "enum_scoped_constructor_missing_arg.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(scoped_missing_arg)).output,
        "enum payload constructor requires exactly one argument");

    const fs::path wrong_type = negative_sample("pattern_matching", "enum_payload_constructor_wrong_type.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(wrong_type)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(empty_binding)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(multi_payload_arity)).output,
        "enum payload pattern requires 2 bindings");

    const fs::path single_payload_arity = negative_sample("pattern_matching", "enum_single_payload_pattern_arity.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(single_payload_arity)).output,
        "enum payload pattern requires 1 binding");
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(array_storage)).output, "enum payload cannot contain array storage");

    const fs::path bool_payload_missing = negative_sample("pattern_matching", "enum_payload_bool_missing_witness.ax");
    expect_contains(
        require_failure(aurexc() + " --check " + q(bool_payload_missing)).output, "match expression is not exhaustive");
}
...
TEST_F(AurexIntegrationTest, MatchWildcardAndScopedCases)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(wrong_enum)).output,
...
TEST_F(AurexIntegrationTest, StructuralMatchExhaustiveness)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(guarded)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(tuple_open)).output,
        "match expression over tuple, struct, array, or slice requires an irrefutable arm");

    const fs::path tuple_bool_wildcard =
        negative_sample("pattern_matching", "tuple_bool_wildcard_tail_missing_witness.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(tuple_bool_wildcard)).output,
        "match expression over tuple, struct, array, or slice requires an irrefutable arm");

    const fs::path array_missing = negative_sample("pattern_matching", "array_bool_missing_witness.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(array_missing)).output,
        "match expression over tuple, struct, array, or slice requires an irrefutable arm");

    const fs::path slice_missing = negative_sample("pattern_matching", "slice_missing_witness.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(slice_missing)).output,
        "match expression over dynamic slice is missing length or element coverage");

    const fs::path slice_dynamic_length = positive_sample("pattern_matching", "slice_dynamic_length_exhaustive.ax");
    require_success(aurexc() + " --check " + q(slice_dynamic_length));

    const fs::path slice_dynamic_missing = negative_sample("pattern_matching", "slice_dynamic_length_missing_empty.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(slice_dynamic_missing)).output,
        "match expression over dynamic slice is missing length or element coverage");

    const fs::path limit = negative_sample("pattern_matching", "structural_match_exhaustiveness_limit.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(limit)).output,
        "match expression over tuple, struct, array, or slice requires an irrefutable arm");

    const fs::path large_array_irrefutable = positive_sample("pattern_matching", "large_array_irrefutable_match.ax");
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(large_array_missing)).output,
...
TEST_F(AurexIntegrationTest, MatchOrPattern)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(payload_binding)).output,
        "or-pattern alternatives must bind the same names");

    const fs::path name_mismatch = negative_sample("pattern_matching", "or_pattern_binding_name_mismatch.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(name_mismatch)).output,
        "or-pattern alternatives must bind the same names");

    const fs::path type_mismatch = negative_sample("pattern_matching", "or_pattern_binding_type_mismatch.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(type_mismatch)).output,
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(refutable_payload)).output, "match expression is not exhaustive");
}
...
TEST_F(AurexIntegrationTest, PatternRemainingSliceLetElseAndBindingOr)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(non_slice)).output,
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(length)).output, "slice pattern length does not match array length");

    const fs::path fallthrough = negative_sample("pattern_matching", "let_else_fallthrough.ax");
    expect_contains(
        require_failure(aurexc() + " --check " + q(fallthrough)).output, "let-else else block must not fall through");
...
TEST_F(AurexIntegrationTest, MatchLiteralPattern)
{
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(missing_wildcard)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(duplicate_integer)).output, "match arm is unreachable");

    const fs::path type_mismatch = negative_sample("pattern_matching", "match_literal_type_mismatch.ax");
    expect_contains(
        require_failure(aurexc() + " --check " + q(type_mismatch)).output, "bool match pattern must be true or false");

    const fs::path enum_literal = negative_sample("pattern_matching", "match_enum_literal_pattern.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(enum_literal)).output,
...
TEST_F(AurexIntegrationTest, MatchGuard)
{
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(not_exhaustive)).output, "match expression is not exhaustive");

    const fs::path literal_false = negative_sample("pattern_matching", "match_guard_literal_false_not_exhaustive.ax");
    expect_contains(require_failure(aurexc() + " --check " + q(literal_false)).output,
