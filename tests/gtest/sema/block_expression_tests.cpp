...
TEST_F(AurexIntegrationTest, BlockExpression)
{
    ...
    expect_contains(
        require_failure(aurexc() + " --check " + q(empty)).output, "block expression requires a final expression");

    const fs::path void_result = negative_sample("expressions", "block_expression_void.ax");
    expect_contains(
        require_failure(aurexc() + " --check " + q(void_result)).output, "block expression result cannot be void");
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(const_initializer)).output,
    ...
    expect_contains(require_failure(aurexc() + " --check " + q(unreachable_tail)).output,
