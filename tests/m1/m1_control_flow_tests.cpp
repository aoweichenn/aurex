...
TEST_F(AurexIntegrationTest, M1IfExpression) {
    const fs::path source = source_root() / "tests" / "m1" / "positive" / "if_expression.ax";
    ...
    const fs::path bin = test_bin_root() / "m1_if_expression";
    require_success(aurexc() + " " + q(source) + " -o " + q(bin));
    EXPECT_EQ(require_success(q(bin)).output, "");

    const fs::path condition = source_root() / "tests" / "m1" / "negative" / "if_expression_condition.ax";
    expect_contains(require_failure(aurexc() + " --check " + q(condition)).output, "if expression condition must be bool");

    const fs::path mismatch = source_root() / "tests" / "m1" / "negative" / "if_expression_type_mismatch.ax";
    expect_contains(require_failure(aurexc() + " --check " + q(mismatch)).output, "if expression branches must have the same type");

    const fs::path missing_else = source_root() / "tests" / "m1" / "negative" / "if_expression_missing_else.ax";
    expect_contains(require_failure(aurexc() + " --check " + q(missing_else)).output, "if expression requires else branch");

    const fs::path const_initializer = source_root() / "tests" / "m1" / "negative" / "if_expression_const_initializer.ax";
