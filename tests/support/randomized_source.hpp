...
public:
    ...
    [[nodiscard]] std::uint64_t next() noexcept
    {
        ...
        value = (value ^ (value >> RANDOM_SOURCE_SPLITMIX_FIRST_SHIFT)) * RANDOM_SOURCE_SPLITMIX_FIRST_MULTIPLIER;
        value = (value ^ (value >> RANDOM_SOURCE_SPLITMIX_SECOND_SHIFT)) * RANDOM_SOURCE_SPLITMIX_SECOND_MULTIPLIER;
...
[[nodiscard]] std::string_view choose(DeterministicRandom& random, const std::array<std::string_view, Size>& values)
{
...
inline void append_statement(DeterministicRandom& random, std::ostringstream& out, const base::usize index)
{
    ...
    switch (random.index(RANDOM_SOURCE_STATEMENT_SHAPE_COUNT)) {
        ...
        case 4:
            out << "  for var loop_" << index << ": i32 = 0; loop_" << index << " < "
                << (random.index(RANDOM_SOURCE_MAX_ARRAY_LENGTH) + 1U) << "; loop_" << index << " += 1 { total += loop_"
                << index << "; }\n";
            break;
        case 5:
            ...
            break;
        case 6:
            out << "  let " << local << ": i32 = match Mode.a { .a => " << i32_expr(random) << ", .b => "
                << i32_expr(random) << ", };\n";
            break;
        case 7:
...
[[nodiscard]] inline std::string parser_recovery_source(DeterministicRandom& random, const base::usize case_index)
{
