...
def append_ast_mixed_prelude(source: list[str]) -> None:
    source.append(
        ...
        "    };\n"
        "}\n\n"
        "fn unwrap_maybe<T>(value: Maybe<T>, fallback: T) -> T where T: Copy {\n"
        "    return match value {\n"
        "        .some(inner) => inner,\n"
        "        .none => fallback,\n"
        "    };\n"
...
def append_ast_feature_block(source: list[str]) -> None:
    source.append(
        ...
        "        let ascii_view: []u8 = ascii_bytes[:];\n"
        "        let ascii_text: str = strfromutf8(ascii_view);\n"
        "        let raw_text: str = unsafe { from_raw(\"ok\".ptr, \"ok\".len) };\n"
        "        if strvalid(ascii_view) && ascii_text.len == raw_text.len {\n"
        "            total += ((raw_text.len) as i32);\n"
        "        }\n\n"
        "        unsafe {\n"
        "            let zero_bits: f32 = bitcast<f32>(((0) as u32));\n"
        "            if sizeof<Packet>() >= alignof<Packet>() {\n"
        "                total += ((sizeof<Code>()) as i32);\n"
        "            }\n"
        "        }\n\n"
...
def append_mixed_statement(source: list[str], index: int, remaining: int) -> int:
    ...
    if case == 8 and remaining >= 4:
        ...
        source.append(
            ...
            f"        let local_slice_{suffix}: []i32 = local_values_{suffix}[:];\n"
