...
AIR_TEXT="${ROOT}/selfhost/src/aurex/selfhost/smoke/air_text.ax"
...
AIR_TEXT_BIN="${SELFHOST_BUILD_DIR}/air_text"
...
"${AUREXC}" "${SELFHOST_IMPORT_FLAGS[@]}" --check "${AIR_TEXT}"
"${AUREXC}" "${SELFHOST_IMPORT_FLAGS[@]}" "${AIR_TEXT}" -o "${AIR_TEXT_BIN}"
test "$("${AIR_TEXT_BIN}" "${SELFHOST_BUILD_DIR}/air_text.air")" = "selfhost air text ok"
grep -q 'block_param %v' "${SELFHOST_BUILD_DIR}/air_text.air"
grep -q 'edge fallthrough \^air0 -> \^air1 args(%v' "${SELFHOST_BUILD_DIR}/air_text.air"
...
grep -q 'air_edges v0' "${STAGE1_FLOW_TAC}"
...
grep -q 'slot_addr' "${STAGE1_FLOW_TAC}"
grep -q 'load' "${STAGE1_FLOW_TAC}"
grep -q 'read(%v' "${STAGE1_FLOW_TAC}"
grep -q 'store' "${STAGE1_FLOW_TAC}"
grep -q 'effect(%v' "${STAGE1_FLOW_TAC}"
...
grep -q 'term jump \^air0' "${STAGE1_FLOW_TAC}"
grep -q 'edge while %t' "${STAGE1_FLOW_TAC}"
grep -q 'false \^air1' "${STAGE1_FLOW_TAC}"
grep -q 'cfg \^air1 edge if' "${STAGE1_FLOW_TAC}"
...
grep -q 'field_addr' "${STAGE1_PLACE_TAC}"
grep -q 'index_addr' "${STAGE1_PLACE_TAC}"
grep -q 'deref_addr' "${STAGE1_PLACE_TAC}"
grep -q 'load' "${STAGE1_PLACE_TAC}"
grep -q 'read(%v' "${STAGE1_PLACE_TAC}"
