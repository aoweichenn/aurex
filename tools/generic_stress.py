...
modules, runs `aurexc --check`, and records elapsed time plus peak RSS. Keep
threshold policy out of this script until the baseline is stable across
machines and compiler configurations.
"""
...
DEFAULT_COUNTS = (200, 500, 1000, 2000)
...
def write_source(instance_count: int) -> pathlib.Path:
    GENERATED_ROOT.mkdir(parents=True, exist_ok=True)
    path = GENERATED_ROOT / f"generic_stress_{instance_count}.ax"
    source = make_generic_stress_source(instance_count)
...
def print_report(rows: list[StressRow]) -> None:
    ...
    print()
    print(f"{'instances':>10} {'source_KiB':>12} {'elapsed_ms':>12} {'peak_RSS_MiB':>14} {'source':<36}")
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="comma-separated instance counts; default: 200,500,1000,2000",
    )
    parser.add_argument(
        "--skip-build",
        action="store_true",
